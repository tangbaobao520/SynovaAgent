# Task Brief: 哨兵文件驱动化 — extensions/sentinels/ 自动发现注册

> 生成: 2026-06-23 19:37:54 | 分支: feat/prompt-architecture | as any: 0
...
### a) 项目拼图
哨兵系统, L3洞察层。已有: sentinel-loader.ts(扫描4个manifest), registry.ts(全局注册表), file-driven-loaders.ts(启动调用)
本任务: 扩展 — 已有loader只扫描不注册, 补注册桥接

### b) 文件审计
extensions/sentinels/: 4个哨兵(cost-health/revenue-health/cash-runway/profit-health) + shared/
src/sentinel/sentinel-loader.ts: 已有loadSentinels(), 缺registerLoadedSentinels()
src/l3/key-person-risk.ts: 硬编码在registry — 需文件化
关系: 扩展 — 已有基础设施, 只差注册调用

### c) 决策
扩展: loader加registerLoadedSentinels(), file-driven-loaders调用注册, KPR加manifest

## Q1: 调研 — 这件事以前怎么做的？

### a) 业界最佳实践
文件驱动 = 扫描目录→读manifest→动态import→注册。已有loadSentinels()扫描, 缺注册步骤。
