# Task Brief: T1 Batch4: HONA+EOB compute重建+KeyPersonRisk补TOOLS

> 生成: 2026-06-24 04:13:22 | 分支: feat/prompt-architecture | as any: 0
...
### a) 项目拼图
T1 Batch4(最后一批): HONA+EOB compute重建+KeyPersonRisk补TOOLS。零engine-core import。

### b) 文件审计
已有: extensions/sentinels/cost-health(模式)。新建: hona/eob computess/。扩展: org/TOOLS(HONA+KeyPersonRisk), business_model/THEORY+TOOLS+RULES(EOB)。无冲突。

### c) 决策
重写(rewrite): L4 GraphStore纯函数。 重读上方 §项目身份。本任务属于哪个系统(GA诊断/哨兵/基础设施)？触及哪层？该层现有模块？新增/替换/扩展？ -->
...
## Q2: 方案
重写: HONA网络拓扑分析 + EOB弹性边界评估 + KeyPersonRisk补充TOOLS。

## Q3: 验收
verify: grep -r engine-core extensions/sentinels/{hona,eob}/computes/ 零结果
...
- [x] verify: expert/org/TOOLS.md 含HONA+KeyPersonRisk
- [x] verify: expert/business_model/TOOLS.md 含EOB边界分析
