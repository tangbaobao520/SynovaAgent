# Task Brief: T3: 3财务哨兵填充 revenue+cash+profit

> 生成: 2026-06-24 04:45:04 | 分支: feat/prompt-architecture | as any: 0
...
### a) 项目拼图
T3: 3财务哨兵(revenue/cash/profit) manifest+aggregate填充。每哨兵含真实计算逻辑+thresholds+L4 GraphStore数据源。零engine-core import。

### b) 文件审计
已有: extensions/sentinels/cost-health(参考模式)。新建: revenue/cash/profit manifest+aggregate。无冲突。

### c) 决策
重写(rewrite): aggregate纯函数+L4 GraphStore。

## Q1: 调研
memory/engine-core-split-fraud.md: 桥接欺诈。本批次直接重写aggregate逻辑。

## Q2: 方案
重写: revenue(CR5+增长率), cash(跑道+应收), profit(利润率+行业对标)

## Q3: 验收
verify: grep -r engine-core extensions/sentinels/{revenue,cash,profit}-health/ 零结果

## 本任务在哪一层
L3(sentinel aggregate)。不跨层。

## Done 标准
- [x] verify: 零 engine-core import
- [x] verify: revenue-health/aggregate.ts 有真实Finding
- [x] verify: cash-runway/aggregate.ts 有真实Finding
- [x] verify: profit-health/aggregate.ts 有真实Finding
