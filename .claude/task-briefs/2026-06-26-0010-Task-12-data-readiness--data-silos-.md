# Task Brief: Task 1.2: data-readiness + data-silos → T3 数据健康度

> 生成: 2026-06-26 00:10:21 | 分支: feat/prompt-architecture | as any: 0
...
本任务属于哨兵系统 Phase 1 迁移。触及 L3（extensions/sentinels/）。
合并 2 个旧适配器的能力，按技术方案重写为 T3 数据健康度哨兵。

### b) 文件审计
旧文件（待删除）：
- src/sentinel/adapters/data-readiness-sentinel.ts
- src/sentinel/adapters/data-silos-sentinel.ts
新目录（待创建）：
- extensions/sentinels/data-health/
关系: 新建（旧适配器仅供参考，按技术方案重写）

### c) 决策
无覆盖冲突。技术方案要求合并为 T3 哨兵，按新签名重写。
...
做什么：
1. 创建 extensions/sentinels/data-health/ 目录 + manifest.json（layer:technology, computeKind:deterministic, expert:tech）
2. 创建 aggregate.ts + computes/（数据就绪度检测 + 数据孤岛检测）
3. 删除 src/sentinel/adapters/{data-readiness,data-silos}-sentinel.ts
4. 更新 extensions/sentinels/manifest.json（添加 tech 分组）
5. 更新 src/sentinel/types.ts 添加 type import（接线验证）
...
处理（中间经过哪些步骤）：sentinel-loader.ts 扫描到 data-health/ → 读 manifest → import aggregate.ts → check()
结果（最终展示在哪）：SentinelFinding[] 存入 Evidence 池 → L3 信号聚合

## 本任务在哪一层
L3（extensions/sentinels/data-health/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/data-health/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "data-readiness\|data-silos" src/ --include="*.ts" | head -5'
