# Task Brief: Task 1.4: customer-dynamics → E4 客户需求迁移

> 生成: 2026-06-26 03:01:25 | 分支: feat/prompt-architecture | as any: 0
...
按技术方案将旧 customer-dynamics 重写为 E4 客户需求迁移哨兵（environment层, heuristic）。

### b) 文件审计
旧文件（待删除）：
- src/sentinel/adapters/customer-dynamics-sentinel.ts
新目录（待创建）：
- extensions/sentinels/customer-demand-shift/
关系: 新建（旧适配器仅供参考，按技术方案重写）

### c) 决策
无覆盖冲突。技术方案要求重写为 E4 哨兵。
...
做什么：
1. 创建 extensions/sentinels/customer-demand-shift/ 目录 + manifest.json（layer:environment, computeKind:heuristic, expert:marketing, auxiliaryExperts:[strategy]）
2. 创建 aggregate.ts + computes/（客户收入集中度 + 客户流失检测）
3. 删除 src/sentinel/adapters/customer-dynamics-sentinel.ts
4. 更新 extensions/sentinels/manifest.json
5. 更新 src/sentinel/types.ts
...
入口：Cron 触发哨兵巡检
处理：sentinel-loader.ts 扫描到 customer-demand-shift/ → 读 manifest → import aggregate.ts → check()
结果：SentinelFinding[] 存入 Evidence 池 → L3 信号聚合

## 本任务在哪一层
L3（extensions/sentinels/customer-demand-shift/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/customer-demand-shift/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "customer-dynamics" src/ --include="*.ts" | head -5'
