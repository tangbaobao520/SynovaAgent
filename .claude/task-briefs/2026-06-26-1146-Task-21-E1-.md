# Task Brief: Task 2.1: E1 市场天花板与生命周期

> 生成: 2026-06-26 11:46:22 | 分支: feat/prompt-architecture | as any: 0
...
本任务属于Phase 2环境层。纯新哨兵(E1), 无旧适配器需要迁移。

### b) 文件审计
grep 'Market|FINANCIAL' extensions/sentinels/ -- 无已有模块。新建哨兵market-lifecycle。
...
做什么：
1. 创建extensions/sentinels/market-lifecycle/ (manifest+aggregate+compute)
2. compute/lifecycle-stage.ts (Klepper 1996判定)

不做什么：
- 不改sentinel-loader.ts / builtins.ts / registry.ts
- 不涉及旧适配器

## Q3: 验收 — 入口 → 交互 → 结果

入口：Cron巡检
处理：loader扫描market-lifecycle/ -> manifest -> import -> check()
结果：SentinelFinding[]存Evidence池

## 本任务在哪一层
L3 (extensions/sentinels/market-lifecycle/)

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/market-lifecycle/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn lifecycle-stage src/ --include="*.ts" | head -5'
