# Task Brief: Task 2.2: E2 结构性机会窗口

> 生成: 2026-06-26 17:00:43 | 分支: feat/prompt-architecture | as any: 0
...
本任务属于Phase 2环境层。纯新哨兵(E2), 检测结构性机会窗口。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
新建哨兵opportunity-window。无已有模块。
...
做什么：
1. 创建extensions/sentinels/opportunity-window/
2. compute/opportunity-window-score.ts (事件检测+技术/法规/竞争变化)

不做什么：
- 不改sentinel-loader/builtins/registry

## Q3: 验收 — 入口 → 交互 → 结果

入口：Cron巡检
处理：loader扫描opportunity-window/ -> import -> check()
结果：SentinelFinding[]

## 本任务在哪一层
L3 (extensions/sentinels/opportunity-window/)

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/opportunity-window/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn opportunity-window src/ --include="*.ts" | head -5'
