# Task Brief: Task 2.3: E3 竞争格局变化

> 生成: 2026-06-26 19:19:30 | 分支: feat/prompt-architecture | as any: 0
...
Phase 2环境层。E3竞争格局变化。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
新建competitive-dynamics。 expert/ sentinel/ extensions/ knowledge/ theory/ skills/ 中。列出已有文件驱动模块。关系: 复用 / 扩展 / 新建 / 冲突
...
### a) Anthropic 决策链
5步序: SPEC -> 测试 -> 实现 -> 接线 -> 自检。
引用: 铁律0-2, 铁律7, 铁律24+31。

### b) 本任务执行约束
- rule: "compute自包含静态import"
  verify: "! grep -rn import.*../ extensions/sentinels/competitive-dynamics/computes/ --include=*.ts | grep -v node_modules"
- rule: "无旧适配器需迁移"
  verify: "echo OK"
...
做什么：
1. extensions/sentinels/competitive-dynamics/
2. compute/hhi-index.ts + compute/competitive-intensity.ts
...
## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/competitive-dynamics/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn competitive-dynamics src/ --include="*.ts" | head -5'
