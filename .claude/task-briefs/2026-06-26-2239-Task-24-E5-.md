# Task Brief: Task 2.4: E5 环境红利依赖性

> 生成: 2026-06-26 22:39:26 | 分支: feat/prompt-architecture | as any: 0
...
Phase 2。E5环境红利依赖性。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
新建environment-rent-dependency。 expert/ sentinel/ extensions/ knowledge/ theory/ skills/ 中。列出已有文件驱动模块。关系: 复用 / 扩展 / 新建 / 冲突
...
2. compute/rent-dependency-index.ts
...
## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/environment-rent-dependency/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn rent-dependency src/ --include="*.ts" | head -5'
