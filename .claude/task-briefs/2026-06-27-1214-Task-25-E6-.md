# Task Brief: Task 2.5: E6 底层结构变化

> 生成: 2026-06-27 12:14:34 | 分支: feat/prompt-architecture | as any: 0
...
Phase 2。E6底层结构变化。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
新建structural-change。 expert/ sentinel/ extensions/ knowledge/ theory/ skills/ 中。列出已有文件驱动模块。关系: 复用 / 扩展 / 新建 / 冲突
...
做什么：
1. extensions/sentinels/structural-change/
2. compute/structural-change-signal.ts
...
## 本任务在哪一层
L3 (extensions/sentinels/structural-change/)

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/structural-change/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn structural-change src/ --include="*.ts" | head -5'
