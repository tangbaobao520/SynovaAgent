# Task Brief: Task 2.6: F1 融资约束指数

> 生成: 2026-06-27 13:36:14 | 分支: feat/prompt-architecture | as any: 0
...
Phase 2资本层。全新F1哨兵，无旧适配器迁移。基于KZ指数公式(Lamont 2001)。

### b) 文件审计
新建extensions/sentinels/financing-constraint/。无已有模块。关系: 新建。
...
### a) Anthropic 决策链
5步序: SPEC -> 测试 -> 实现 -> 接线 -> 自检6问。
引用: 铁律0-2, 铁律7, 铁律24+31。

### b) 本任务执行约束
- rule: "compute自包含静态import"  verify: "! grep -rn import.*\.\./ extensions/sentinels/financing-constraint/computes/ --include='*.ts' | grep -v node_modules"

## Q2: 范围 — 正确的最简方案是什么？

做什么：
1. extensions/sentinels/financing-constraint/
2. compute/kz-index.ts（Lamont-Polk-Saá-Requejo KZ指数）
...
## 本任务在哪一层
L3 (extensions/sentinels/financing-constraint/)

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/financing-constraint/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn financing-constraint src/ --include="*.ts" | head -5'
