# Task Brief: Task 2.8: F4 增长质量指数

> 生成: 2026-06-27 15:07:09 | 分支: feat/prompt-architecture | as any: 0
...
属于 Phase 2 资本层。全新 F4 增长质量指数。基于 FINANCIAL+Client 节点计算现金流转化率、有机增长比例。无旧适配器。

### b) 文件审计
新建 extensions/sentinels/growth-quality/。无已有模块。关系：新建。
...
### a) Anthropic 决策链
5 步序：SPEC/Done → 测试 → 实现（全部完成项+测试通过+接线完整+错误有log+tsc零失败）→ 接线 → 自检 6 问。引用：铁律 0-2、铁律 7、铁律 24+31。

### b) 本任务执行约束
- rule: "compute 通过同目录静态 import 引用，禁止跨目录"
  verify: "! grep -rn 'import.*\\.\\./' extensions/sentinels/growth-quality/computes/ --include='*.ts' | grep -v node_modules"

## Q2: 范围

做什么：
1. 创建 extensions/sentinels/growth-quality/ + manifest.json（layer:capital, computeKind:deterministic, expert:finance）
2. 创建 aggregate.ts + compute/cash-conversion-rate.ts + compute/organic-growth-pct.ts
3. 更新 extensions/sentinels/manifest.json
4. 更新 src/sentinel/types.ts
...
入口：Cron 触发
处理：loader 扫描 growth-quality/ → manifest → import → check()
结果：SentinelFinding[]

## 本任务在哪一层
L3（extensions/sentinels/growth-quality/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/growth-quality/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "growth-quality" src/ --include="*.ts" | head -5'
