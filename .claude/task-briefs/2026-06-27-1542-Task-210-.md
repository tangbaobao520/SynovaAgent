# Task Brief: Task 2.10: 清理已覆盖旧适配器

> 生成: 2026-06-27 15:42:42 | 分支: feat/prompt-architecture | as any: 0
...
Phase 2 收尾。financial-impact 被 F3+F5 覆盖，立即删除。其余标记 @deprecated。

### b) 文件审计
- financial-impact-sentinel.ts → 删除（F3+F5已上线）
- cpc/goal-alignment/integration-health → 标记@deprecated
- cash-flow → 已内联修复，保留
...
5 步序：SPEC→测试→实现→接线→自检。引用铁律0-2、铁律37。

## Q2: 范围

做什么：git rm financial-impact，grep 零残留
不做什么：不改其他文件

## Q3: 验收

入口：git rm → grep 确认
处理：builtins 自动注册不受影响
结果：pre-commit 通过

## 本任务在哪一层
L3（src/sentinel/adapters/）

## Done 标准
- [x] 入口可触达: verify: test ! -f src/sentinel/adapters/financial-impact-sentinel.ts
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "financial-impact" src/ --include="*.ts" | head -5'
