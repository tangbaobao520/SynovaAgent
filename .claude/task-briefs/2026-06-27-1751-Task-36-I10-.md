# Task Brief: Task 3.6: I10 单位经济可持续性

> 生成: 2026-06-27 17:51:56 | 分支: feat/prompt-architecture | as any: 0
...
Phase 3。I10 单位经济可持续性。LTV/CAC比、单位毛利率。基于 FINANCIAL+Client 节点。

### b) 文件审计
新建 extensions/sentinels/unit-economics/。关系：新建。
...
做什么：
1. extensions/sentinels/unit-economics/ + manifest（layer:interface, deterministic, finance, auxiliaryExperts:[marketing]）
2. compute/ltv-cac-ratio.ts + compute/gross-margin-per-unit.ts
...
## 本任务在哪一层
L3（extensions/sentinels/unit-economics/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/unit-economics/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "unit-economics" src/ --include="*.ts" | head -5'
