# Task Brief: Task 4.1: I5 网络权力指数

> 生成: 2026-06-27 21:09:17 | 分支: feat/prompt-architecture | as any: 0
...
Phase 4。I5 网络权力指数。基于 Person/Agent/Client/Supplier 节点计算介数中心性、结构洞。

### b) 文件审计
新建 extensions/sentinels/network-power/。关系：新建。
...
做什么：
1. extensions/sentinels/network-power/ + manifest（layer:interface, deterministic, org, auxiliaryExperts:[strategy]）
2. compute/betweenness-centrality.ts + aggregate.ts + tests
...
## 本任务在哪一层
L3（extensions/sentinels/network-power/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/network-power/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "network-power" src/ --include="*.ts" | head -5'
