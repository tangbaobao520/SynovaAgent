# Task Brief: Task 7.1: 飞轮聚合引擎

> 生成: 2026-06-27 22:26:19 | 分支: feat/prompt-architecture | as any: 0
...
Phase 7 第一步。飞轮聚合引擎 src/sentinel/flywheel-aggregator.ts。输入46哨兵SentinelFinding[]，输出三飞轮转速+瓶颈。

### b) 文件审计
新建 src/sentinel/flywheel-aggregator.ts。关系：新建L3基础设施。
...
做什么：
1. 创建 src/sentinel/flywheel-aggregator.ts
2. computeFlywheelSpeeds() 函数
3. 三飞轮映射（价值创造/兑现/再生）
4. 瓶颈检测 + 行动优先
...
入口：单元测试调用 computeFlywheelSpeeds
处理：飞轮映射+转速计算+瓶颈检测
结果：{ valueCreation, valueCapture, valueRegeneration, bottleneck }

## 本任务在哪一层
L3（src/sentinel/flywheel-aggregator.ts）

## Done 标准
- [x] 入口可触达: verify: test -f src/sentinel/flywheel-aggregator.ts
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -rn "computeFlywheelSpeeds" src/sentinel/flywheel-aggregator.ts | head -5'
