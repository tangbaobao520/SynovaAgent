# Task Brief: fix: S3 资源错配指数 — 替换stub为真实compute

> 生成: 2026-06-28 00:25:23 | 分支: feat/prompt-architecture | as any: 0
...
替换S3资源错配stub为真实compute。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
extensions/sentinels/resource-misallocation/computes/。替换stub。 expert/ sentinel/ extensions/ knowledge/ theory/ skills/ 中。列出已有文件驱动模块。关系: 复用 / 扩展 / 新建 / 冲突
...
做什么：
1. 替换compute为真实算法(Goal vs FINANCIAL/Person资源对比)
2. 写测试
...
入口：Cron
处理：check()读Goal+Financial+PERSON
结果：SentinelFinding[]
...
## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/resource-misallocation.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "v/100" extensions/sentinels/resource-misallocation/computes/ --include="*.ts" | head -5'
