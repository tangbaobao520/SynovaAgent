# Task Brief: fix: O2 惯例变异率 — 替换stub为真实compute

> 生成: 2026-06-28 00:51:02 | 分支: feat/prompt-architecture | as any: 0
...
O2 惯例变异率。基于Process节点变化频率评估组织惯例演化速度。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
替换extensions/sentinels/routine-mutation/computes/的stub。 expert/ sentinel/ extensions/ knowledge/ theory/ skills/ 中。列出已有文件驱动模块。关系: 复用 / 扩展 / 新建 / 冲突
...
做什么：
1. 替换compute: 读取Process(更新时间/变更记录)+Event节点
2. 重写aggregate
3. 写测试(>=4 expect, 4场景)
...
## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/routine-mutation.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -n "computeRoutineMutation" extensions/sentinels/routine-mutation/computes/compute-routine-mutation.ts | grep -v "Math.min"'
