# Task Brief: fix: O3 激励增长对齐度 — 替换stub为真实compute

> 生成: 2026-06-28 01:01:01 | 分支: feat/prompt-architecture | as any: 0
...
替换 O3 激励增长对齐度 stub。基于 Goal 节点（增长/创新目标）与 Event 节点（短期激励事件）对比，评估激励与增长目标的对齐程度。

### b) 文件审计
替换 extensions/sentinels/incentive-alignment/computes/ 的 stub 文件。
...
做什么：
1. 替换 compute：读取 Goal(增长/创新目标) + Event(短期激励事件)，计算激励对齐度
2. 重写 aggregate.ts
3. 写测试（>=4 expect，覆盖空数据/高对齐/低对齐/无目标四种场景）
...
入口：Cron → check()
处理：读 Goal+Event → computeIncentiveAlignment() → 比较阈值
结果：SentinelFinding[]

## 本任务在哪一层
L3（extensions/sentinels/incentive-alignment/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/incentive-alignment.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -n "computeIncentiveAlignment" extensions/sentinels/incentive-alignment/computes/compute-incentive-alignment.ts | head -3'
