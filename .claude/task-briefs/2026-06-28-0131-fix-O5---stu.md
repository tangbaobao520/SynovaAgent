# Task Brief: fix: O5 惯例扩散速度 — 替换stub为真实compute

> 生成: 2026-06-28 01:31:35 | 分支: feat/prompt-architecture | as any: 0
...
替换 O5 惯例扩散速度 stub。基于 Process/Team/Person 节点评估新惯例在不同团队间的扩散速度。

### b) 文件审计
替换 extensions/sentinels/routine-diffusion/computes/ 的 stub。
...
做什么：
1. 替换 compute：分析 Process 节点在多团队间的使用比例
2. 重写 aggregate.ts
3. 写测试（覆盖空数据/快速扩散/缓慢扩散）
...
入口：Cron → check()
处理：读 Process+Team → computeRoutineDiffusion() → 输出
结果：SentinelFinding[]

## 本任务在哪一层
L3（extensions/sentinels/routine-diffusion/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/routine-diffusion.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep "adoption\|adopted\|teams" extensions/sentinels/routine-diffusion/computes/compute-routine-diffusion.ts | head -3'
