...
> 生成: 2026-06-28 01:53:34 | 分支: feat/prompt-architecture | as any: 0
...
替换O6组织信道容量stub。基于Person/Team/Event节点评估信息流通容量。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
替换extensions/sentinels/channel-capacity/computes/的stub。 expert/ sentinel/ extensions/ knowledge/ theory/ skills/ 中。列出已有文件驱动模块。关系: 复用 / 扩展 / 新建 / 冲突
...
做什么：
1. 替换compute: 分析Person/Team/Event节点,计算信息流通容量
2. 重写aggregate
3. 写测试
...
## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/channel-capacity.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -E "channel|capacity|overload|communication" extensions/sentinels/channel-capacity/computes/compute-channel-capacity.ts | head -3'
