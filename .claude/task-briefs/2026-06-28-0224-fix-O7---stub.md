# Task Brief: fix: O7 信息失真率 — 替换stub为真实compute

> 生成: 2026-06-28 02:24:34 | 分支: feat/prompt-architecture | as any: 0
...
19 个 stub 修复之一。替换 O7 信息失真率的 3 行 stub 为真实算法。基于组织信息传递理论，比较 Goal 节点在不同层级间的一致性、Person 节点的管理层级深度、以及 Event 节点中沟通故障事件的比例。层级越多、目标一致性越低、沟通故障越多 = 信息失真率越高。触及 L3（extensions/sentinels/info-distortion/）。

### b) 文件审计
待替换：
- extensions/sentinels/info-distortion/computes/compute-info-distortion.ts（现在为 3 行 stub）
- extensions/sentinels/info-distortion/aggregate.ts（传 nodes.length，需重写）

### c) 决策
替换 stub。不涉及 manifest 和 exportKey。

## Q1: 调研 — 决策链 + 执行约束

5 步序：SPEC/Done → 先写测试 → 实现 → 接线 → 自检 6 问。引用铁律 0-2、铁律 7、铁律 24+31。compute 必须引入 Goal/Person/Event 节点数据，仅传节点数。

## Q2: 范围

做什么：
1. 替换 compute：输入 Goal 节点（目标一致性）+ Person 节点（管理层级深度）+ Event 节点（沟通故障），计算信息失真率
2. 重写 aggregate.ts（不再传 nodes.length）
3. 写测试（4 个用例）

不做什么：不改 manifest / sentinel-loader / builtins / registry

## Q3: 验收

入口：Cron 触发
处理：check() → 读 Goal+Person+Event → computeInfoDistortion() → 输出
结果：SentinelFinding[]（失真率高/中/低）

## 本任务在哪一层
L3（extensions/sentinels/info-distortion/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/info-distortion.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -E "computeInfoDistortion" extensions/sentinels/info-distortion/computes/compute-info-distortion.ts | head -2'
