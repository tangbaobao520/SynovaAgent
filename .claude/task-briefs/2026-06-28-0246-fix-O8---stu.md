# Task Brief: fix: O8 组织修复能力 — 替换stub为真实compute

> 生成: 2026-06-28 02:46:19 | 分支: feat/prompt-architecture | as any: 0
...
替换 O8 组织修复能力 stub。基于 Event 节点的问题检测与修复能力评估。核心指标：问题发现率、修复率、平均修复时间。组织修复能力是组织韧性的关键指标。触及 L3（extensions/sentinels/org-repairability/）。

### b) 文件审计
待替换：
- extensions/sentinels/org-repairability/computes/compute-org-repairability.ts（3 行 stub）
- extensions/sentinels/org-repairability/aggregate.ts（传 nodes.length）
...
5 步序：SPEC/Done → 测试 → 实现 → 接线 → 自检。引用铁律 0-2、铁律 7、铁律 24+31。compute 必须传入 Event 节点相关数据。

## Q2: 范围

做什么：
1. 替换 compute：分析 Event 节点的问题类型事件，计算问题检测率、修复率、修复时间
2. 重写 aggregate.ts（不传 nodes.length）
3. 写测试（4 个用例）
...
入口：Cron 触发
处理：check() → 读 Event 节点 → computeOrgRepairability() → 评估修复能力
结果：SentinelFinding[]（修复能力高/中/低）

## 本任务在哪一层
L3（extensions/sentinels/org-repairability/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/org-repairability.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -E "repaired|repair|resolved" extensions/sentinels/org-repairability/computes/compute-org-repairability.ts | head -3'
