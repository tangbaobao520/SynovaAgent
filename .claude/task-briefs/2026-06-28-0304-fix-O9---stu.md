# Task Brief: fix: O9 权力结构刚性 — 替换stub为真实compute

> 生成: 2026-06-28 03:05:00 | 分支: feat/prompt-architecture | as any: 0
...
替换 O9 权力结构刚性 stub。基于 Person 节点的管理关系评估权力集中度。管理比越高、决策越集中在少数人手中 = 权力结构越刚性。触及 L3（extensions/sentinels/power-rigidity/）。

### b) 文件审计
待替换：
- extensions/sentinels/power-rigidity/computes/compute-power-rigidity.ts（3 行 stub）
- extensions/sentinels/power-rigidity/aggregate.ts（传 nodes.length）
...
5 步序：SPEC/Done → 测试 → 实现 → 接线 → 自检。引用铁律 0-2、铁律 7、铁律 24+31。

## Q2: 范围

做什么：
1. 替换 compute：分析 Person 节点的 manager/reportsTo 关系，计算管理比和权力集中度
...
入口：Cron 触发
处理：check() → 读 Person 节点 → computePowerRigidity() → 评估
结果：SentinelFinding[]（刚性高/中/低）

## 本任务在哪一层
L3（extensions/sentinels/power-rigidity/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/power-rigidity.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -E "manager|rigidity|centralized" extensions/sentinels/power-rigidity/computes/compute-power-rigidity.ts | head -3'
