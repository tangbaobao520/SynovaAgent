# Task Brief: fix: T4 连接器覆盖率 — 替换stub为真实compute

> 生成: 2026-06-28 03:49:25 | 分支: feat/prompt-architecture | as any: 0
...
替换 T4 连接器覆盖率 stub。基于 Tool 节点的 connector 属性，计算有连接器的工具占全部工具的比例。触及 L3（extensions/sentinels/connector-coverage/）。

### b) 文件审计
待替换：
- extensions/sentinels/connector-coverage/computes/compute-connector-coverage.ts（3 行 stub）
- extensions/sentinels/connector-coverage/aggregate.ts
...
做什么：
1. 替换 compute：读取 Tool 节点，计算有 connector/toolType 的工具比例
2. 重写 aggregate.ts
3. 写测试
...
入口：Cron → check() → 读 Tool 节点 → compute → 输出覆盖率
结果：SentinelFinding[]

## 本任务在哪一层
L3（extensions/sentinels/connector-coverage/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/connector-coverage.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -E "connector|coverage|connection" extensions/sentinels/connector-coverage/computes/compute-connector-coverage.ts | head -3'
