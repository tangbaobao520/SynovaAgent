# Task Brief: fix: SentinelConfig 添加 layer/auxiliaryExperts/computeKind/technoEconomic字段

> 生成: 2026-06-28 09:29:35 | 分支: feat/prompt-architecture | as any: 0
...
SentinelConfig 接口（src/sentinel/types.ts）缺失技术方案第五章要求的 4 个字段：layer、auxiliaryExperts、computeKind、technoEconomicPhaseCalibration。这些字段已在 SentinelManifest（sentinel-loader.ts）中存在，但未在 SentinelConfig 中声明，sentinel-loader.ts 注册哨兵时也未传递。需要补全接口 + 加载器接线。

### b) 文件审计
改：src/sentinel/types.ts（SentinelConfig 接口 +4字段）
改：src/sentinel/sentinel-loader.ts（config 构建时传递新字段）

### c) 决策
补全接口，补传输线。不涉及 manifest 变更。

## Q2: 范围

做什么：
1. types.ts SentinelConfig +layer/auxiliaryExperts/computeKind/technoEconomicPhaseCalibration
2. sentinel-loader.ts 注册哨兵时从 manifest 传递这些字段到 config

不做什么：不改 manifest.json。不改旧适配器代码。

## Q3: 验收

入口：tsc 编译 → loader 注册 → runner 路由
处理：config 包含 layer → findSignalRoute 读取 layer
结果：专家路由基于 layer 正常工作

## 本任务在哪一层
L3（src/sentinel/types.ts + sentinel-loader.ts）

## Done 标准
- [x] 入口可触达: verify: grep -n "layer\?:" src/sentinel/types.ts | head -3
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -n "manifest.layer\|manifest.auxiliaryExperts\|manifest.computeKind" src/sentinel/sentinel-loader.ts | head -5'
