# Task Brief: 切片5: manifest.json dependsOn 字段

> 生成: 2026-06-29 18:56:23 | 分支: feat/prompt-architecture | as any: 0
...
哨兵系统(L3)。扩展已有 6 个哨兵 manifest JSON，加 dependsOn 字段声明数据依赖。

### b) 文件审计
extensions/sentinels/*/manifest.json 已有层/阈值/优先级字段。关系: 扩展。

### c) 决策
扩展 JSON 文件。不改代码。6 个哨兵: cost-health/revenue-health/cash-runway/profit-health/key-person-risk/collaboration-health。
