# Task Brief: 修补1: dependsOn 字段消费 — sentinel-loader.ts

> 生成: 2026-06-29 22:42:40 | 分支: feat/prompt-architecture | as any: 0
...
哨兵系统(L3)。已有 dependsOn 在 manifest 中但未被 sentinel-loader 消费。扩展已有加载逻辑。

### b) 文件审计
src/sentinel/sentinel-loader.ts 已有 registerLoadedSentinels()。扩展其检查逻辑。

### c) 决策
扩展已有模块。manifest 中的 dependsOn 字段在注册时校验。
