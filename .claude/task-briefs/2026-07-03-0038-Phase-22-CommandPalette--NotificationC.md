# Task Brief: Phase 2.2 CommandPalette + NotificationCenter

> 生成: 2026-07-03 00:39:00 | 分支: feat/prompt-architecture | as any: 0
...
### a) 项目拼图
- [x] 纵向（改 L1 交互层 UI）
Phase 2.2 新增命令面板(Ctrl+K 全局搜索)和通知中心(铃铛→列表)。
依赖 Phase 2.1 的 app-store(userRole/workspaces) 和 useKeyboard。

### b) 文件审计
无重复。

### c) 决策
新建 CommandPalette.tsx 和 NotificationCenter.tsx。复用 app-store 和 useKeyboard。

## Q1: 调研 — 决策链 + 执行约束

### a) Anthropic 决策链
  ① SPEC → ② 实现 → ③ 接线 (App.tsx + TitleBar) → ④ 验证

引用：铁律 0-2 (spec→test→impl→wire)、铁律 38 (as any 零容忍)

### b) 执行约束
- rule: "Ctrl+K 打开命令面板，Escape 关闭"
  verify: grep -rn "ctrlKey.*k\|openCommandPalette" electron-renderer/src/hooks/useKeyboard.ts
- rule: "通知按优先级排序 (critical > warning > info)"
  verify: grep -rn "priority" electron-renderer/src/components/NotificationCenter.tsx

## Q2: 范围

**做什么：**
1. CommandPalette.tsx — Ctrl+K 打开, 搜索工作区/对话/命令/哨兵, 实时过滤
2. NotificationCenter.tsx — 铃铛→通知列表, 按优先级排序, 未读计数
3. useKeyboard.ts — 增加 Ctrl+K (openCommandPalette), Ctrl+F (搜索)
4. TitleBar.tsx — 通知铃铛点击展开通知中心
5. App.tsx — 集成 CommandPalette 和 NotificationCenter
6. CSS — 命令面板遮罩+搜索, 通知列表卡片
...
入口：Ctrl+K 打开命令面板, 铃铛图标打开通知列表
处理：输入关键词实时过滤, 通知按优先级排列
结果：命令面板显示匹配项, 通知列表显示未读计数

## 本任务在哪一层
L1（electron-renderer/）

## Done 标准
- [ ] Ctrl+K 打开命令面板, 输入过滤工作区/对话/命令
- [ ] 通知铃铛显示未读计数, 点击展开通知列表
- [ ] 通知按 critical/warning/info 排序
