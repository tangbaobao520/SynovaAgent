...
### a) 项目拼图
- [x] 纵向（改 L1 scripts/ + L2 服务层: email-service + boss-mailbox + L5 server.ts 接线）

Phase 4.2: 老板信箱 + 邮件服务。EmailService + BossMailbox.sendEmail。
...
### c) 决策
新建 EmailService + 扩展 BossMailbox。沿用现有 generateReport/renderText。
...
**不做什么：**
- 不改 `electron-renderer/` 前端组件
- 不改 `src/notifications/` 已有适配器
- ❌ 老板信箱 (Phase 4.2)
- ❌ 邮件服务 (Phase 4.2)
- ❌ 免打扰模式 (v2)
...
## 本任务在哪一层
L1（scripts/ + electron-renderer/）+ L2（src/services/ + src/agent/）+ L5（src/server.ts）
## Done 标准
- [ ] 托盘图标三种状态（绿/橙/红）
- [ ] 右键菜单含"暂停通知2小时""今日摘要"
