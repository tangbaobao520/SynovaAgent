...
### a) 项目拼图
- [x] 纵向（改 L2 服务层: email-service + boss-mailbox）
...
### c) 决策
复用 BossMailbox 现有接口。新建 EmailService + boss-mailbox sendEmail。
...
## 本任务在哪一层
L1（electron-renderer/）+ L2（src/services/ + src/agent/ + src/server.ts）
## Done 标准
- [ ] 托盘图标三种状态（绿/橙/红）
- [ ] 右键菜单含"暂停通知2小时""今日摘要"
