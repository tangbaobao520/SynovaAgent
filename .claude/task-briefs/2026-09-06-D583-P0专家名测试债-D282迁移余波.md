# Task Brief: D583 P0专家名测试债-D282迁移余波

> 生成: 2026-09-06 | 任务: D583 | 认领: DSH 编码线（CTO 直派测试债批）
...
## Q0: 定位 — 项目拼图 + 文件审计
### a) 项目拼图
P0 测试债批（Win 盘点台账 898785d6 承接）。绿测试 = 专家架构 P1 改名的安全网。
### b) 文件审计
...
## Done 标准
- [x] 清单文件全绿或显式豁免 — verify: npx vitest run tests/expert-quality/layer2-judge.test.ts
- [x] 全量失败集净减 ≥18 — verify: npx vitest run 2>&1 | grep -c FAIL
