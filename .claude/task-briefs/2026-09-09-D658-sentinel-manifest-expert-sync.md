# Task Brief: D658 哨兵 manifest expert 字段同步——专家体系末段残留收口（45 manifest expert/auxiliaryExperts 旧 8 名 → 新 5+host）

> 生成: 2026-09-09 | 分支: feat/win-d658-sentinel-manifest-expert-sync (base=origin/main 含已合并的 #463/D658 dev-doc) | as any: 0
...
- [x] DS8 无绕过 + 推送 CI: verify: grep -c "no-verify" .claude/bypass.log → 今日 0 新增；git push 后 CI task-relevant jobs 绿（job 级）✅ 真实门禁提交，无 --no-verify；push 后 CI 见 PR
