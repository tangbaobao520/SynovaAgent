...
## Q2: 范围 — 正确的最简方案
做什么：
- `docs/plans/codex/implementation/SYNOVA-IMPL-DSH-D1-monitoring-contract.md` — 监测契约 schema（每客户/每指标：阈值、渠道、**节奏**、**格式**、升级规则、责任人）+ 文件位置 + 示例 + 默认值（24h 未回应提醒 / 7 天无进展进周报首页 / 默认周报一页纸 / 明确不处理项每周回顾）+ 与状态机（六状态）的映射
- `docs/plans/codex/implementation/SYNOVA-IMPL-DSH-1-5-dual-guide-convergence.md` — 1-5 收敛 spec：收敛到哪个入口、另一入口如何退场（删除 or 重定向）、既有书签/URL 兼容、如何物理证明「只有一个入口」
