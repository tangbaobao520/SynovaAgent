...
做什么：
- src/llm/resilient-chat-adapter.ts — 新建装配缝：`createResilientChatAdapter(provider, opts)`
  返回 `{ chat }`（协作式：成功透传 content/toolCalls + usage；失败转**分类错误**
  `LlmResilienceError`（code/phase/retryable，铁律 32），错误码取自 outcome.code）
- src/llm/retry-middleware.ts — `pickChatOptions` 白名单补齐 tools/cacheConfig（原实现静默丢弃 tools）
- src/agent/tool-loop-executor.ts — 4 处 `provider.chat` → `callWithResilience`；
  `ok:false` → log.warn(code/degraded) + 返回降级文案（**不抛**）；onRetry → 重试事件落盘
- src/agent/engine-context.ts — 补 `sessionStore?` 结构缝（L2 不 import L5 具体类）
- src/agent/conversation-engine.ts — sessionStore 装配去 as 投射（D487 缝复用给重试事件）
- src/deploy/bootstrap.ts — 副作用导入 `store/retry-projection`（投影单元注册，同 session-projection 范式）
- src/routes/conversations.ts — 适配器字面量 → `createResilientChatAdapter`
- src/routes/diagnosis.ts — 同上（保留 D598 TokenMeter 计量语义）+ SSE 透传 `code`
- src/orchestrator/context-engine.ts — SummaryProvider → outcome 消费，失败降级 `{content:''}` + log.warn
- src/services/background-review.ts — 两处 → outcome 消费，失败 log.warn + 返回 null（既有降级语义）
- src/expert-platform/extractor.ts — 三处 → outcome 消费，失败走既有降级分支（null/默认值）
- src/l2-interfaces/diagnosis-engine.ts — DiagnosisEvent 补 `code?`（分类码透传到 L1，铁律 31）
- src/l3/synova-diagnosis-engine-impl.ts — error 事件 code 取韧性层分类码（非固定 LLM_ERROR）
- src/store/retry-projection.ts — `appendRetryEvent` 参数由 `SessionStore` 具体类放宽为结构类型
  （`RetryEventSink`），使 L2 装配点可调用而不引入 L2→L5 具体类耦合
- tests/llm/resilient-chat-adapter.test.ts — 新建：正常/重试成功的 attempt 计数/超时 TOOL_TIMEOUT/不可重试
- tests/agent/tool-loop-executor-resilience.integration.test.ts — 新建：驱动**真实生产类**
  （ToolLoopExecutor），断言重试生效（attempt≥2）、超时不抛+降级文案、retryEventSink 收到事件
- tests/store/retry-projection.test.ts — 落盘缝迁移后 import 路径同步（行为零变）
- scripts/golden-scenarios/evidence/GS-01-2026-09-18.json — GS-01 实跑证据（含既有 noauth-401 红，如实登记）
- docs/synova/product-lines/evidence/test-2026-09-18.json — 20-3/20-5/22-1 绑定证据（M2 物理化动作 2）
- memory/notes/implemented/2026-09-18-d810-llm-resilience-wiring.md — 决策 Note（铁律 49: 落盘缝上移裁决）
