# Task Brief: Task 0.5.0: 扩展 SentinelManifest 接口

> 生成: 2026-06-25 21:24:41 | 分支: feat/prompt-architecture | as any: 0
...
本任务属于哨兵系统基础设施。触及 L3（sentinel/sentinel-loader.ts）。
该层现有模块：sentinel-loader.ts（扫描extensions/sentinels/并加载哨兵）、registry.ts（注册表）。
本任务是扩展已有接口，非新增模块。

### b) 文件审计
grep 关键词 "SentinelManifest" 在 src/sentinel/sentinel-loader.ts — 找到内联接口定义。
需扩展的字段：layer、auxiliaryExperts、computeKind、technoEconomicPhaseCalibration。
关系: 扩展（在已有接口上加可选字段）。
无冲突。

### c) 决策
无覆盖问题。直接在 src/sentinel/sentinel-loader.ts 的 SentinelManifest 接口加可选字段。
走横向解耦（改 TypeScript 类型定义）。
...
根据决策链和本任务特点，提炼 2-3 条必须遵守的规则。每条 rule 必须包含 verify 命令。
  - rule: "新增字段必须是可选(?)类型 — 不破坏已有哨兵的 manifest"
    verify: "grep -n 'layer?:|auxiliaryExperts?:|computeKind?:' src/sentinel/sentinel-loader.ts"
  - rule: "sentinel-loader 解析新字段时不做验证 — 存在就传，不存在 undefined"
    verify: "grep -n 'layer\|auxiliaryExperts\|computeKind' src/sentinel/sentinel-loader.ts | grep -v 'interface SentinelManifest'"

## Q2: 范围 — 正确的最简方案是什么？

做什么：
1. 在 SentinelManifest 接口加4个可选字段：layer, auxiliaryExperts, computeKind, technoEconomicPhaseCalibration
2. 确保 sentinel-loader 解析时不验证新字段（存在就传，不存在 undefined）

不做什么：
- 不改已有 manifest.json（Task 0.3已做完，不改任何哨兵目录）
- 不改 aggregate.ts 或任何哨兵逻辑
- 不改 registry.ts 或其他文件

## Q3: 验收 — 入口 → 交互 → 结果

入口（用户从哪触发）：tsc --noEmit 编译通过
处理（中间经过哪些步骤）：grep 确认新字段存在于 SentinelManifest 接口
结果（最终展示在哪）：其他开发者在 manifest.json 中可使用新字段，tsc 无错误

## 本任务在哪一层
L3（src/sentinel/sentinel-loader.ts）

## Done 标准
- [x] 入口可触达: tsc 编译零错误
- [x] 链路走通: grep 验证新字段存在于 SentineManifest 接口
- [x] 结果可见: 新字段声明为可选类型，不破坏已有 manifest
