# Task Brief: Task 0.5.2: 删除 risk-aggregator-sentinel.ts

> 生成: 2026-06-25 21:58:02 | 分支: feat/prompt-architecture | as any: 0
...
本任务属于哨兵系统清理。删除旧的 risk-aggregator-sentinel.ts 适配器。该哨兵的功能已被 signal-aggregator.ts 管线覆盖。

### b) 文件审计
grep 'riskaggregator|risk-aggregator' src/ -- 确认引用情况。
src/sentinel/adapters/risk-aggregator-sentinel.ts -- 待删除
关系: 删除（能力已被 signal-aggregator.ts 管线覆盖）。
