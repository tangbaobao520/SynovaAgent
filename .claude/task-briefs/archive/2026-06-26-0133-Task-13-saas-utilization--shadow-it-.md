# Task Brief: Task 1.3: saas-utilization + shadow-it → T1 软件资产健康度

> 生成: 2026-06-26 01:34:00 | 分支: feat/prompt-architecture | as any: 0
...
合并 2 个旧适配器的能力，按技术方案重写为 T1 软件资产健康度哨兵。

### b) 文件审计
旧文件（待删除）：
- src/sentinel/adapters/saas-utilization-sentinel.ts
- src/sentinel/adapters/shadow-it-sentinel.ts
新目录（待创建）：
- extensions/sentinels/software-health/
关系: 新建（旧适配器仅供参考，按技术方案重写）

### c) 决策
无覆盖冲突。技术方案要求合并为 T1 哨兵，按新签名重写。
...
做什么：
1. 创建 extensions/sentinels/software-health/ 目录 + manifest.json（layer:technology, computeKind:deterministic, expert:tech）
2. 创建 aggregate.ts + computes/（SaaS利用率评分 + Shadow IT 检测）
3. 删除 src/sentinel/adapters/{saas-utilization,shadow-it}-sentinel.ts
4. 更新 extensions/sentinels/manifest.json
5. 更新 src/sentinel/types.ts 添加 type import
...
入口：Cron 定时触发哨兵巡检
处理：sentinel-loader.ts 扫描到 software-health/ → 读 manifest → import aggregate.ts → check()
结果：SentinelFinding[] 存入 Evidence 池 → L3 信号聚合

## 本任务在哪一层
L3（extensions/sentinels/software-health/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/software-health/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "saas-utilization\|shadow-it" src/ --include="*.ts" | head -5'
