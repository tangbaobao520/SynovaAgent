# Task Brief: Task 3.1: I1 生态位宽度与深度

> 生成: 2026-06-27 16:01:35 | 分支: feat/prompt-architecture | as any: 0
...
Phase 3 界面层。全新 I1 生态位宽度与深度哨兵。基于 Client/Location/Market/Product 节点计算 Levins 生态位指标。无旧适配器。

### b) 文件审计
新建 extensions/sentinels/niche-breadth/。关系：新建。
...
5 步序：SPEC/Done → 测试 → 实现 → 接线 → 自检 6 问。

### b) 本任务执行约束
- rule: "compute 同目录静态 import"
  verify: "! grep -rn 'import.*\\.\\./' extensions/sentinels/niche-breadth/computes/ --include='*.ts' | grep -v node_modules"

## Q2: 范围

做什么：
1. 创建 extensions/sentinels/niche-breadth/ + manifest（layer:interface, computeKind:heuristic, expert:strategy, auxiliaryExperts:[marketing]）
2. compute/levins-breadth.ts（B/D/V 三指标）
3. aggregate.ts
4. types.ts + manifest.json
...
入口：Cron → loader 扫描 niche-breadth/
处理：manifest → import → check()
结果：SentinelFinding[]

## 本任务在哪一层
L3（extensions/sentinels/niche-breadth/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/niche-breadth/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "niche-breadth" src/ --include="*.ts" | head -5'
