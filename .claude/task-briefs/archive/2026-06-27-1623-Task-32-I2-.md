# Task Brief: Task 3.2: I2 生态位挤压指数

> 生成: 2026-06-27 16:23:29 | 分支: feat/prompt-architecture | as any: 0
...
Phase 3 界面层。全新 I2 生态位挤压指数哨兵。基于 Market/Client/Supplier 节点的 HHI/CR5 计算竞争挤压程度。

### b) 文件审计
新建 extensions/sentinels/niche-squeeze/。关系：新建。
...
- rule: "compute 同目录静态 import"
  verify: "! grep -rn 'import.*\\.\\./' extensions/sentinels/niche-squeeze/computes/ --include='*.ts' | grep -v node_modules"

## Q2: 范围

做什么：
1. extensions/sentinels/niche-squeeze/ + manifest（layer:interface, deterministic, strategy）
2. compute/niche-squeeze-index.ts（HHI+竞争者密度+价格压力）
3. aggregate.ts + tests
...
## 本任务在哪一层
L3（extensions/sentinels/niche-squeeze/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/niche-squeeze/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "niche-squeeze" src/ --include="*.ts" | head -5'
