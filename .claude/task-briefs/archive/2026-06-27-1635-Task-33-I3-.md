# Task Brief: Task 3.3: I3 护城河结构性强度

> 生成: 2026-06-27 16:35:30 | 分支: feat/prompt-architecture | as any: 0
...
Phase 3 界面层核心。I3 护城河结构性强度。6力聚合：规模经济+网络效应+切换成本+流程威力+反向定位SLM+独占资源。含SLM公式（§6.3）。

### b) 文件审计
新建 extensions/sentinels/competitive-moat-structural/。关系：新建。
...
5 步序：SPEC/Done → 测试 → 实现 → 接线 → 自检。

### b) 执行约束
- rule: "compute 同目录静态 import"
  verify: "! grep -rn 'import.*\\.\\./' extensions/sentinels/competitive-moat-structural/computes/ --include='*.ts' | grep -v node_modules"

## Q2: 范围

做什么：
1. extensions/sentinels/competitive-moat-structural/ + manifest（layer:interface, deterministic/heuristic, strategy, auxiliaryExperts:[tech]）
2. 6 computes + aggregate.ts
3. types.ts + tests + manifest.json
...
入口：Cron → loader → check()
结果：SentinelFinding[]

## 本任务在哪一层
L3（extensions/sentinels/competitive-moat-structural/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/competitive-moat-structural/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "competitive-moat-structural" src/ --include="*.ts" | head -5'
