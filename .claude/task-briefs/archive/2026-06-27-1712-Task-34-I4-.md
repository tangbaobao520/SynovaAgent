# Task Brief: Task 3.4: I4 护城河感知强度

> 生成: 2026-06-27 17:12:04 | 分支: feat/prompt-architecture | as any: 0
...
Phase 3 界面层。I4 护城河感知强度。基于品牌溢价/客户忠诚度/认知份额评估感知壁垒。

### b) 文件审计
新建 extensions/sentinels/competitive-moat-perceptual/。关系：新建。
...
### b) 执行约束
- rule: "compute 同目录静态 import"  verify: "..."

## Q2: 范围

做什么：
1. extensions/sentinels/competitive-moat-perceptual/ + manifest（layer:interface, heuristic, strategy, auxiliaryExperts:[marketing]）
2. compute/brand-premium.ts + compute/customer-loyalty.ts
3. aggregate.ts + tests + types.ts
...
## 本任务在哪一层
L3（extensions/sentinels/competitive-moat-perceptual/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/competitive-moat-perceptual/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "competitive-moat-perceptual" src/ --include="*.ts" | head -5'
