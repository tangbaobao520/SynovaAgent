# Task Brief: Task 3.5: I7 商业模式一致性

> 生成: 2026-06-27 17:32:54 | 分支: feat/prompt-architecture | as any: 0
...
Phase 3。I7 商业模式一致性。基于 BusinessModel/Goal/Channel/Capability 节点评估价值主张-收入-成本结构一致性。

### b) 文件审计
新建 extensions/sentinels/business-model-coherence/。关系：新建。
...
做什么：
1. extensions/sentinels/business-model-coherence/ + manifest（layer:interface, heuristic, business_model, auxiliaryExperts:[strategy]）
2. compute/model-consistency-score.ts + aggregate.ts + tests + types
...
## 本任务在哪一层
L3（extensions/sentinels/business-model-coherence/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/business-model-coherence/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "business-model-coherence" src/ --include="*.ts" | head -5'
