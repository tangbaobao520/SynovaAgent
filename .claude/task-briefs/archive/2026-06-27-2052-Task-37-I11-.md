# Task Brief: Task 3.7: I11 内部交易成本

> 生成: 2026-06-27 20:52:33 | 分支: feat/prompt-architecture | as any: 0
...
Phase 3 最后一步。I11 内部交易成本增长率。基于 FINANCIAL/Person/Team/Event 节点计算管理成本率、协调成本趋势。

### b) 文件审计
新建 extensions/sentinels/internal-transaction-cost/。关系：新建。
...
做什么：
1. extensions/sentinels/internal-transaction-cost/ + manifest（layer:interface, deterministic, org, auxiliaryExperts:[finance]）
2. compute/transaction-cost-trend.ts + aggregate.ts + tests
...
## 本任务在哪一层
L3（extensions/sentinels/internal-transaction-cost/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/internal-transaction-cost/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "internal-transaction-cost" src/ --include="*.ts" | head -5'
