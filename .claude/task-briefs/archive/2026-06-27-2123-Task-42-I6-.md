# Task Brief: Task 4.2: I6 价值捕获效率

> 生成: 2026-06-27 21:23:13 | 分支: feat/prompt-architecture | as any: 0
...
Phase 4。I6 价值捕获效率。基于 FINANCIAL 节点计算利润留存率、定价权、毛利率趋势。

### b) 文件审计
新建 extensions/sentinels/value-capture/。关系：新建。
...
做什么：
1. extensions/sentinels/value-capture/ + manifest（layer:interface, heuristic, finance, auxiliaryExperts:[strategy]）
2. compute/value-capture-score.ts + aggregate.ts + tests
...
## 本任务在哪一层
L3（extensions/sentinels/value-capture/）

## Done 标准
- [x] 入口可触达: verify: test -d extensions/sentinels/value-capture/
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "value-capture" src/ --include="*.ts" | head -5'
