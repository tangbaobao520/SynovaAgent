# Task Brief: fix: S2 战略调适速度 — 替换stub为真实compute

> 生成: 2026-06-28 00:10:39 | 分支: feat/prompt-architecture | as any: 0
...
修复19个stub之一。S2战略调适速度。（GA诊断/哨兵/基础设施）？触及哪层？该层现有模块？新增/替换/扩展？

### b) 文件审计
扩展sentinels/adaptation-velocity/computes/。替换stub。 expert/ sentinel/ extensions/ knowledge/ theory/ skills/ 中。列出已有文件驱动模块。关系: 复用 / 扩展 / 新建 / 冲突
...
做什么：
1. 替换compute/adaptation-velocity.ts为真实算法(基于Event节点检测战略调适速度)
2. 重写aggregate.ts
3. 写测试
...
## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/adaptation-velocity.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c '! grep -rn "computeAdaptationVelocity" extensions/sentinels/adaptation-velocity/computes/compute-adaptation-velocity.ts | head -5'
