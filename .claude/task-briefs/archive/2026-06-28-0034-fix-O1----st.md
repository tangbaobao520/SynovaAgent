# Task Brief: fix: O1 探索-利用平衡 — 替换stub为真实compute

> 生成: 2026-06-28 00:34:43 | 分支: feat/prompt-architecture | as any: 0
...
修复 19 个 stub 之一。O1 探索-利用平衡（March 1991）。基于 Goal 节点 goalType（innovation vs optimization）和 Event 节点评估资源在探索与利用之间的分配。

### b) 文件审计
替换 extensions/sentinels/explore-exploit-balance/computes/ 中的 stub。
...
做什么：
1. 替换 compute 为真实算法（读取 Goal 节点的 goalType, 计算探索/利用比例）
2. 重写 aggregate.ts（传入真实数据, 输出有意义的 Finding）
3. 写测试（>=3 expect, 覆盖空数据/纯探索/纯利用/平衡四种场景）
...
入口：Cron 触发 check()
处理：读取 Goal 节点 → computeExploreExploitBalance() → 比较阈值
结果：SentinelFinding[]（探索比例偏离 30%-70% 时告警）

## 本任务在哪一层
L3（extensions/sentinels/explore-exploit-balance/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/explore-exploit-balance.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sed -n '4,6p' extensions/sentinels/explore-exploit-balance/computes/compute-explore-exploit-balance.ts | grep -v "Math.min.*100" | head -5
