# Task Brief: fix: O10 人才密度 — 替换stub为真实compute

> 生成: 2026-06-28 03:30:56 | 分支: feat/prompt-architecture | as any: 0
...
替换 O10 人才密度 stub。基于 Person 节点（skills、proficiencyLevel）和 Capability 节点评估组织的人才密集度。触及 L3（extensions/sentinels/talent-density/）。

### b) 文件审计
待替换：
- extensions/sentinels/talent-density/computes/compute-talent-density.ts（3 行 stub）
- extensions/sentinels/talent-density/aggregate.ts（传 nodes.length）
...
做什么：
1. 替换 compute：分析 Person(skills/proficiencyLevel) + Capability 节点
...
入口：Cron → check()
处理：读 Person+Capability → computeTalentDensity() → 评估
结果：SentinelFinding[]

## 本任务在哪一层
L3（extensions/sentinels/talent-density/）

## Done 标准
- [x] 入口可触达: verify: vitest run tests/sentinels/talent-density.test.ts 2>&1 | tail -5
- [x] 链路走通: verify: npx tsc --noEmit 2>&1 | head -5
- [x] 结果可见: verify: sh -c 'grep -E "proficiency|skill|density" extensions/sentinels/talent-density/computes/compute-talent-density.ts | head -3'
