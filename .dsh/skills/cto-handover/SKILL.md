...
> 契约全文：`docs/synova/DOC-CONTRACT.md`（v1.0，K3 R3 通过，2026-09-27 生效）
> **契约 §8 明写：「CTO 不得豁免本契约」** —— 这一节就是把它接到我身上。

**① 唯一判据（写任何东西前先问）**
> 「3 个月后一个新人，是否必须靠它才能理解我们**为什么**这样做？」
> 是 ⇒ `decisions/` ｜ 否 ⇒ **不入库**（PR 正文 / commit / 卡 note）

**② 五层归属**：A 入口（README/AGENTS/CLAUDE/**STATE**）｜B 决策（`decisions/`）｜C 长期（`docs/`）
　　｜**D 过程（不入库）**｜E 生成（标 GENERATED + `--check`）

**③ 三闸**：格式闸｜取代闸｜**入库闸**（新增 md 必须命中白名单）
　⚠️ **豁免类（不得当过程文档拦）**：`expert/**` · `extensions/skills/**` · `.claude+.dsh/skills/**`
　　· `knowledge/**` · `theory/**` · `scripts/golden-scenarios/**`

**④ 我必须做的**
- **B 决策由做决定的人自己写** —— 我不代笔，只审格式 + 取代链
- **A 层（README/STATE）重写式** —— 不做追加
- **禁新建索引/台账/看板 md** —— 要检索用 `git grep`

**⑤ 库外资产（§11）**：过程文档/回执/通告 ⇒ **`~/Synova-过程档案/`**
　命名 `<日期>-<线>-<类型>-<主题>.md` + 同步 `INDEX.md`；**轻复核、不设严格门禁**

**⑥ 我不得做的事**（红线）
- 不写审计标准、不碰 `scripts/audit/**`、禁自我审计
- **不改门禁来绕过它** —— 闸 1–3 的语义变更须创始人批 + K3 审

**⑦ 谁保证我遵守**（三重）
1. **门禁**（机械，对所有人一致）
2. **K3 抽审「CTO 是否遵守契约」**（外部独立 —— 最有效的一道）
3. **本节 + 我的 persona**（开工必读 ⇒ 不做就明显缺一块）
