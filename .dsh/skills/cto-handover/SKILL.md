...
## ⚠️ CTO 容易犯的错（开工必读 —— 都是实际发生过的，按次数排序）

> 这一节的存在理由：**同类错误第 2 次出现 = 防线系统性失效**。
> 把错写进"开工必读"里，比"提醒自己注意"更可靠。

**E1 · bash 里嵌 Python 的引号错误（已 4 次）**
- 症状：`python3 -c "...{'k':v}..."` ⇒ shell 吃掉花括号/ASCII 引号 ⇒ `SyntaxError`，白跑一轮
- **正确做法**：**一律用 heredoc** —— `python3 - "$1" <<'PY' … PY`，参数走 `sys.argv`
- **禁用**：内容含 `{}` 或引号时用 `python3 -c "…"`

**E2 · 以为提交成功、其实被门禁拦住（已 2 次）**
- 症状：`git commit` 被 gate 拒绝（exit 0 但无新 sha），脚本继续推了一个**空分支/旧 sha**
- **正确做法**：**提交后必核 `git log -1 --format=%h`**，与提交前对比；**变了才算成功**
- 推送前同理：**12 位 local == remote** 才算推成

**E3 · 下指令前不核仓库自身约定（已 2 次）**
- 实例："叠加探针已推"（未核 ci.yml 内容）；"用 `--ours` 解冲突"（未核 `.gitattributes` 的 union 声明）
- **正确做法**：凡涉文件处置，**先查** `.gitattributes` / `.gitignore` / `ownership.yaml` / **该文件既有注释**

**E4 · 把"工程指标"当成目标，丢掉产品目的（已 2 次被创始人纠正）**
- 实例："把 PR 降到 ≤20 条"；"数量不重要"被纠正两次
- **正确做法**：**每条判据/DoD 必须能填「服务于哪个产品目标」**（服务第一个客户 / 多客户准备 / 能演示）

**E5 · 手动输入的值不复查（已 1 次）**
- 实例：手敲 head 全 sha 得 404
- **正确做法**：**所有 sha / 数字一律从命令原始输出取**，不手写

**E6 · 「需要沟通」→ 反射式写文档（已 2 次）**
- 症状：遇到要"跟谁讲清楚"的事，第一反应是**新开一份 md**。
  实例：先写《口径统一通告 v1》文件，又写《K3 审计任务书》文件 ——
  **刚立完「过程不入库」的契约，自己就违反它**。
- **正确做法**：**先在既有载体里找位置，找不到才考虑新文件**：
  ```
  ① PR 描述（过程的第一载体 —— 契约 §2.4 明写「PR 三段式是唯一过程载体」）
  ② 卡 note（task-state/<D>.json）
  ③ commit message
  ④ 已有的入口文档（AGENTS / CLAUDE / STATE）
  ⇒ 只有当以上都装不下，才新开文件
  ```
- **判据**：**新文件的必要性由「没有它信息会丢失或被误读」决定，不由「我想说清楚」决定。**
- **实例对照**：K3 审计要求 ⇒ 应进 **PR #853 描述**（它就是那个 PR 的审计要求）；
  创始人转发时只需一句「K3，审 PR #853，按它描述里的审计要求」。

**E7 · 修复型提交不做「diff 对照清单」自检（K3 R2 实测 1 次）**
- 症状：commit message 声称"§7 数字移出完成""六段三处同步"，
  实测**仍有 8 处残留 + 1 处漏改**（K3 R2 抓出，归因 implement，D315 同款）
- **正确做法**：**照清单改的修复型提交，commit 前必须逐条跑对照**：
  ```bash
  # 清单项 → grep 模式；全 0 才提交
  grep -c '五段'   docs/synova/DOC-CONTRACT.md   # 必须 0
  grep -c '| 711'  docs/synova/DOC-CONTRACT.md   # 必须 0
  ```
- **判据**：**"声称完成"必须有「计数为 0」的原始输出支撑** —— 不是"我改了"，是"残留 = 0"。
- 来源：K3 文档契约 R2 审计（N1）；L4 建议原文"收件闸加一行规则即可，零脚本"。

**⇒ 通用规律**：**能"不做就明显缺一块"的，别靠"记得做"。**
