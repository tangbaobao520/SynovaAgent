<!--
  ...
  并行: 写集=src/tools/org-expert-tools.ts + tests/tools/expert-tools-d234.test.ts，与 D481（tests/middleware/）**文件级零交集**，可 worktree 隔离并行；与 DSH 线（scripts/、src/sentinel/）零重叠；若必须并行先 worktree 隔离
-->
...
* `src/tools/org-expert-tools.ts` L46-49：`message: ${dataSource} 连接器已就绪。用户授权后可自动拉取组织数据。`——dataSource 参数描述（L31）为 `feishu/dingtalk/wecom/manual`，dingtalk/wecom 传入时声称「已就绪」，但物理上仅 feishu 真实接入（src/connectors/index.ts L9-13）。
...
* `tests/tools/expert-tools-d234.test.ts`：零覆盖 buildOrgGraphTool 连接器文案（grep 无命中）——需新增断言。
...
| tests/tools/expert-tools-d234.test.ts | 修改 | 新增 buildOrgGraphTool 3 断言：①manual 分支 nextStep 不含「钉钉/企微连接器自动拉取」旧声称、含「飞书」；②feishu → message 含「已就绪」；③dingtalk → message 含「待接入」（red=现状返回「已就绪」 → green=「待接入」） |

> 共享资源标注（S-8）：本写集不含 VERSION.md（声称降级，非门禁/工具行为变化，不 bump）；current-brief / 暂存区共享，串行触碰；src/tools/ 与 D481 的 tests/middleware/ 零交集。
...
| 层 | 类型 | 数量 | 覆盖 |
|----|------|------|------|
| L1 | 单元 tests/tools/expert-tools-d234.test.ts（修改既有文件新增） | +3 | ①manual 文案不含钉钉/企微自动拉取旧声称、含飞书（red=现状含旧声称 → green=降级）；②feishu → 「已就绪」；③dingtalk → 「待接入」（red=现状「已就绪」 → green=「待接入」） |
| L2 | 回归 既有 tools 测试 | 全量 | expert-tools-d234 + tool-registry 不回归 |
...
* 决策点 2：dingtalk/wecom 降级提示 vs 参数校验拒绝？
  * 参考系：Anthropic——fail-open 优于 fail-closed 的交互面：现有调用方（builtin-tools 注册）可能已传 dingtalk，拒绝会破坏；降级文案保留 pending 状态，调用方可见「待接入」。
  * 结论：降级文案，不拒绝参数。
...
* **DS3 测试全绿**：`vitest run tests/tools/expert-tools-d234.test.ts` 全 pass（含新增 3 断言；red 先行已证）。
...
| DS3 测试全绿 | vitest run tests/tools/expert-tools-d234.test.ts | 全 pass |
...
> 交付声明 DS 须与本文档 DS1-DS7 一一对应（S-10）；派发说明：与 D481 **可并行**（写集零交集：src/tools/+tests/tools/ vs tests/middleware/），必须 worktree 隔离；**只降级声称，不实现连接器、不扩写集**；dataSource 参数契约不变（降级提示而非拒绝）；暂存前查 session-registry（S-9）；merge main 时 reference-map 冲突由本任务所有者解决、bypass.log 噪声行不提交。
