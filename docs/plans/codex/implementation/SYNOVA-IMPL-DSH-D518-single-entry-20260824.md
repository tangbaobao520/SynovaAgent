...
- **审计基线**: `docs/synova/audit-reports/2026-08-23-D504-D505.md`（backend-spawn.cjs 契约 8 用例 25/25、main.cjs 集成属实；F4 注释漂移"dist/index.js"→实为"dist/src/index.js"待同步）
- **铁律**: AGENTS.md 铁律 0-2（WIRE CHECK 硬门禁）/ 4（入口→交互→结果）/ 24+31（降级显式传播）

## 2. Problem Statement

D504 已把双分支引导写进 main.cjs（dev: vite 5173 热更新→回退 SERVER_URL 登录页；prod: ensureBackend spawn `node dist/src/index.js` + `loadFile(resources/renderer/index.html)`），但：① 模式判定靠 `app.isPackaged` 隐式区分，无显式开发模式标记与文档；② SERVER_URL 固定 config.json localhost:18790，spawn vs 外连语义无说明；③ ensureBackend 被 main.cjs 调用的 WIRE 只有代码无运行证据；④ F4 注释漂移未修。本任务=收敛声明+显式化+物理验证。
...
- 缺口: ① main.cjs 启动时无 `[electron] mode=dev|prod` 显式日志；② F4 注释漂移（"dist/index.js"→"dist/src/index.js"）在 backend-spawn.cjs L12/L62 注释与 main.cjs 注释；③ 无 dev/prod 双路径 runbook；④ 无 WIRE 运行证据模板。
...
| 层 | 用例 | red 前提 |
|:---|------|------|
| L1 单元 | buildCommand('prod') === node dist/src/index.js（新，锁 F4 事实） | 当前测试未锁——改错路径会静默通过 |
...
3. **DS3**: F4 注释漂移修复——`grep -n "dist/index.js" electron/*.cjs` 零结果（全部为 dist/src/index.js）
