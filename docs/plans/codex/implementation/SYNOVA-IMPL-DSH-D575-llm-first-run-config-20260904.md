...
| src/server.ts | 修改 | L340 附近（credentialRoutes 挂载相邻）`app.use(llmConfigRoutes)` + `onLlmCredentialChanged` 订阅一条 `config/llm-changed` 事件日志（进程不重启声明）。〔2026-09-05 编码修正：实际挂载位置在 L293 `app.use(jwtAuthMiddleware)` **之前**——编码实测发现全局 JWT 中间件会拦截 L340 位置（credentials.ts 亦被拦，spec §3.1「无认证中间件」仅路由层成立）；spec §6 决策 5 要求首启在无用户系统时可用 → 挂载前移实现同等语义，生产态探测通过（evidence/D575/e2e-prod-mode-probe.txt）〕 |
...
| tests/services/llm-credential-store.test.ts | 新建 | L1 单元：正常存取+0600 断言（skipIf win32）/ 空值=未配置 / env 回退 / 损坏文件降级不崩 / onChanged 触发 / tmp 注入隔离。〔2026-09-05 编码修正：原声明 tests/llm-credential-store.test.ts——pre-commit 组 2b 配对映射强制 src/services/X → tests/services/X（CI 权威硬阻断），用例内容零变化〕 |
| tests/routes/llm-config.test.ts | 新建 | L2a 真实路由（createServer 先例，铁律 12 不 mock 管线）+ L2b 降级（stub 上游 401/429/500/连接拒绝→错误码分类）+ L2c 边界（空/非法字符 400 不回显、未知字段拒绝、retryPolicy 预留不炸、热重载同进程断言 loadConfig 变化）。〔2026-09-05 编码修正：原声明 tests/llm-config-api.integration.test.ts——同上配对映射 src/routes/X → tests/routes/X（tests/routes/ 既有 *.test.ts 惯例，sessions-api.test.ts 先例），用例内容零变化〕 |
...
| 层 | 文件 | 覆盖 | 数量 |
|---|---|---|---|
| L1 单元（store 契约三路径） | tests/services/llm-credential-store.test.ts（原声明 tests/llm-credential-store.test.ts，组 2b 配对映射修正——§3.3.1 注记） | 正常存取+0600 / 空值未配置 / env 回退 / 损坏降级 / onChanged / 注入隔离 | ≥7 用例 |
| L2a 接线（真实路由，铁律 12） | tests/routes/llm-config.test.ts（原声明 tests/llm-config-api.integration.test.ts，同上修正） | createServer 真实 HTTP：GET 空值语义 / POST 保存+maskedKey / 热重载同进程断言 / test 200 | ≥5 用例 |
...
2. **DS2** tests/services/llm-credential-store.test.ts 全绿（≥7 用例：正常+0600 / 空值 / env 回退 / 损坏降级 / onChanged / 隔离）。verify: `npx vitest run tests/services/llm-credential-store.test.ts`
3. **DS3** tests/routes/llm-config.test.ts 全绿（L2a+L2b+L2c：真实路由 + stub 上游错误码分类 + 400 不回显 + 热重载同进程断言）。verify: `npx vitest run tests/routes/llm-config.test.ts`
