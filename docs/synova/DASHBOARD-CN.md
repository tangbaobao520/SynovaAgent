...
> | **D356** | **P0 哨兵阈值告警接线 + 降级误报修复**（P0-1 + P1-1 + P1-3）：sentinel-loader 注册挂 manifest（禁 this.manifest 门控空转）；aggregate 拦截 degraded（不误报 critical"现金流危急"）；capital-health 缺字段不默认为 0 | **P0** | ❌ 需写 dev doc |
