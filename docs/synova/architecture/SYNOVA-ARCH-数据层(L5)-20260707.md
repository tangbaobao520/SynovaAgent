...
| 通道 | 数据类型 | 代码位置 | 状态 |
|------|---------|---------|------|
| 连接器管道 | 结构化经营数据（ERP/财务/电商） | `l5/connector-pipeline.ts` | ⚠️ 管线已通、真实连接器未实现（MVP 走上传/CSV/字段映射，直连推迟到部署后） |
...
电商后台的直连连接器未实现，推迟到部署后。MVP 阶段结构化经营数据走上传/CSV +
...
> 现状注记：外部系统直连推迟到部署后（MVP 走上传/CSV/字段映射）；IM 当前仅飞书接入（企微待实现）。
...
- **部署接入**：数据接入走上传/CSV/字段映射（真实连接器推迟到部署后）→ 对比诊断（访谈认知 vs 数据现实）
...
直连推迟到部署后，MVP 走上传/CSV/字段映射；IM 飞书已接入、企微待实现；访谈引擎
