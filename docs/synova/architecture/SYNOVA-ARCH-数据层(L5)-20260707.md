...
| 通道 | 数据类型 | 代码位置 | 状态 |
|------|---------|---------|------|
| 连接器管道 | 结构化经营数据（ERP/财务/电商） | `l5/connector-pipeline.ts` | ⚠️ 管线已通、直连连接器按施工图 Stage 1+ 推进（非搁置）；配置层（凭证/字段映射）照常，MVP 走上传/CSV/字段映射 |
| IM 接入 | 非结构化沟通数据（飞书消息，企微待实现） | `l1/im-inbound.ts` | ✅ 飞书 Webhook接收→身份识别→Session存储 |
| GA/访谈采集 | 结构化访谈数据（初次诊断的10-15个问题） | `interview/engine.ts` | ✅ 多角色访谈引擎+矛盾检测 |

### 1.1 连接器管道

管线流程已通：Python连接器 → PythonBridge → OntologyEvent[] → OntologyEventBus → GraphStore。
支持多租户隔离（orgId），凭证加密存储。
真实连接器当前仅 `synova_worker/connectors/feishu.py`（飞书）一个——ERP、财务软件、
电商后台的直连连接器按 DSH 迁移施工图 Stage 1+ 推进（非搁置）；配置层（连接器凭证/字段映射/客户包 CFG）不受影响、照常推进。MVP 阶段结构化经营数据走上传/CSV +
字段映射（`extensions/ontology/field-mappings/` 的 crm/erp/hr-standard.json）接入。

### 1.2 IM 接入

飞书的 Webhook 推送消息 → 解析发送者身份 → 存储到 Session（企微适配待实现）。
...
> 现状注记：外部系统直连按施工图 Stage 1+ 推进（非搁置；配置层照常推进），MVP 走上传/CSV/字段映射；IM 当前仅飞书接入（企微待实现）。
...
- **部署接入**：数据接入走上传/CSV/字段映射（直连连接器按施工图 Stage 1+ 推进、配置层照常）→ 对比诊断（访谈认知 vs 数据现实）
...
**数据层三条采集通道已定义（连接器管线已通、真实连接器仅飞书，ERP/财务/电商
直连按施工图 Stage 1+ 推进（非搁置），MVP 走上传/CSV/字段映射；IM 飞书已接入、企微待实现；访谈引擎
可用）。存储层基于 SQLite 本地部署，PII 脱敏已实现。**

- 连接器管道：Python桥接模式已通，可扩展新数据源（当前真实连接器仅飞书）
- IM接入：飞书已对接（企微待实现）
