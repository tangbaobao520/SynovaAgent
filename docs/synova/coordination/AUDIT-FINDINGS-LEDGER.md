...
| 09-15 | K3 批次四 A 组 | 复核 | **A 组 6 项独立重跑全绿**（D759 tsc TS2503=0 / 残余 28 全在 ci.yml:77 过滤名单；D757 反向实验：改回旧 import 路径 → 该文件重整 red；D758 豁免正向单域 win + ownership.yaml:158 在位 + 两套测试 51/24；D751 三环 E2E 5/5 + 反向删夹具 manifest.expert → 3 环必红（真硬断言）；D752 门禁 9✅ + 集成 4/4；D754 numstat +19/−3 且过注释符后增删=0 行）。**3 处 evidence 数字失实**（不改变方向性结论）：D759 报告称 @types/nodemailer 8.1.1 实测锁文件 8.0.1；D751 称 tests/sentinel 919 passed 四口径均不可复现（实测 227 passed / 1 skipped）；D754 称 runner 相关 3 文件 12 passed 实测直连 2 文件 8 用例 | M2 声称 vs 事实（证据数字未与实测对齐） | 无需修复（结论成立）；证据规范：引用测试须标覆盖率归属 | skill（dev doc 证据规范） |
| 09-15 | K3 批次四 C 组 | PASS | **D651 完美复现（本批最高质量交付）**：在 K3 工作区新建 **pinned worktree @742a3416**（=报告自述评估时点）重跑 **8/8 样本 + 全局基线逐数字命中**（W1 20/20+40/40 · W2 33=5+17+11 · W3 4/4 · W4 12/12 · M1 40/40 · M2 64/64 · M3 10/10+DS8 flip 1/1 · M4 21/21；全量 `29 failed\|4245 passed\|24 skipped (4298)` + tsc 28 —— **含失败数**均一致）。M3 的 `D577_FLIP_TEST=1` 门控确认为刻意设计（会改盘 manifest → 防污染并行测试 + try/finally 还原；带 flag 实测 1 passed 且运行后工作区干净）。**P2**：报告数字绑 742a3416 未显式声明 ⇒ 在 current main 复跑会见 W1 20→22 / W4 12→13 / M4 21→25 的自然增长（第三方易误判失实）；**P2**：样本 W1=D485/W4=D492 本身仍 `impl_done`/audit 空（代码质量评估 ≠ 任务验收）| 数字时点声明缺失（M2 根因同族，并入 CT-67）| 报告头部加『数字口径: <sha>』；排 D485/D492 补审 | audit + 控制塔 |
| 09-15 | K3 批次四 B 组 | P1×2 | **D713**：证据 json context.tests 称 desktop-build.test.ts → 26/26，实测 **23 passed + 3 skipped**；更关键——该文件全文 grep -c 'D713|codesign|verify-package-signature' = **0**，23 个 it 全属 D504/D517/D581 ⇒ **被引为该验收证据支撑的测试完全不含本任务覆盖**（D713 真实载体是 tests/desktop/verify-package-signature.test.sh 9 断言）。机制本体扎实（先自测再校验、fail-closed 三态、CI 上传前拦截）故判 **CONDITIONAL PASS** | M4 证据链断裂（引用关系无机器校验） | 修证据文件口径 | skill（dev doc 证据引用须标覆盖率归属） |
| 09-15 | K3 批次四 B 组 | P1×1 | **D741 时点漂移**：交付物定位的「audited+FAIL 显示为已完成」已由**同日** 09d8754c(#553) D748 修复（sync.js:171-181 新增 verdictBoardStatus()/resolveBoardStatus()，VERDICT_FAIL_PATTERNS=[FAIL,NOT-AUDITABLE]）——D741 定位正确且直接驱动 D748 立项，但交付物未标时效 ⇒ 审计方直采会重复修已修缺陷。同批提示：D748 自身 status=claimed / audit=null 未经独立审计。（定位结论本体 8 条 evidence 逐条复验 **7 成立**）| 无双向引用（closes_finding）→ 报告永挂「开着」态 | 派单模板增 closes_finding；D748 排下一批 | 控制塔 |
| 09-15 | K3 批次四 B 组 | PASS | **D714**（去重：已在 2026-09-13-D715.md:183-187 审过，本批做现状复验）：preload 收敛真实性成立——preload.cjs:17 仅 require electron / :23-30 改 argv 读取；全 electron/ grep nodeIntegration 仅 main.cjs:88=false（**无泄漏**）+ contextIsolation:true(:87)；走 additionalArguments 官方通道而非关沙箱（安全姿态未降级）。补跑 D715:187 自认未跑的回归：tests/electron/ **13 文件 170 passed / 3 skipped**。首跑 1 红的 react-markdown 为 K3 工作区 node_modules 环境缺口（非产品缺陷，D316 口径）| — | 1-4 转绿仍需发布物路径实测（缺口见 CT-70） | — |
| 09-15 | K3 批次四 C 组 | 复核×9 | **9 项全过**：D584（tests/sentinels 178 文件 696 用例全绿）、D597（server.ts:86→bootstrap.ts:27 副作用 import 链完整 + 7/7）、D601（68 处实违 vs 架构门禁「全部通过」假绿论断确认为结构性事实）、D660（ACTIVITY_WINDOW_MS=48h + 42/42）、D661（scripts/ 全目录 -P 清零 + 回归网 38 通过；ci.yml:140 判非缺陷：ubuntu-latest + GNU grep）、D663/D665（CI 接线在位；job 级结论环境受限不可复验）、D721（15 通过；三点差语义正确）。**D589 CONDITIONAL PASS**：9 份证据 schema/时间戳合格，但对线 1 六点全判 pass 中 **1-4/1-5 现均 failed**（1-4 判据 backend-spawn.test.ts+WIRE CHECK 全绿——正是「断链时全绿」盲区断言组）| M2 口径（09-08 快照以现在时陈述）+ 重验机制结构性风险（旧断言集不覆盖真缺陷 → 产出无信息量的绿，见 CT-69）| 无需修复 | audit |
| 09-15 | K3 批次四 D 组 | NOT-AUDITABLE×8 | **治理类 8/11 无可审计证据**：D531/D532/D552/D553/D554/D555/D557/D562 的 task-state impl.evidence/impl.files 全空（D555 甚至 commit: "<待提交>" 字面占位仍在）⇒ 无法做写集对账、无验证命令可复跑，按派单 §三.4 判 **NOT-AUDITABLE 并写清缺什么**（不写「基本通过」）。有 commit 的 4 项标题可读但「commit message 说了什么 ≠ 做成了且验证过」（M2）。**PASS 3 项**：D530（CT-45 merge 豁免 post-commit.sh:43-53 HEAD^2 判定 + bypass 留痕 1 条）、D570（4 项 grep 逐字相符）、D571（push-sync-guard 16/16 实跑绿；ALLOW_MAIN_PUSH 实测 5 处 vs 声称 1 为口径不精确）| 文档豁免路径是否该保 impl.evidence 必填（CT-71）| 补证据后重审 | 控制塔 + devdoc |
| 09-15 | K3 批次四 A 组 | 复核 | **D712 六点 quote↔证据文件 6/6 逐条对齐**（含 1-5 判 fail 的口径交叉互证：D716/#538 做的正是旧引导退场，反向证明 1-5 当时非真无需此修复）。3 条轻微 finding（tables 表数 54 无原始输出佐证 / refresh 快照未入 git / 1-6 quote 细节未落盘）| M4 轻微（快照固化习惯）| — | audit |
...
| CT-67 | **证据-任务归属未校验（M2/M4 统一机制）**：证据文件/证据 json 引用测试时，(1) 引用的测试须 ≥1 处命中被审任务 ID；(2) 证据中的数字必须附可复现命令 + 原始输出（或明示环境依赖/时点），禁以现在时陈述历史快照；(3) 引用测试须标注「覆盖面归属」（该测试覆盖哪个任务/哪批）。2026-09-15 批次四实证：D713 证据引 `desktop-build.test.ts` 而该文件 **0 行 D713 覆盖**（真实载体是另一个 9 断言脚本）；D759 报告称 @types/nodemailer 8.1.1 实测锁文件 8.0.1；D751 称「919 passed」四口径不可复现；D754 称「3 文件 12 passed」实测 2 文件 8 用例；D589 以现在时陈述 09-08 快照计数。**一处机制收口五处同族**（不为每错加脚本）| 2026-09-15 K3 批次四（A/B/C 组各 1-3 处同族实证） | 🔧 dev doc skill（evidence 规范）+ K3 审计断言集 |
| CT-68 | **CONDITIONAL PASS 的「转正触发器」缺失**：CP 条件满足后无人被通知、无归属人、无到期——实证 2026-09-15 D740（K3 判 PASS）：「9 个对象中 **0 个已完全闭合**，7 个 CP 仍有 8 条条件开着（含 D518/D519 条件已满足但 verdict 未转正）」；`task-state/D518.json`/`D519.json` 仍为 CP。**修复方向**：`gen-cto-health.py` 或看板增独立列/独立待办「CP 条件已满足但 verdict 未转正」（需读 audit.findings 的条件状态或 task-state 显式条件字段）| 2026-09-15 K3 D740 审计 L4 发现 1 | 🔧 控制塔（CTO 派单） |
| CT-69 | **重验类任务须附「断言集覆盖度」判据**：「跑既有测试出新证据」作为 stale 恢复手段时，若既有断言集不覆盖该点历史缺陷，产出的绿**没有信息量**。实证 2026-09-15 批次四 D589：9 份证据对线 1 六点全判 pass，其中 **1-4/1-5 现均 failed** —— 1-4 的判据是 `backend-spawn.test.ts` 全绿 + `main.cjs` WIRE CHECK 断言全绿，**该断言组无法发现 F3 渲染层断链**（正是 D715:159 已指出的「断链时全绿」盲区组）。**修复方向**：K3 审计标准增补——重验类结论须回答「本判据能否发现该点的历史缺陷形态」；验收点转绿须绑**端到端形态**证据而非仅测试套件绿 | 2026-09-15 K3 批次四 C 组（D589）+ D715:159 前置发现 | 🔧 K3 审计标准 + dev doc（证据类型选择）|
| CT-70 | **启动探针输出不落盘（机制建成、出口未接，M3 家族）**：D714 修的 F3 启动自检哨兵（`electron/main.cjs:111-125` `did-finish-load` 后 `executeJavaScript` 探测 `window.electronAPI` → 打印 `[preload-check] OK/FAIL`）设计正确，**但输出只进 stdout 日志**——人不在场即蒸发。后果：1-4 验收点至今无发布物路径实测证据（`docs/synova/product-lines/evidence/` 无 D714 证据文件），状态机维持 `failed`。**修复方向**：(1) 探针结果同时写入文件（如 `~/.synova/preload-check.log` 或包内日志）；(2) `desktop-build.yml` 增日志采集 step 作为 artifact 落盘 | 2026-09-15 K3 D714/D713 审计 L4 发现 | 🔧 编码线（探针落盘）+ 控制塔（CI 采集）|
| CT-71 | **文档/治理豁免路径下 `impl.evidence` 未被强制**：纯文档提交走 CT-34 异常通道（只保留 Secrets 扫描），治理类任务的 task-state 证据字段校验一并被跳过 → `impl_done` 可长期零证据。实证 2026-09-15 批次四 D 组：**8/11 治理任务** `impl.evidence`/`impl.files` 全空（D531/D532/D552/D553/D554/D555/D557/D562），其中 D554 **改 post-commit 行为**（门禁邻近变更）同样无证据；D555 的 `commit: "<待提交>"` 字面占位长期存活。**修复方向**：(1) `cto-taskstate-consistency` 增占位符检测（`<待提交>`/`TBD`/空 evidence 且 status=impl_done）；(2) 豁免路径是否保留「evidence 非空」一项待创始人/CTO 裁决 | 2026-09-15 K3 批次四 D 组 | 🔧 控制塔（task-state 一致性校验）|
