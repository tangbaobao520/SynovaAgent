...
| CT-78 | **canary 假覆盖 fail-closed 在 CI 层不生效 + tests/project 绿腿 ⚠ 不可见（D858 收口实测，2026-09-21）**：①`.github/workflows/ci.yml:277` `bash scripts/control-tower/check-canary-drift.sh \|\| true` **吞掉 D858 新增的 exit 1** → K3 的 L4-1 条件在 CI 层仍不阻断（脚本层已 fail-closed + `::error` 上面板，job 仍绿）；②PR #682 的 tests/project 循环把 stdout 重定向到 `/tmp/ct-out-project.log` 且**仅失败时 cat** → 绿腿上 Windows 反斜杠 ⚠ 不可见（K3 §5 P2 的载体段；D858 只闭环了机制段/独立通道）。**修复方向**：①`RC=$?; [ "$RC" -eq 1 ] && FAIL=1` 或删 `\|\| true`（漂移/幽灵须保持 exit 0，不误伤存量 71 条）②成功路径回捞 `grep '⚠' /tmp/ct-out-project.log && echo "::warning …"`，或测试直写 `$GITHUB_STEP_SUMMARY`（独立通道范例见 `docs/synova/coordination/D858-证据包标准件-匿名取CI证据-20260921.md`） | 2026-09-21 D858 队内自验 C3 + 队长实测 grep（D858 写集硬边界禁碰 ci.yml） | 🔧 **D862**（已登记 `task-state/D862.json`；等 #682 与 #657 合并释放 ci.yml 单写者后开工） |
