...
| CT-72 | **旧格式 audit 段对新口径不可见**：`task-state/D492.json` 的 audit = `{result:'pass', by:'codex', at:'2026-09-04'}`（**无 `verdict` 字段**）→ 新口径（verdict 维度）与看板读不到该裁决。全仓普查：新格式 153 / 无 audit 91 / **旧格式 1**（仅 D492）→ 需一次性归一（把 `result` 映射为 `verdict` 或标注为历史验收） | 2026-09-16 CTO 验收 K3 批次四时普查发现 | 🔧 控制塔（一次性归一脚本） |
| CT-73 | **BACKFILL 留痕机制未实现**：K3 自报「用 `core.hooksPath=/dev/null` 跳过本地软提示后手工补记 bypass」时发现，全仓 `scripts/`+`.github/` **零 BACKFILL 代码** （台账 §6.1 第 3 项计划未落地）→ 绕过留痕目前只能靠自觉手工写；本批 K3 自报的那条已按手工格式落 `bypass.log`，属可核。需把「跳过本地门禁」变成自动登记（hook 层或 pre-push 对账） | 2026-09-16 K3 批次四自报 + CTO 复核实测（grep 零命中） | 🔧 控制塔（hook/对账） |
