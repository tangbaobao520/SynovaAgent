...
`git diff origin/main...HEAD --name-status`：**73 files = 16 A + 26 D + 31 M**（含卡提交当刻实测；勘误：原记 72=15A 少 1 A，V 自验指正）。
- PR 预算（D734 ≤12 文件 + 单域）：**73 ≫ 12 超限**。治理产物豁免口径（D860）适用与否由 CTO 判。
