...
| docs/计划库 + 双仪表盘 | **Codex (Win)** | Claude 线 dev doc + D# 统一分配 + 任务登记；**DSH 线 dev doc 由 DSH 自出**（护栏见 §一） |
...
| 写集重叠 | 出 doc 方（Codex/DSH）声明写集；verify-parallel 查重叠；重叠 → 停手问创始人 | 机器（pre-push） |
...
### DSH 自出 dev doc 的护栏（2026-08-16 定，防"自我闭环"）

DSH 线任务的 dev doc 由 DSH 自出（见 [DSH-DEVDOC-MODE.md](DSH-DEVDOC-MODE.md)），但 maker-checker 分离不取消——只是换层：

1. **结构门禁**：doc 过 dev-doc-gatekeeper（与 Codex 出的一样：Q0-Q3/写集/DS 清单/Done 可证伪）
2. **D# 编号**：Codex 统一分配（唯一登记口，防重号）——DSH 出 doc 前向 Codex 要号
3. **PR 预审**：DSH 的 PR（doc+实现）→ Codex 预审
4. **K3 审计**：无豁免——自出 doc 自实现的任务，交付端由 K3 全量核（红线 3）
5. **写集契约**：照样声明写集，verify-parallel 照样查重叠
...
```
周一/三：Codex 出 Claude 线 dev doc + DSH 自出 DSH 线 dev doc（双线并行，D# 由 Codex 分配）
周中：两条线同时实现（一人一事一分支）
