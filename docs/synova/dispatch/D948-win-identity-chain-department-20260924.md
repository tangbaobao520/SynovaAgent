# D948 派单件 — 身份链补全：部门可见性恢复（服务端签发/接收 + 桌面端身份通道切换）

> 出件：Win-Codex-CTO ｜ 2026-09-24 ｜ 状态：**待 CTO 复核 PLAN 后放行实施**
> 号段：`D947–D951`（Mac-CTO 2026-09-24 发放）。本件取 **D948**；D947 已被本方占用（未合 main）。
> 归属基线：`docs/synova/coordination/ownership.yaml`（现行表，机器可读唯一源）
> 依赖前置：**D947 必须先合 main**（本件切片 A 的 base = D947 分支 tip）
> 派单即规格。凡本件与任何旧基线冲突，**以本件 + ownership.yaml 为准**。
