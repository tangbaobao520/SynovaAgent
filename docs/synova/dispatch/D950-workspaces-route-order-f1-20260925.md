# D950 派单件 — 工作区路由遮蔽修复（W2/F-1）+ 错误码顺序（E-2）

> 出件：Win-Codex-CTO ｜ 2026-09-25 ｜ 号段 `D947–D951` 内自取 D950
> 归属基线：`docs/synova/coordination/ownership.yaml`（写集实测全 win）
> 来源：D947 `D947-RULINGS.md:253`（L-28「本卡不修 ordering，另立卡」）+ D948 PLAN §1-补 2 运行时探针 + K3 审计 §一.5（E-2）
...
**后果链**：`/mine` 恒 404 → 部门工作区页（`department-workspace.ts:89-98`）对 `undefined.length` 抛错被 `console.warn` 吞掉 → 页面永久停在「加载中…」。D948 切片 A 修好签发/验签层后，用户**仍然看不到本部门工作区**，因为入口本身是死的。
...
| `tests/routes/workspace-access-write-endpoint.test.ts` | 改：其 `:425` 起的「锁定现状 404」断言已过时，改为锁新语义 | code-a |
| `task-state/D950.json` | 卡片 | 队长 |

**不做（明确排除）**：

- **不碰 `src/middleware/auth.ts`** —— 该文件由 D948 切片 A 持有（`JwtPayload +department`），本卡若碰即同文件双写者。K3 审计提的 `auth.ts:82` 陈旧注释（写「挂载于 server.ts:315」，实测 344）**移出本卡写集**，登记为待办（D948 A 合后随下一张顺手改）。
- 不修 `GET /api/workspaces`（`:92`，`_req` 无守卫）的既有缺口 —— 那是 F-5，**与本卡的"新放开可达性"不同族**，另立卡；本卡只保证"新变可达的端点必须有守卫"。
...
| 理由 | `src/routes/workspaces-api.ts` 会被 D947 PR-2（`2ea4df10`）改写（`:266` 起自报读取已删、加 `requireVerifiedRbac`）；基于旧 main 开卡必冲突 |
| 分支 | `fix/d950-workspaces-route-order` |
| 前置 | D949 与本卡**可并行**（域不同、文件不重叠）；D948 切片 A 与本卡**同域不同文件**，可并行 |
...
| F5 | 三路径夹具：正常 / 降级（无 `JWT_SECRET` → 401 + 留痕）/ 边界（空 id、不存在 id） | — |
| F6 | 变异体：至少对 F1、F2 各做 1 个「改坏即红」并给**原始输出 + 真实退出码** | — |
...
| # | 项 | 归属 | 处置 |
|---|---|---|---|
| 1 | `auth.ts:82` 陈旧注释（`server.ts:315` → 实测 344） | Win | 移出本卡（D948 A 持有该文件）；D948 A 合后随下一张改 |
...
| 4 | 渲染层 15 个 fetch 点身份覆盖面（D948 §7-12） | **mac** | 本卡不碰；D948 出白名单对照表后另立卡 |
| 5 | W1（`department` 无生产写入者）、W3（桌面无部门工作区消费点） | **产品决策** | 待创始人拍板后立卡 |
