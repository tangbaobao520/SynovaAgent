...
const sqlStats = { graphTriplesPrepare: 0, graphTriplesPrepareFail: 0, graphTriplesInsert: 0, graphTriplesInsertFail: 0 };
  ...
  return new Proxy(target, {
    get(t, prop, recv) {
      ...
      if (typeof v === 'function' && typeof prop === 'string') {
        return (...args: unknown[]) => { counts[prop] = (counts[prop] || 0) + 1; return (v as (...a: unknown[]) => unknown).apply(t, args); };
      }
      return v;
    },
  });
...
for (const { manifest, dir } of load.sentinels) {
  ...
  sqlStats.graphTriplesPrepare = 0; sqlStats.graphTriplesPrepareFail = 0; sqlStats.graphTriplesInsert = 0; sqlStats.graphTriplesInsertFail = 0;
  ...
  try {
    raw = await (obj as { check: (...a: unknown[]) => unknown }).check(storeProxy, 'probe-team', travProxy, thresholds);
  } catch (e) {
...
writeFileSync(`/tmp/verify-d965/probe-${label}.json`, JSON.stringify({ label, dbPath, loadSentinels: { count: load.sentinels.length, degraded: load.degraded, errors: load.errors }, rows, summary: { total, arrayCount: arrayRows.length, threwCount: threwRows.length, nonArrayCount: nonArrayRows.length, zeroFindingsRaw, zeroFindingsUnpacked, zeroStoreCalls, unpackedNonZero, edgeReadPrepare: edgeRead, edgeReadPrepareFailed: edgeReadFail, edgeReadPrepareSucceeded: edgeRead - edgeReadFail } }, null, 2));
db.close();
