 ...
 */
const WTL = (process.argv.find(a => a.startsWith('--wt=')) || '').slice(5) || '/Users/wane/SynovaAgent/.synova-wt-verify-d965';
const LBL = (process.argv.find(a => a.startsWith('--label=')) || '--label=default').slice(8);
process.env.SYNOVA_DB_PATH = '/tmp/verify-d965/db-loader-' + LBL + '.db';
const WT = WTL;
process.chdir(WT);
...
fs.writeFileSync('/tmp/verify-d965/probe-loader-path-' + LBL + '.json', JSON.stringify({ dbPath: (db as unknown as { name: string }).name, registered: reg, listLength: list.length, out }, null, 2));
console.log('json_written=/tmp/verify-d965/probe-loader-path-' + LBL + '.json');
