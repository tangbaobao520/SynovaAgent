...
**详细锚点附件（B-01…B-20 逐符号分册）**：
- `04-技术研究/DSH锚点重核-0.1.7-alpha.2源码树-B01-B05.md`（B-01…B-05，634 行）
- `04-技术研究/DSH锚点重核-0.1.7-alpha.2源码树.md`（B-11…B-15，504 行；25 锚点 / 24 条复现命令）
- `04-技术研究/DSH锚点重核-0.1.7-alpha.2源码树-B16-B20.md`（B-16…B-20，496 行）

> **仓库交付位置**：`docs/synova/research/D927-DSH借鉴重新对比审计-20260923/`（分支 `docs/d927-dsh-recompare-audit-v2`，已推送 `81e68ba0`）。上述三份附件在该目录 `附件/` 子目录下；`task-state/D927.json` 已同步指向 v2 路径。
