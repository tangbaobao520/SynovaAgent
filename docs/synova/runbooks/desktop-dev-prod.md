...
| 路径 | 触发 | 后端 | 窗口加载 | 定位 |
|---|---|---|---|---|
| **prod（唯一用户路径）** | 双击安装的 SynovaAgent.app | `ensureBackend(prod)` 本机 spawn `node dist/src/index.js`，注入 `SYNOVA_DB_PATH=<userData>/data/synova.db` | `loadFile(resources/renderer/index.html)` → 首诊页 | FDE |
...
双击 SynovaAgent.app
  ...
  → ensureBackend({ mode:'prod', cwd:process.resourcesPath,
      ...
      → 不可达 → spawn node dist/src/index.js（SYNOVA_DB_PATH 注入）→ 探活轮询 → 重启限次 3/10min
