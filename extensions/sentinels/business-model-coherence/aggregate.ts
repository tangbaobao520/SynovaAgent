...
export const businessModelCoherenceSentinel = {
  async check(store: GraphStoreReader, teamId: string, traversal?: GraphTraversal): Promise<SentinelFinding[]> {
    ...
    try {
      ...
      if (r.score < 0.2) return [{ id: `i7-crit`, severity: 'critical', title: `商业模式一致性低 (${(r.score*100).toFixed(0)}%)`, description: '价值主张-收入-成本结构存在明显不一致。', evidence: [`一致性: ${(r.score*100).toFixed(0)}%`, ...r.signals], suggestion: '审视核心价值主张与收入模式的匹配度。', detectedAt: checkedAt }];
      if (r.score < 0.4) return [{ id: `i7-warn`, severity: 'warning', title: `商业模式一致性偏低 (${(r.score*100).toFixed(0)}%)`, description: '部分维度存在不匹配。', evidence: [`一致性: ${(r.score*100).toFixed(0)}%`, ...r.signals], suggestion: '优化收入模式或成本结构。', detectedAt: checkedAt }];
      if (r.signals.length > 0) return [{ id: `i7-info`, severity: 'info', title: `商业模式一致性 (${(r.score*100).toFixed(0)}%)`, description: '基础一致但部分定义缺失。', evidence: r.signals, suggestion: '补充缺失的商业模式定义。', detectedAt: checkedAt }];
      return [];
    } catch (err: unknown) { log.error({ err }, '[bizmodel-coherence] 失败'); return [{ id: `i7-error`, severity: 'warning', title: '检测异常', description: `${(err as Error)?.message || String(err)}`, evidence: [], suggestion: '检查数据源。', detectedAt: checkedAt }]; }
  },
};
