...
export const capitalTurnoverSentinel = {
  async check(store: GraphStoreReader, teamId: string): Promise<SentinelFinding[]> {
    ...
    try {
      const f = store.queryNodes('FINANCIAL', { teamId }).map(n => ({ revenue: Number(n.props.revenue) || 0, totalAssets: Number(n.props.totalAssets) || 0, currentAssets: Number(n.props.currentAssets) || 0, accountsReceivable: Number(n.props.accountsReceivable) || 0 }));
