...
export const financial-snapshotSentinel = {
  ...
  async check(store: GraphStoreReader, teamId: string): Promise<SentinelFinding[]> {
    ...
    try {
      ...
      log.info({ teamId }, 'financial-snapshot check completed (stub)');
    } catch (err: any) {
      log.warn({ err, teamId }, 'financial-snapshot check failed — degraded');
    }
    return findings;
  },
};
