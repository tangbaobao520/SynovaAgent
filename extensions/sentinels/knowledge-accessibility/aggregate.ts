...
export const knowledgeAccessibilitySentinel = {
  async check(s: GSR, tid: string): Promise<SentinelFinding[]> {
    ...
    try { const nodes = s.queryNodes("ALL",{tid}); const r = computeKnowledgeaccessibility(nodes.length);
      if (r.score<0.2) return [{id:"o4-${n.getTime()}",severity:"critical" as const,title:"O4低",description:"需改进",evidence:[`${(r.score*100).toFixed(0)}%`],suggestion:"评估。",detectedAt:ca}];
      return [];
