...
export const makeOrBuySentinel = {
  async check(store: GraphStoreReader, teamId: string, traversal?: GraphTraversal): Promise<SentinelFinding[]> {
    ...
    try {
      ...
      if (r.health < 0.2) return [{ id: `i12-crit`, severity: 'critical', title: `自制/外购决策风险 (${(r.health*100).toFixed(0)}%)`, description: '核心能力被外包。', evidence: [`健康度: ${(r.health*100).toFixed(0)}%`, `外包核心能力: ${r.outsourcedCore.join(',') || '无'}`], suggestion: '评估核心能力是否不应外包。', detectedAt: checkedAt }];
      return [];
