...
export const orgRepairabilitySentinel = {
  async check(s: GSR, tid: string): Promise<SentinelFinding[]> {
    ...
    try { const nodes = s.queryNodes("ALL",{tid}); const r = computeOrgrepairability(nodes.length);
      if (r.score<0.2) return [{id:"o8-${n.getTime()}",severity:"critical" as const,title:"O8低",description:"需改进",evidence:[`${(r.score*100).toFixed(0)}%`],suggestion:"评估。",detectedAt:ca}];
      return [];
