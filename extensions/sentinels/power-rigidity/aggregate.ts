...
export const powerRigiditySentinel = {
  async check(s: GSR, tid: string): Promise<SentinelFinding[]> {
    ...
    try { const nodes = s.queryNodes("ALL",{tid}); const r = computePowerrigidity(nodes.length);
      if (r.score<0.2) return [{id:"o9-${n.getTime()}",severity:"critical" as const,title:"O9低",description:"需改进",evidence:[`${(r.score*100).toFixed(0)}%`],suggestion:"评估。",detectedAt:ca}];
      return [];
