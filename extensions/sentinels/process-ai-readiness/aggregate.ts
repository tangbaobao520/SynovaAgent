...
export const ProcessaireadinessSentinel = {
  async check(s: GSR, tid: string): Promise<SentinelFinding[]> {
    ...
    try { const r = computeProcessaireadiness(s.queryNodes("Tool",{tid}).length*10);
      if (r.score<0.2) return [{id:"t5-${n.getTime()}",severity:"critical" as const,title:"T5 流程AI化就绪度低",description:"需改进",evidence:[`${(r.score*100).toFixed(0)}%`],suggestion:"评估改进。",detectedAt:ca}];
      return [];
