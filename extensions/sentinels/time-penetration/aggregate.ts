...
export const timePenetrationSentinel = {
  async check(store: GraphStoreReader, teamId: string, traversal?: GraphTraversal): Promise<SentinelFinding[]> {
    ...
    try {
      ...
      if (r.penetration < 0.2) return [{ id: `i9-crit`, severity: 'critical', title: `时间穿透力弱 (${(r.penetration*100).toFixed(0)}%)`, description: '组织对外部变化响应迟缓。', evidence: [`穿透力: ${(r.penetration*100).toFixed(0)}%`], suggestion: '建立前瞻性监测机制。', detectedAt: checkedAt }];
      return [];
    } catch (err: unknown) { log.error({ err }, '[time-penetration] 失败'); return [{ id: `i9-error`, severity: 'warning', title: '检测异常', description: `${(err as Error)?.message || String(err)}`, evidence: [], suggestion: '检查数据源。', detectedAt: checkedAt }]; }
  },
};
