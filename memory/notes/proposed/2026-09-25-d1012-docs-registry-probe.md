...
# D1012 文档登记对账：口径收窄 + 匹配语义升级

任务: D1012（task-state/D1012.json，状态 in_progress）
...
4. 子串匹配会造成**双向误判**：把 `evidence/README.md` 误判为已登记（命中 D:/ 遗留条目里的
   `README.md` 子串），同时把 `pat.h` 改坏也检不出来（D1012 改坏即红首轮实证：探针恒 exit 0）。
...
- **D1013/D1011 的新 `.md`**（`号段水位.md` 与两份 evidence 回执）在本分支不存在；
