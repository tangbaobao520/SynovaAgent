...
exports.auditGenomes = auditGenomes;
exports.auditGenomesRulesOnly = auditGenomesRulesOnly;
const llm_client_1 = require("../llm-client");
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/pipeline/audit-agent');
// ================================================================
// 规则驱动预审（零 LLM，快速拦截明显问题）
// ================================================================
function preAuditRules(genomes, taskDef) {
    const items = [];
    for (const g of genomes) {
        ...
        if (Array.isArray(g.antiPatterns)) {
            const genericPatterns = g.antiPatterns.filter(p => p.length < 10 ||
...
// LLM 审计 Prompt
// ================================================================
function buildAuditPrompt(genomes, taskDef, ruleItems) {
    const genomeText = genomes.map(g => {
        const models = (g.mentalModels || []).map(m => `    - ${m.name}: ${m.oneLiner} (source: ${m.source || '未标注'})`).join('\n');
...
// 解析 LLM 审计输出
// ================================================================
function parseAuditResponse(content, ruleItems) {
    let parsed;
    try {
        // 尝试提取 JSON
        ...
        log.debug('LLM 审计结果解析失败，回退到规则结果: %s', String(_e));
    }
    // 合并规则预审 + LLM 审计
    const llmItems = Array.isArray(parsed?.items)
        ? parsed.items.map((item) => ({
            dimensionId: item.dimensionId || 0,
            dimension: item.dimension || '未知',
            verdict: (['verified', 'inferred', 'unverifiable', 'failed'].includes(item.verdict) ? item.verdict : 'unverifiable'),
 ...
 * @returns 审计结果
 */
async function auditGenomes(genomes, taskDef, abortSignal) {
    // Step 1: 规则预审
    ...
    try {
        ...
        // LLM 审计不可用时，仅依赖规则预审
        log.info(`[audit-agent] LLM 审计不可用: ${err.message}，降级为规则预审`);
    }
    // Step 3: 合并结果
 ...
 * 用于冷启动或无 API Key 场景。
 */
function auditGenomesRulesOnly(genomes, taskDef) {
    const ruleItems = preAuditRules(genomes, taskDef);
    const summary = {
