...
exports.convertBlueprintToTeamTemplate = convertBlueprintToTeamTemplate;
exports.oceanTraitDesc = oceanTraitDesc;
// ================================================================
// 主转换函数
// ================================================================
function convertBlueprintToTeamTemplate(blueprint) {
    const fiveFormatsAgents = blueprint.fiveFormats?.templatePreset?.agents || [];
    const agents = blueprint.teamStructure.roles.map((role) => {
...
// Agent 描述
// ================================================================
function buildAgentDescription(role, genome) {
...
function governanceLabel(layer) {
    switch (layer) {
...
// SOUL.md 生成
// ================================================================
function buildSoulContent(role, genome) {
    ...
    if (genome.honestBoundaries.length > 0) {
        lines.push('## 诚实边界', ...genome.honestBoundaries.map((b) => `- ${b}`), '');
    }
    // 反模式
    if (genome.antiPatterns.length > 0) {
        lines.push('## 反模式', ...genome.antiPatterns.map((a) => `- ${a}`), '');
    }
    // 安全锚点
    lines.push('## 安全锚点（硬约束，绝对不可违反）', '- **禁止越权操作**：不得执行超出角色职责范围的操作', '- **禁止编造信息**：不确定时必须明确标注不确定性', '- **禁止绕过审批**：需要人工审批的操作不可自行决定', '', `**置信度**：${((genome.confidence ?? 0) * 100).toFixed(0)}%`);
    return lines.join('\n');
}
/** OCEAN 各维度的行为特征描述 */
function oceanTraitDesc(dimension, score) {
    const high = score >= 70;
    const low = score <= 30;
    const mid = !high && !low;
    switch (dimension) {
...
function buildFallbackSoul(role) {
    return [
        `# ${role.name} · SOUL.md`,
        '',
        '## 核心使命',
...
// ================================================================
function buildBootstrapMd(role) {
...
// ================================================================
function buildMemoryMd(role, blueprintId) {
...
// ================================================================
function buildToolsMd(role, skillSet) {
    const lines = [
        `# ${role.name} · TOOLS.md`,
        '',
    ...
    lines.push('## 权限边界', `- 治理层：${governanceLabel(role.governanceLayer)}`, '- 不可越权操作超出职责范围的任务', ...(role.specialPrivileges || []).map((p) => `- **特权**：${p}`), '');
    if (role.collaboratesWith?.length) {
        lines.push('## 协作对象', '', ...role.collaboratesWith.map((c) => `- ${c}`), '');
    }
    return lines.join('\n');
}
function buildAgentsMd(blueprint) {
    ...
    const lines = [
        ...
        '| 角色 | 治理层 | 职责 | 协作对象 |',
        '|------|--------|------|---------|',
        ...ts.roles.map((r) => `| **${r.name}** | ${governanceLabel(r.governanceLayer)} | ${r.responsibilities.slice(0, 2).join('、')} | ${(r.collaboratesWith || []).join('、') || '全部角色'} |`),
        '',
        '## 协作协议',
        '',
    ...
    lines.push('## 安全基线', '', `**最大自主权**：${safety.maxAutonomyLevel}`, `**审计日志**：${safety.auditLogEnabled ? '已启用' : '未启用'}`, `**需人工审批的操作**：${safety.requireHumanApproval.length > 0 ? safety.requireHumanApproval.join('、') : '无特殊限制'}`, '');
    // 设计依据
    ...
    // 工程纪律
    lines.push('## 工程纪律', '', '1. **角色边界不可逾越**：每个 Agent 严格在自身职责范围内行动', '2. **不确定即上报**：遇到超出认知边界的问题，标记为不确定而非猜测', '3. **安全基线刚性**：需人工审批的操作不可自行绕过', '4. **证据可追溯**：关键决策必须引用设计依据或证据链', '5. **Agent 之间直接协作**：无需经过人类中转，但敏感操作需人类审批', '6. **窗口期优先（张良 L-05）**：85 分的架构不优先于 60 分的可用性。当窗口期与"技术完美"冲突时，接受降级方案', '7. **单文件锁（墨子 M-04）**：多人并行改同一段代码前，先锁文件（公告或标记），改完解锁通知所有人', '8. **消息必须走中间件**：Agent 间通信必须通过 POST /api/v1/agent-message。L3 协议中间件对每条消息做运行时裁决（LOCK/BLOCK/WARN），违规消息自动阻断——这是团队协议的运行时保障，不可绕过', '', '> **USER.md delta-merge 已实装**：引擎以追加方式更新 USER.md（新 section 追加到末尾，已有 section 不覆盖）。用户通过 L0 对话或后续配置输入的个人信息，引擎有权写入新增 section。用户已编辑的内容（包括之前引擎写入的 section）不受影响。', '', `> 此 AGENTS.md 由 Synova 引擎（${blueprint.engineVersion}）自动生成。`, `> 覆盖等级：${blueprint.coverageLevel}`);
    return lines.join('\n');
}
// ================================================================
// protocol.json 生成（L3 中间件数据）
// ================================================================
function buildProtocolJson(blueprint) {
    const cm = blueprint.collaborationMode;
    const visibilityByStrategy = {
        central_repo: 'internal',
        pair_sharing: 'internal',
        downward_pour: 'internal',
        free_for_all: 'internal',
    };
    const sandboxByStrategy = {
        gatekeeper: 'full',
        ambassador: 'semi',
        buffer: 'semi',
        open_door: 'none',
