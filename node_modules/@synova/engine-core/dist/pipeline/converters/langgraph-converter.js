...
exports.synovaToLangGraph = synovaToLangGraph;
exports.langGraphToJSON = langGraphToJSON;
exports.langGraphToPython = langGraphToPython;
// ================================================================
// 主转换入口
// ================================================================
function synovaToLangGraph(yml) {
...
function buildSystemPrompt(role) {
    ...
    if (g.mentalModels.length > 0) {
        ...
        for (const m of g.mentalModels) {
            ...
            if (m.limitation)
                lines.push(`  Limitation: ${m.limitation}`);
        }
    }
...
function inferGovernanceLayer(role, index, total) {
    // 基于角色在列表中的位置和名称推断治理层级
    const name = (role.name + role.id).toLowerCase();
    if (/战略|决策|ceo|director|exec|board|govern/i.test(name))
        return 3;
    if (/管理|协调|orchestrat|manager|lead/i.test(name))
        return 2;
    if (/执行|开发|运营|design|dev|ops/i.test(name))
        return 1;
...
function inferTopology(yml) {
    const mode = yml.team.protocol.mode;
    const powerDist = yml.team.protocol.authorityGovernance;
    // 从协作模式推断拓扑
    if (mode === 'iron_captain' || mode === 'bytedance_flat')
        return 'star';
    if (mode === 'loose_federation')
        return 'chain';
    if (mode === 'democratic_council')
        return 'full_mesh';
    if (mode === 'cross_check_balance')
        return 'full_mesh';
    if (mode === 'haier_ren_dan_he_yi' || mode === 'haidilao_frontline_auth')
        return 'hierarchical';
    if (mode === 'mckinsey_partnership')
        return 'full_mesh';
    if (mode === 'tencent_internal_race')
        return 'star';
    // 从权力分布推断
    if (powerDist === 'hierarchical' || powerDist === 'centralized')
        return 'star';
    return 'full_mesh';
}
function buildStarEdges(roles) {
    if (roles.length < 2)
        return [];
...
function buildChainEdges(roles) {
    const edges = [];
...
function buildFullMeshEdges(roles) {
    const edges = [];
...
function buildHierarchicalEdges(roles) {
    const edges = [];
...
// 序列化
// ================================================================
function langGraphToJSON(config) {
    return JSON.stringify(config, null, 2);
}
function langGraphToPython(config) {
    const lines = [
        '"""',
