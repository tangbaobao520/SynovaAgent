...
exports.checkCrossPersonaCoherence = checkCrossPersonaCoherence;
exports.formatConflictsForLLM = formatConflictsForLLM;
exports.getCrossPersonaPenalties = getCrossPersonaPenalties;
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('pipeline/cross-persona-checker');
function buildProfiles(team, genomes) {
...
function r01_opennessClash(profiles) {
...
function r02_conscientiousnessClash(profiles) {
...
function r03_extraversionClash(profiles) {
...
function r04_agreeablenessClash(profiles) {
...
function r05_neuroticismClash(profiles) {
...
function r06_governanceLayerClash(profiles, mode) {
...
function r07_complianceVsInnovation(profiles) {
...
// R08: 速度角色 + 质量角色 — 节奏冲突
function r08_speedVsQuality(profiles) {
...
function r09_mentalModelGap(profiles) {
...
function r10_allGovernanceNoExecution(profiles, mode) {
 ...
 */
function checkCrossPersonaCoherence(team, genomes, mode) {
    const profiles = buildProfiles(team, genomes);
    const conflicts = [];
    const modePenalties = new Map();
        ...
        try {
            ...
            if (conflict) {
                ...
                for (const [m, penalty] of Object.entries(rule.modePenalties)) {
                    const entry = modePenalties.get(m) || { penalty: 0, conflicts: [] };
                    entry.penalty += penalty;
                    entry.conflicts.push(conflict.id);
                    modePenalties.set(m, entry);
                }
            }
        }
        catch (_e) {
            log.debug('跨角色规则检查异常: %s', String(_e));
        }
    }
    // 计算评分
    let score = 100;
 ...
 * 生成冲突摘要（注入 LLM 提示词）
 */
function formatConflictsForLLM(report) {
 ...
 */
function getCrossPersonaPenalties(team, genomes) {
    const dummyMode = 'iron_captain';
