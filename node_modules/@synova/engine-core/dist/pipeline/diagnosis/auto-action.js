...
const log = (0, logger_1.createLogger)('engine-server/pipeline/diagnosis/auto-action');
// ====================================================================
...
const ACTION_RULES = [
    ...
    // ── Intent Alignment ──
    {
        ...
        condition: (d) => !!(d.intentAlignment && (d.intentAlignment.humanOrgGap > 0.4 ||
            d.intentAlignment.agentOrgGap > 0.4 ||
            d.intentAlignment.humanAgentGap > 0.4)),
        generate: (d) => {
            const ia = d.intentAlignment;
            const issues = [];
            if (ia.humanOrgGap > 0.4)
                issues.push('人的注意力与组织目标偏离');
            if (ia.agentOrgGap > 0.4)
                issues.push('Agent 任务方向与组织目标不一致');
            if (ia.humanAgentGap > 0.4)
                issues.push('人与 Agent 工作方向存在分歧');
            return {
...
function deduplicateItems(items) {
    ...
    for (const item of items) {
        const isDuplicate = result.some(existing => jaccardSimilarity(existing.title, item.title) > 0.7);
        if (!isDuplicate)
            result.push(item);
    }
    return result;
 ...
 * @returns 行动方案
 */
async function generateActionPlan(diagnosis, narrative) {
...
// LLM supplement
// ====================================================================
async function generateLLMItems(diagnosis, narrative, existingItems) {
    const { chat } = await Promise.resolve().then(() => __importStar(require('../../llm-client')));
...
function parseActionItems(content, teamId) {
    try {
        ...
        return items.slice(0, 3).map((item) => ({
            ...
            sourceDimension: item.sourceDimension || 'unknown',
            title: item.title || '未命名行动',
            description: item.description || '',
            targetSystem: item.targetSystem || 'manual',
...
//# sourceMappingURL=auto-action.js.map