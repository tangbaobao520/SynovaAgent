...
exports.enrichDiagnosis = enrichDiagnosis;
...
function detectRepoPath() {
    ...
    const candidates = [
        ...
        path_1.default.join(os_1.default.homedir(), 'ClawOrg'),
        path_1.default.join(os_1.default.homedir(), 'novis'),
    ].filter(Boolean);
...
function collectLocalGit(repoPath) {
    const run = (cmd) => {
        try {
            const output = (0, child_process_1.execSync)(cmd, { cwd: repoPath, timeout: 5000, encoding: 'utf-8' });
    ...
    let avgCommitSize = 0;
    try {
        const stats = (0, child_process_1.execSync)('git log --since="30 days ago" --pretty=format: --shortstat 2>/dev/null', { cwd: repoPath, timeout: 5000, encoding: 'utf-8' });
        ...
        (0, engine_context_1.getEngineContext)().logger.debug('[diagnosis/enricher] git avgCommitSize 计算失败');
        avgCommitSize = 0;
    }
    return {
        repoPath,
...
const TOOL_TO_CATEGORY = new Map();
...
function scanInstalledSoftware() {
    ...
    if (process.platform === 'win32') {
        const programDirs = [
            ...
            path_1.default.join(os_1.default.homedir(), 'AppData', 'Local'),
            path_1.default.join(os_1.default.homedir(), 'AppData', 'Roaming'),
        ];
...
function detectIntegrationGaps(tools, categories) {
    const gaps = [];
...
async function fetchGitHubMetrics(owner, repo, token) {
    const headers = {
    ...
    try {
        ...
        const mergedPRs = prs.filter((p) => p.merged_at && p.merged_at >= thirtyDaysAgo);
        // Issues
        const issueRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues?state=all&per_page=100&since=${thirtyDaysAgo}`, { headers });
        const issues = await issueRes.json();
        const openIssues = issues.filter((i) => i.state === 'open' && !i.pull_request).length;
        const closedIssuesLast30Days = issues.filter((i) => i.state === 'closed' && !i.pull_request && i.closed_at && i.closed_at >= thirtyDaysAgo).length;
        const totalIssuesInPeriod = issues.filter((i) => !i.pull_request).length;
...
const builtinPlugins = [
    {
        ...
        fetch: async (_teamId) => {
            ...
            if (!repoPath)
                return null;
            return collectLocalGit(repoPath);
        },
    },
    {
        id: 'software',
        ...
        fetch: async (_teamId) => {
            ...
            return { installedTools: tools, categories, integrationGaps };
        },
    },
    {
        id: 'github-api',
        ...
        fetch: async (_teamId) => {
            ...
            if (!owner || !repo)
                return null;
            return fetchGitHubMetrics(owner, repo, config.token);
        },
    },
];
// ====================================================================
 ...
 */
async function enrichDiagnosis(teamId) {
