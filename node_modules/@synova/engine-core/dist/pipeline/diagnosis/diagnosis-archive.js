...
// ====================================================================
/** 归档一个诊断报告 */
function archiveDiagnosis(report, teamId, tags = []) {
...
/** 最新一次诊断 */
function getLatestDiagnosis(teamId) {
    const results = getStore().query({ teamId, order: 'desc', limit: 1 });
    return results[0] ?? null;
}
/** 清理过期归档 */
function cleanupArchive(maxAgeMs) {
...
// ====================================================================
/** 从历史归档中提取可复用知识 */
function extractKnowledge(teamId) {
    ...
    const recurringDimensions = [...dimFrequency.entries()]
        ...
        .map(([dim]) => dim);
    // 持久性模式：总结高频维度
    const persistentPatterns = recurringDimensions.map(dim => `「${dim}」在最近 ${history.length} 次诊断中出现 ${dimFrequency.get(dim)} 次，属于持续性挑战`);
    // 趋势对比（最新 vs 最早）
...
//# sourceMappingURL=diagnosis-archive.js.map