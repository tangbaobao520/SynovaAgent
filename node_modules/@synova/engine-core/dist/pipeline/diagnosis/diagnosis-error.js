...
exports.NON_RECOVERABLE_CODES = exports.RECOVERABLE_CODES = exports.RecoveryExhaustedError = exports.EvidencePoolError = exports.SessionCompactionError = exports.ModuleExecError = exports.LLMCallError = exports.PhaseExecError = exports.DiagnosticAgentError = void 0;
exports.isRetryable = isRetryable;
exports.isDegraded = isDegraded;
exports.getFailurePhase = getFailurePhase;
exports.phaseExecFailed = phaseExecFailed;
exports.llmCallFailed = llmCallFailed;
exports.moduleFailedDegraded = moduleFailedDegraded;
exports.moduleFailedHard = moduleFailedHard;
exports.sessionCompactionFailed = sessionCompactionFailed;
exports.evidencePoolOverflow = evidencePoolOverflow;
exports.evidencePoolCorrupted = evidencePoolCorrupted;
exports.recoveryExhausted = recoveryExhausted;
exports.normalizeDiagnosisError = normalizeDiagnosisError;
...
class DiagnosticAgentError extends Error {
    ...
    cause;
    constructor(code, message, phase, retryable, cause) {
        super(message);
...
exports.DiagnosticAgentError = DiagnosticAgentError;
// ============================================================================
 ...
 */
class PhaseExecError extends DiagnosticAgentError {
    /** Which module within the phase triggered the failure. */
    moduleName;
    /** Human-readable description of what the phase was trying to do. */
    phaseDescription;
    constructor(phase, moduleName, phaseDescription, cause) {
        super(types_1.DiagnosisErrorCode.MODULE_FAILED, `Phase ${phase} execution failed in module "${moduleName}": ${phaseDescription}`, phase, 
...
exports.PhaseExecError = PhaseExecError;
/**
 ...
 */
class LLMCallError extends DiagnosticAgentError {
    ...
    requestId;
    constructor(provider, model, httpStatus, retryable, suggestedAction, body, requestId, cause) {
        const snippet = truncateBodySnippet(body ?? '', 500);
        const message = `LLM call failed: ${provider}/${model} returned HTTP ${httpStatus}`
            + (requestId ? ` [trace ${requestId}]` : '')
            + (snippet ? ` — ${snippet}` : '');
        super(httpStatus === 429 || (httpStatus >= 500 && httpStatus < 600)
            ? types_1.DiagnosisErrorCode.LLM_TIMEOUT // 429 / 5xx → recoverable timeout class
            : httpStatus === 401 || httpStatus === 403
                ? types_1.DiagnosisErrorCode.PERMISSION_DENIED
                : types_1.DiagnosisErrorCode.MODULE_FAILED, message, 
 ...
 */
class ModuleExecError extends DiagnosticAgentError {
    /** The module that failed (matches {@link ModuleRegistry} module names). */
    moduleName;
    /** Whether this failure allows degraded continuation. */
    degraded;
    constructor(moduleName, message, degraded, cause) {
        super(types_1.DiagnosisErrorCode.MODULE_FAILED, `Module "${moduleName}" failed: ${message}`, 
...
exports.ModuleExecError = ModuleExecError;
 ...
 */
class SessionCompactionError extends DiagnosticAgentError {
    /** Estimated total tokens before compaction was attempted. */
    estimatedTokens;
    /** The context window limit (in tokens) of the target model. */
    contextWindowTokens;
    constructor(estimatedTokens, contextWindowTokens, cause) {
        super(types_1.DiagnosisErrorCode.LLM_TIMEOUT, // context window failures reuse LLM_TIMEOUT
...
exports.SessionCompactionError = SessionCompactionError;
 ...
 */
class EvidencePoolError extends DiagnosticAgentError {
    ...
    corruptedKey;
    constructor(type, itemCount, message, maxCapacity, corruptedKey, cause) {
        // Overflow is recoverable (trim evidence); corruption is not (data is lost).
        const recoverable = type === 'overflow';
        super(type === 'corruption'
            ? types_1.DiagnosisErrorCode.SESSION_CORRUPTED
            : types_1.DiagnosisErrorCode.EVIDENCE_INSUFFICIENT, message, 
...
exports.EvidencePoolError = EvidencePoolError;
 ...
 */
class RecoveryExhaustedError extends DiagnosticAgentError {
    ...
    lastError;
    constructor(scenario, attempts, lastError) {
        super(types_1.DiagnosisErrorCode.RECOVERY_EXHAUSTED, `Recovery exhausted for "${scenario}" after ${attempts} attempt${attempts > 1 ? 's' : ''}: ${lastError.message}`, 
        /* phase */ -1, 
...
exports.RecoveryExhaustedError = RecoveryExhaustedError;
// ============================================================================
 ...
 */
function phaseExecFailed(phase, module, description, cause) {
 ...
 */
function llmCallFailed(provider, model, status, body, requestId, cause) {
    const { retryable, suggestedAction } = classifyLLMStatus(status, body ?? '');
    return new LLMCallError(provider, model, status, retryable, suggestedAction, body, requestId, cause);
}
/**
 ...
 */
function moduleFailedDegraded(moduleName, message, cause) {
 ...
 */
function moduleFailedHard(moduleName, message, cause) {
 ...
 */
function sessionCompactionFailed(estimatedTokens, contextWindowTokens, cause) {
 ...
 */
function evidencePoolOverflow(itemCount, maxCapacity) {
    return new EvidencePoolError('overflow', itemCount, `Evidence pool overflow: ${itemCount} items exceed capacity of ${maxCapacity}`, maxCapacity, undefined);
}
/**
 * Create an {@link EvidencePoolError} for evidence corruption.
 */
function evidencePoolCorrupted(itemCount, corruptedKey, cause) {
    return new EvidencePoolError('corruption', itemCount, `Evidence pool corruption: item "${corruptedKey}" is invalid or unreadable`, undefined, corruptedKey, cause);
}
/**
 * Create a {@link RecoveryExhaustedError} when all recovery attempts fail.
 */
function recoveryExhausted(scenario, attempts, lastError) {
    return new RecoveryExhaustedError(scenario, attempts, lastError);
}
/** Error codes that are considered retryable/recoverable. */
const RECOVERABLE_CODES = new Set([
    types_1.DiagnosisErrorCode.LLM_TIMEOUT,
    types_1.DiagnosisErrorCode.TOOL_TIMEOUT,
    types_1.DiagnosisErrorCode.MODULE_FAILED,
    types_1.DiagnosisErrorCode.EVIDENCE_INSUFFICIENT,
    types_1.DiagnosisErrorCode.GATE_CHECK_FAILED,
]);
exports.RECOVERABLE_CODES = RECOVERABLE_CODES;
/** Error codes that are never retryable. */
const NON_RECOVERABLE_CODES = new Set([
    types_1.DiagnosisErrorCode.SESSION_CORRUPTED,
    types_1.DiagnosisErrorCode.PERMISSION_DENIED,
    types_1.DiagnosisErrorCode.RECOVERY_EXHAUSTED,
    types_1.DiagnosisErrorCode.SUBAGENT_LOST,
]);
exports.NON_RECOVERABLE_CODES = NON_RECOVERABLE_CODES;
/**
 ...
 */
const PATTERN_MAP = [
    { pattern: /time\s*out|timed?\s*out|ETIMEDOUT|ECONNABORTED|abort/i, code: types_1.DiagnosisErrorCode.LLM_TIMEOUT },
    { pattern: /rate.?limit|429|too many requests/i, code: types_1.DiagnosisErrorCode.LLM_TIMEOUT },
    { pattern: /permission.?denied|unauthorized|forbidden|401|403/i, code: types_1.DiagnosisErrorCode.PERMISSION_DENIED },
    { pattern: /module.?fail|computation.?fail/i, code: types_1.DiagnosisErrorCode.MODULE_FAILED },
    { pattern: /tool.?timeout|tool.?fail/i, code: types_1.DiagnosisErrorCode.TOOL_TIMEOUT },
    { pattern: /session.?corrupt|deserializ|JSON\.parse|invalid state/i, code: types_1.DiagnosisErrorCode.SESSION_CORRUPTED },
    { pattern: /recovery.?exhausted|max.?retry|max.?attempt/i, code: types_1.DiagnosisErrorCode.RECOVERY_EXHAUSTED },
    { pattern: /evidence.?insufficient|not enough data|empty result/i, code: types_1.DiagnosisErrorCode.EVIDENCE_INSUFFICIENT },
    { pattern: /gate.?check.?fail|data.?completeness/i, code: types_1.DiagnosisErrorCode.GATE_CHECK_FAILED },
    { pattern: /sub.?agent.?lost|orphaned|subagent/i, code: types_1.DiagnosisErrorCode.SUBAGENT_LOST },
    { pattern: /context.?window|token.?limit|maximum context/i, code: types_1.DiagnosisErrorCode.LLM_TIMEOUT },
    { pattern: /network|ECONNREFUSED|ENOTFOUND|fetch failed/i, code: types_1.DiagnosisErrorCode.LLM_TIMEOUT },
];
/**
 ...
 */
function normalizeDiagnosisError(err) {
    ...
    return {
        code: types_1.DiagnosisErrorCode.MODULE_FAILED,
        message,
        recoverable: false,
    };
}
 ...
 */
function tryMatchEnum(message) {
    const values = Object.values(types_1.DiagnosisErrorCode);
    // Exact match
 ...
 */
function truncateBodySnippet(body, maxChars) {
 ...
 */
function classifyLLMStatus(status, body) {
    ...
    if (status === 400 || status === 413 || status === 422) {
        if (lowerBody.includes('context window') ||
...
//# sourceMappingURL=diagnosis-error.js.map