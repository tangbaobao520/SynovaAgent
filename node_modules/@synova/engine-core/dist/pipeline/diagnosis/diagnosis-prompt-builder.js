...
exports.DiagnosisPromptBuilder = void 0;
exports.createScopePromptBuilder = createScopePromptBuilder;
exports.createHypothesisPromptBuilder = createHypothesisPromptBuilder;
// ====================================================================
// 阶段定义
// ====================================================================
...
class DiagnosisPromptBuilder {
    ...
    renderSkillCards() {
        ...
        for (const card of this.skillCards) {
            lines.push(`## ${card.name}`, `- 描述: ${card.description}`, `- 使用时机: ${card.whenToUse}`, `- 输入: ${card.inputFormat}`, `- 输出: ${card.outputFormat}`);
        }
        return lines.join('\n');
    }
...
exports.DiagnosisPromptBuilder = DiagnosisPromptBuilder;
/** 快速创建 Phase 0 界定阶段 builder */
function createScopePromptBuilder(posture, postureLabel) {
    const builder = new DiagnosisPromptBuilder()
        ...
        .addSection({
        priority: 20,
        content: '六缝隙维度包括：决策质量、信息流通、知识共享、信任水平、目标对齐、角色清晰度。',
        source: 'domain',
    });
...
function createHypothesisPromptBuilder(posture, postureLabel) {
    const builder = new DiagnosisPromptBuilder()
        .withPhase(2)
        .addSection({
        priority: 10,
        content: '基于以下证据池，生成 3-5 个诊断假设。\n\n' +
 ...
 */
function createPostureConstitution(posture, postureLabel) {
...
/** 姿态 → 分析框架映射 */
const POSTURE_ANALYSIS_FRAMEWORKS = {
    moat_builder: {
        framework: [
...
//# sourceMappingURL=diagnosis-prompt-builder.js.map