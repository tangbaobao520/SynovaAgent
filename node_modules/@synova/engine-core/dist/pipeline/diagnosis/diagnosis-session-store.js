...
const log = (0, logger_1.createLogger)('diagnosis/session-store');
// ====================================================================
// Config
// ====================================================================
...
function initSessionStore(dir, database) {
    dataDir = dir;
...
function getSession(sessionId) {
    if (db) {
        return db.prepare('SELECT * FROM diagnosis_sessions WHERE session_id = ?').get(sessionId);
    }
    return null;
}
// ====================================================================
...
// ====================================================================
function appendMessage(sessionId, orgId, message) {
    const now = new Date().toISOString();
    const fullMsg = { ...message, timestamp: now };
...
function rotateIfNeeded(file) {
    try {
        ...
        if (stat.size < ROTATE_AFTER_BYTES)
            return;
...
// ====================================================================
function compactSession(orgId, sessionId, compactionSummary) {
    // 标记当前会话为 compacted
    if (db) {
    ...
    log.info({ parentSessionId: sessionId, childSessionId: childSession.sessionId }, '[session-store] 会话压缩完成，子会话已创建');
    return childSession.sessionId;
}
// ====================================================================
// Search (对标 Hermes FTS5 + Claw-Code JSONL grep)
// ====================================================================
function searchSessions(orgId, query, limit = 10) {
    const results = [];
    // SQLite FTS5（对标 Hermes）
    if (db) {
        const rows = db.prepare(`SELECT session_id, content, phase, rank FROM session_messages_fts WHERE session_messages_fts MATCH ? AND session_id IN
       (SELECT session_id FROM diagnosis_sessions WHERE org_id = ?) ORDER BY rank LIMIT ?`).all(query, orgId, limit);
...
// ====================================================================
function loadSessionMessages(orgId, sessionId) {
    const file = sessionPath(orgId, sessionId);
...
//# sourceMappingURL=diagnosis-session-store.js.map