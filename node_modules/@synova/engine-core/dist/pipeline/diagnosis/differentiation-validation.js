...
exports.validateDifferentiation = validateDifferentiation;
exports.refineWithLLM = refineWithLLM;
 ...
 */
function validateDifferentiation(params) {
 ...
 */
async function refineWithLLM(result, llmJudge) {
...
// Helpers
// ====================================================================
function extractPerceivedKeywords(responses) {
    const freq = new Map();
 ...
 */
function computeCJKMOverlap(a, b) {
...
function checkOrgCapability(claimed, snapshot) {
    if (!snapshot) {
        return {
            relevantDimensions: [],
            avgScore: 0,
    ...
    for (const [keyword, dims] of Object.entries(CLAIM_TO_GAPS)) {
        if (claimed.includes(keyword)) {
            for (const d of dims) {
                if (!relevantDimensions.includes(d))
                    relevantDimensions.push(d);
            }
        }
    }
...
function buildInterpretation(verdict, ctx) {
...
//# sourceMappingURL=differentiation-validation.js.map