...
exports.EvidenceManager = void 0;
...
class EvidenceManager {
    ...
    /** 将证据同步到本体图 */
    syncEvidenceToGraph(ev, isSuperseded) {
        if (!this.graphStore)
            return;
        const graph = ev.orgId;
        if (!graph) {
            log.warn({ evidenceId: ev.id }, '[evidence-manager] 证据缺少 orgId，跳过本体图同步');
            return;
        }
        // Evidence → Event 节点
        const nodeId = this.graphStore.createNode(sog_core_schema_1.SOGNodeType.EVENT, {
        ...
        const dimNodes = this.graphStore.queryNodes(sog_core_schema_1.SOGNodeType.TEAM, { name: ev.dimension }, graph);
        if (dimNodes.length === 0) {
            this.graphStore.createNode(sog_core_schema_1.SOGNodeType.TEAM, { name: ev.dimension, type: 'dimension' }, graph);
        }
        // Superseded: mark old evidence as superseded
        if (isSuperseded && ev.supersededBy) {
            this.graphStore.createEdge(sog_core_schema_1.SOGEdgeType.CORRESPONDS_TO, nodeId, ev.supersededBy, 0.3, { type: 'superseded' }, graph);
            // Mark the superseding relationship
            this.graphStore.createEdge(sog_core_schema_1.SOGEdgeType.CORRESPONDS_TO, ev.supersededBy, nodeId, 0.8, { type: 'supersedes' }, graph);
        }
    }
    /** 批量添加 */
    addAll(evidenceList) {
