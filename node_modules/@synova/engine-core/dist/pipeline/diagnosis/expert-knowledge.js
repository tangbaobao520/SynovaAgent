...
exports.initExpertKnowledge = initExpertKnowledge;
exports.getExpertKnowledge = getExpertKnowledge;
exports.renderKnowledgeForSystemPrompt = renderKnowledgeForSystemPrompt;
exports.addExpertKnowledge = addExpertKnowledge;
exports.clearKnowledgeStore = clearKnowledgeStore;
...
function initExpertKnowledge() {
    ...
    for (const [type, entries] of Object.entries(SEED_KNOWLEDGE)) {
        ...
        knowledgeStore.set(type, list);
    }
}
// ═══ CRUD ═══
function getExpertKnowledge(type) {
    initExpertKnowledge();
    return knowledgeStore.get(type) || [];
}
function renderKnowledgeForSystemPrompt(type) {
...
function addExpertKnowledge(type, entry) {
    initExpertKnowledge();
    const now = new Date().toISOString();
    const full = { ...entry, id: `kn_${++knowledgeCounter}`, addedAt: now, lastUpdatedAt: now };
    if (!knowledgeStore.has(type))
        knowledgeStore.set(type, []);
    knowledgeStore.get(type).push(full);
    return full;
}
function clearKnowledgeStore() {
    knowledgeStore.clear();
    initialized = false;
    knowledgeCounter = 0;
}
//# sourceMappingURL=expert-knowledge.js.map