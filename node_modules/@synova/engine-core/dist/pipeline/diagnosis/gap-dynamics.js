...
exports.computeDynamics = computeDynamics;
 ...
 */
function computeDynamics(teamId) {
    const snapshots = (0, gap_recorder_1.getGapTimeline)(teamId);
...
// ====================================================================
function extractDimensions(snapshots) {
    const firstGaps = snapshots[0]?.gaps;
    if (!firstGaps)
        return [];
    return Object.keys(firstGaps);
}
// ====================================================================
// Internal: velocity = Δscore / Δtime (normalized)
// ====================================================================
function computeVelocity(snapshots, dimensions) {
    const result = {};
...
// ====================================================================
function computeAcceleration(snapshots, dimensions) {
    const result = {};
    for (const dim of dimensions) {
...
// ====================================================================
function computePhaseCoupling(snapshots, dimensions) {
    const couplings = [];
...
// ====================================================================
function computeStickyDimensions(snapshots, dimensions) {
    const result = [];
    const firstTime = new Date(snapshots[0].observedAt).getTime();
    const lastTime = new Date(snapshots[snapshots.length - 1].observedAt).getTime();
...
// ====================================================================
function computeOverallChangeRate(velocity) {
    const absVelocities = Object.values(velocity).map(Math.abs);
    if (absVelocities.length === 0)
        return 0;
...
function computeAvgSnapshotInterval(snapshots) {
...
//# sourceMappingURL=gap-dynamics.js.map