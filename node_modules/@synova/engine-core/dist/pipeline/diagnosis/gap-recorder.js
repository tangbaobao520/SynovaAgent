...
exports.GAP_DIMENSIONS = void 0;
exports.buildGapSnapshot = buildGapSnapshot;
exports.recordGapSnapshot = recordGapSnapshot;
exports.getGapTimeline = getGapTimeline;
exports.getLatestSnapshot = getLatestSnapshot;
exports.getSnapshotCount = getSnapshotCount;
exports.clearTeamSnapshots = clearTeamSnapshots;
exports.resetAllSnapshots = resetAllSnapshots;
exports.recordLegacyGapSnapshot = recordLegacyGapSnapshot;
...
function ensureRecovered() {
    if (_recovered)
        return;
    _recovered = true;
    const persisted = (0, persistence_1.loadAllTimelines)();
 ...
 */
function extractGapMode(mode, dim) {
    switch (dim) {
 ...
 */
function computeEngineScore(mode, dim) {
    switch (dim) {
        case 'division_of_labor': {
            let score = 0.5;
            const dol = mode.divisionOfLabor;
            if (dol.mode && dol.mode !== 'flexible')
                score += 0.2;
            if (dol.roleAssignment && Object.keys(dol.roleAssignment).length > 0)
                score += 0.15;
            if (dol.fallbackRoles && Object.keys(dol.fallbackRoles).length > 0)
                score += 0.15;
        ...
        case 'information_flow': {
            ...
            if (flow.syncMode)
                score += 0.15;
            if (flow.visibilityMatrix && Object.keys(flow.visibilityMatrix).length > 0)
                score += 0.1;
            if (flow.routingMap && Object.keys(flow.routingMap).length > 0)
                score += 0.1;
            return Math.min(score, 1.0);
        }
        case 'authority_governance': {
            ...
            if (ag.decisionFlow)
                score += 0.1;
            if (ag.vetoRoles && ag.vetoRoles.length > 0)
                score += 0.1;
            if (ag.escalationPath && ag.escalationPath.length > 0)
                score += 0.05;
            return Math.min(score, 1.0);
        }
        case 'trust_incentive': {
            ...
            if (ti.alignment)
                score += 0.15;
            if (ti.initialTrust && ti.initialTrust !== 'medium')
                score += 0.1;
            if (ti.degradationTriggers && ti.degradationTriggers.length > 0)
                score += 0.15;
            if (ti.successSignal && ti.failureSignal)
                score += 0.1;
            return Math.min(score, 1.0);
        }
        case 'knowledge_sharing': {
            ...
            if (ks.strategy)
                score += 0.2;
            if (ks.syncIntervalHours > 0)
                score += 0.15;
            if (ks.hasTacitKnowledge !== undefined)
                score += 0.15;
            return Math.min(score, 1.0);
        }
        case 'external_interface': {
            ...
            if (ei.strategy)
                score += 0.2;
            if (ei.authorizedRoles && ei.authorizedRoles.length > 0)
                score += 0.15;
            if (ei.auditLogEnabled)
                score += 0.15;
            return Math.min(score, 1.0);
        }
        default:
...
function deriveConfidence(engineScore) {
    if (engineScore >= 0.75)
        return 'high';
    if (engineScore >= 0.5)
        return 'medium';
 ...
 */
function buildSourceBreakdown(mode, dim, phaseATeamSize, phaseBRoleCount) {
    const breakdown = {};
    ...
    switch (dim) {
        case 'information_flow':
            if (mode.informationFlow.syncMode)
                breakdown['sync_mode'] = 0.15;
            if (mode.informationFlow.visibilityMatrix)
                breakdown['visibility_matrix'] = 0.1;
            break;
        case 'authority_governance':
            if (mode.authorityGovernance.decisionFlow)
                breakdown['decision_flow'] = 0.15;
            if (mode.authorityGovernance.vetoRoles?.length)
                breakdown['veto_config'] = 0.1;
            break;
        case 'knowledge_sharing':
            if (mode.knowledgeSharing.syncIntervalHours > 0)
                breakdown['sync_interval'] = 0.15;
            if (mode.knowledgeSharing.hasTacitKnowledge)
                breakdown['tacit_knowledge'] = 0.1;
            break;
        default:
 ...
 */
function buildGapSnapshot(teamId, taskDef, phaseAResult, phaseBResult, phaseCResult) {
    const mode = phaseCResult.collaborationMode;
    const gaps = {};
    for (const dim of exports.GAP_DIMENSIONS) {
        const engineScore = computeEngineScore(mode, dim);
        gaps[dim] = {
            mode: extractGapMode(mode, dim),
            engineScore,
            confidence: deriveConfidence(engineScore),
            sourceBreakdown: buildSourceBreakdown(mode, dim, phaseAResult.teamStructure.totalRoles, phaseBResult.personaGenomes.length),
        };
    }
    return {
        teamId,
 ...
 */
function recordGapSnapshot(snapshot) {
    ensureRecovered();
 ...
 */
function getLatestSnapshot(teamId) {
    ensureRecovered();
    const entries = timeline.get(teamId);
 ...
 */
function getSnapshotCount(teamId) {
    ensureRecovered();
 ...
 */
function clearTeamSnapshots(teamId) {
...
function computeLegacyGapScore(protocolConfig, dim) {
    ...
    switch (dim) {
        ...
        case 'trust_incentive': {
            ...
            if (ia?.alignment)
                score += 0.15;
            if (tm?.initialTrust && tm.initialTrust !== 'medium')
                score += 0.1;
            if (ia?.successSignal && ia?.failureSignal)
                score += 0.1;
            return Math.min(score, 1.0);
        }
        case 'knowledge_sharing': {
...
function extractLegacyGapMode(protocolConfig, dim) {
    const pc = protocolConfig;
    switch (dim) {
        case 'division_of_labor':
 ...
 */
function recordLegacyGapSnapshot(teamId, taskDef, buildResult) {
...
//# sourceMappingURL=gap-recorder.js.map