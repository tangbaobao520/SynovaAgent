...
exports.computeHTM = computeHTM;
const engine_context_1 = require("../../engine-context");
const collaboration_collector_1 = require("../collaboration-collector");
const logger_1 = require("../../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/pipeline/diagnosis/htm');
// ====================================================================
// DB access helper
// ====================================================================
function getDb() {
    try {
        return (0, engine_context_1.getEngineContext)().database.getDb();
    }
    catch {
        log.debug('[htm] database not available via engine context');
        return null;
    }
}
// ====================================================================
 ...
 */
function computeHTM(teamId) {
    const logger = (0, engine_context_1.getEngineContext)().logger;
    const db = getDb();
    ...
    const trustHealthScore = Math.round((autoAcceptRate * 0.4 + (1 - escalationRate) * 0.3 + (1 - deadlockRate) * 0.2 + agentAgentHealth * 0.1) * 100) / 100;
    ...
    return {
        ...
        decayEvents,
        singlePointRisks,
        interpretation: buildInterpretation(trustHealthScore, autoAcceptRate, escalationRate, decayEvents, singlePointRisks),
    };
}
// ====================================================================
// Internal: Trust curves
// ====================================================================
function computeTrustCurves(db, teamId, logger) {
    const curves = [];
    try {
        if (!db)
            return curves;
...
// ====================================================================
function computeAgentAgentHealth(db, teamId, logger) {
    try {
        if (!db)
...
// ====================================================================
function detectTrustDecay(db, teamId, logger) {
    const events = [];
    try {
        if (!db)
            return events;
    ...
    `).all();
        if (rows.length < 5)
            return events;
        const avgCorrectionRate = rows.reduce((s, r) => s + (r.total > 0 ? r.corrections / r.total : 0), 0) / rows.length;
        for (const row of rows) {
            if (row.total < 3)
                continue;
...
// ====================================================================
function detectSinglePointRisks(db, teamId, logger) {
    const risks = [];
    try {
        if (!db)
            return risks;
...
// ====================================================================
function buildInterpretation(trustScore, autoAcceptRate, escalationRate, decayEvents, singlePointRisks) {
    const parts = [];
