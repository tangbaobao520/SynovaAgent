...
function createInterviewProject(input) {
    const id = generateId('ivp');
    const defaultChannels = ['one_on_one', 'anonymous_survey', 'document_review'];
    const project = {
        id,
        teamId: input.teamId,
        name: input.name,
...
// ====================================================================
function getIntervieweeProfile(projectId, intervieweeId) {
...
function getRecommendedDimensions(role) {
    switch (role) {
...
function getSensitivityConstraints(_role) {
...
// ====================================================================
function generateQuestionnaire(input) {
...
function getQuestionTemplates(role) {
    const common = [
...
function updateInterviewSession(input) {
    const project = projectStore.get(input.projectId);
    if (!project)
        return null;
...
function distributeSurvey(projectId, baseUrl = '/platform/survey') {
    const project = projectStore.get(projectId);
    if (!project)
        return null;
    // Find interviewees with anonymous_survey channel who haven't completed
    const candidates = project.interviewees.filter(i => i.channels.includes('anonymous_survey') && i.status !== 'completed' && i.status !== 'declined');
...
// ====================================================================
function aggregateFindings(input) {
    const project = projectStore.get(input.projectId);
    if (!project)
        return [];
    ...
    if (completedInterviewees.length === 0)
        return [];
    return dimensions.map(dim => {
        ...
        for (const person of completedInterviewees) {
            ...
            totalWeight += w;
            if (!roleContributions[person.role])
                roleContributions[person.role] = 0;
...
// Utility
// ====================================================================
function addInterviewee(projectId, input) {
    const project = projectStore.get(projectId);
    if (!project)
        return null;
    if (project.interviewees.length >= project.scope.maxInterviewees)
        return null;
    const person = {
        id: generateId('ivw'),
        name: input.name,
...
function getRecommendedChannels(role) {
    switch (role) {
...
/** 根据团队规模推荐通道组合 */
function recommendChannels(teamSize) {
    if (teamSize <= 5)
        return CHANNEL_RECOMMENDATIONS['1-5'];
    if (teamSize <= 20)
        return CHANNEL_RECOMMENDATIONS['6-20'];
    if (teamSize <= 50)
        return CHANNEL_RECOMMENDATIONS['21-50'];
...
//# sourceMappingURL=interview-project-manager.js.map