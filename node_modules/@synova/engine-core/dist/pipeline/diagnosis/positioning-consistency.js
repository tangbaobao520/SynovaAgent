...
exports.computePositioningConsistency = computePositioningConsistency;
 ...
 */
function computePositioningConsistency(params) {
    ...
    if (externalClaims.length === 0 || internalDescriptions.length === 0 || customerDescriptions.length === 0) {
        log.info('[positioning-consistency] 三方数据不完整（对外%d/内部%d/客户%d），跳过', externalClaims.length, internalDescriptions.length, customerDescriptions.length);
        return null;
    }
    ...
    // ── 4. Identify gaps ──
    const gaps = identifyGaps(externalInternalAlignment, externalCustomerAlignment, internalCustomerAlignment, { externalKeywords, internalKeywords, customerKeywords });
    // ── 5. Interpretation ──
...
function extractPositioningTerms(responses) {
    const freq = new Map();
    ...
    const globalBigramTokens = new Map();
    for (const resp of responses) {
        ...
        for (const token of tokens) {
            if (token.length < 2)
                continue;
            if (STOP_POSITIONING_WORDS.has(token))
                continue;
 ...
 */
function computeAlignment(textA, textB) {
...
function extractCharUnigrams(text) {
    const clean = text
        .replace(/[，。！？、；：""''（）…【】《》\s]+/g, '')
        .trim();
    if (clean.length === 0)
...
function extractCharBigrams(text) {
...
function identifyGaps(ei, ec, ic, keywords) {
    const gaps = [];
    ...
    for (const { pair, score, ours, theirs, ourLabel, theirLabel } of pairs) {
        if (score >= STRONG_THRESHOLD)
            continue;
        const severity = score < BROKEN_THRESHOLD ? 'critical' : score < STRONG_THRESHOLD ? 'moderate' : 'minor';
...
function buildInterpretation(alignment, gaps, ctx) {
...
//# sourceMappingURL=positioning-consistency.js.map