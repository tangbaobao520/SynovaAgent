...
exports.recordSelfAssessment = recordSelfAssessment;
exports.getSelfAssessments = getSelfAssessments;
exports.clearTeamSelfAssessments = clearTeamSelfAssessments;
exports.computeSelfAwareness = computeSelfAwareness;
...
function recoverTeamIfNeeded(teamId) {
    if (selfAssessments.has(teamId))
        return;
    const data = (0, persistence_1.loadSelfAssessments)(teamId);
    if (data.length > 0) {
 ...
 */
function recordSelfAssessment(teamId, dimension, score) {
    const record = {
        teamId,
        dimension,
    ...
    (0, persistence_1.saveSelfAssessment)(record);
    return record;
}
/**
 * Get all self-assessment records for a team.
 */
function getSelfAssessments(teamId) {
    return [...(selfAssessments.get(teamId) ?? [])];
}
/**
 * Clear all self-assessments for a team.
 */
function clearTeamSelfAssessments(teamId) {
 ...
 */
function computeSelfAwareness(teamId) {
    const snapshot = (0, gap_recorder_1.getLatestSnapshot)(teamId);
    ...
    const dims = Object.keys(snapshot.gaps);
    const deltas = [];
    ...
    const overallGap = validDeltas.length > 0
        ? validDeltas.reduce((sum, d) => sum + Math.abs(d.delta), 0) / validDeltas.length
        : 0;
    const significantDeltas = deltas.filter((d) => d.delta !== null && Math.abs(d.delta) > SIGNIFICANT_DELTA_THRESHOLD);
    return {
        deltas,
...
function interpretDelta(dim, delta, engineScore, humanScore, sampleCount) {
...
function buildOverallInterpretation(overallGap, significantDeltas, allDeltas) {
    ...
    const dimNames = significantDeltas.map((d) => DIM_LABELS[d.dimension] ?? d.dimension);
