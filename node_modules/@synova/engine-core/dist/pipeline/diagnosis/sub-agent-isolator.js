...
exports.SubAgentIsolator = void 0;
const diagnosis_prompt_builder_1 = require("./diagnosis-prompt-builder");
const diagnosis_error_1 = require("./diagnosis-error");
...
class SubAgentIsolator {
    ...
    constructor(llmClient) {
        ...
        this.promptBuilder = new diagnosis_prompt_builder_1.DiagnosisPromptBuilder();
     ...
     * 独立 LLM 会话，超时隔离，自动重试。
     */
    async runAgent(type, context, options = {}) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                const result = await this.executeWithTimeout(type, systemPrompt, userMessage, timeoutMs, controller.signal);
                ...
                const normalized = (0, diagnosis_error_1.normalizeDiagnosisError)(err);
                const isRecoverable = diagnosis_error_1.RECOVERABLE_CODES.has(normalized.code);
     ...
     */
    async runAgents(types, context, options = {}) {
        const results = await Promise.allSettled(types.map(type => this.runAgent(type, context, options)));
    ...
    cancelAll() {
        ...
        if (this.tracer && count > 0) {
            this.tracer.trace({
                type: 'error',
                code: types_1.DiagnosisErrorCode.SUBAGENT_LOST,
    ...
    // ── Internal ──
    buildPrompt(type, role, context) {
        const builder = new diagnosis_prompt_builder_1.DiagnosisPromptBuilder()
    ...
    async executeWithTimeout(type, systemPrompt, userMessage, timeoutMs, signal) {
        let timer = null;
...
exports.SubAgentIsolator = SubAgentIsolator;
//# sourceMappingURL=sub-agent-isolator.js.map