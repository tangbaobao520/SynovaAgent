...
exports.generateDialogueQualityBrief = generateDialogueQualityBrief;
exports.assessIFPrecision = assessIFPrecision;
/**
 * S0: 生成 L0 对话质量简报
 *
 * 纯文本模式匹配，不调用额外 LLM。
 * 宪法安全：只在对话文本层面采集，零 I/O 零网络。
 */
function generateDialogueQualityBrief(sessionId, conversationHistory) {
    ...
    // 整体质量判定
    const overallQuality = confusionCount >= 2 ? 'low' :
 ...
 * 返回结构化精度分数和改进建议。
 */
function assessIFPrecision(taskDef) {
    const evalField = (text, fieldType) => {
        ...
        if (fieldType === 'job')
            return (hasNums || hasPlatform) && text.length > 15 ? 1 : text.length > 8 ? 0.5 : 0;
        let s = 0;
        if (hasNums)
            s++;
        if (hasPlatform)
    ...
    let suggestion;
    if (expressedCount <= 2)
        suggestion = '信息太少，建议追问: 行业/品类、团队规模或预算、目标市场';
    else if (unexpressedGaps.length > 0)
        suggestion = `仍需明确: ${unexpressedGaps.join('、')}`;
    else
        suggestion = '信息充足，可以进入孵化确认';
    return {
