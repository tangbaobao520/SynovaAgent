...
exports.SEED_FRAMEWORKS = void 0;
exports.getFrameworkLibrary = getFrameworkLibrary;
exports.getCategoryForFrameworkId = getCategoryForFrameworkId;
exports.matchFrameworksByConstraint = matchFrameworksByConstraint;
exports.SEED_FRAMEWORKS = [
    // ═══════════════════ 原始30个种子 ═══════════════════
    // ─── 心理学 ──────────
    {
        id: 'incentive_bias',
        ...
        skillPatterns: [
            { name: '激励方案设计', summary: '基于激励偏差原理设计奖惩机制，对齐利益驱动行为', category: '组织设计', tags: ['激励', '绩效', '薪酬'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '绩效指标工程', summary: '设计KPI/OKR体系，避免激励扭曲和指标博弈', category: '管理工具', tags: ['KPI', 'OKR', '考核'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    {
    ...
    // ─── 经济学/博弈论 ────
    {
        ...
        skillPatterns: [
            { name: '供应商尽职调查', summary: '基于信息不对称理论，系统评估供应商的真实能力和风险', category: '采购管理', tags: ['供应商', '尽调', '风险'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '信息验证工作流', summary: '建立多层验证机制，降低信息不对称带来的质量风险', category: '质量管理', tags: ['验证', '质量', '审核'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    {
        id: 'principal_agent',
        ...
        skillPatterns: [
            { name: '代理成本评估', summary: '基于代理人理论评估委托-代理结构中的利益偏离和监控成本', category: '组织设计', tags: ['代理', '成本', '治理'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '委托代理合规框架', summary: '设计监督机制和激励兼容契约，防范代理人道德风险', category: '项目管理', tags: ['合规', '监督', '契约'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    {
        id: 'opportunity_cost',
    ...
    // ─── 数学/工程学 ──────
    {
        ...
        skillPatterns: [
            { name: '安全库存规划', summary: '基于安全边际原理计算最优库存缓冲，平衡成本与断货风险', category: '供应链管理', tags: ['库存', '安全', '缓冲'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '质量冗余设计', summary: '在质量标准和交付时间中嵌入安全因子，降低缺陷率', category: '质量控制', tags: ['质量', '冗余', '标准'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    {
        id: 'compounding',
    ...
    },
    {
        ...
        skillPatterns: [
            { name: '数据驱动迭代框架', summary: '基于反馈回路原理建立"度量→分析→改进"的持续优化循环', category: '产品管理', tags: ['迭代', '数据', '优化'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '质量反馈闭环', summary: '将客户反馈系统性接入质检和产品设计流程，形成修正链路', category: '运营管理', tags: ['反馈', '质量', '闭环'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    {
        id: 'redundancy',
    ...
    },
    {
        ...
        skillPatterns: [
            { name: 'ABC分类管理', summary: '基于帕累托原则将管理对象按重要性分为A/B/C三级，差异化投入资源', category: '运营管理', tags: ['分类', '优先级', '效率'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '优先级矩阵工具', summary: '用影响度-投入度矩阵筛选关键少数事务，拒绝低价值消耗', category: '策略规划', tags: ['优先级', '矩阵', '决策'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    // ─── 医学/风险 ──────
    {
    ...
    },
    {
        ...
        skillPatterns: [
            { name: '紧急度分级系统', summary: '基于分诊原则建立四级紧急度矩阵（危重/紧急/一般/可延迟），优化资源分配', category: '运营管理', tags: ['分级', '紧急', '资源'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '资源调度框架', summary: '按可治疗性×紧急度双维度排序任务，确保有限资源聚焦高价值事项', category: '项目管理', tags: ['调度', '优先级', '资源'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    {
        id: 'base_rate',
    ...
    // ─── 法律/治理 ────
    {
        ...
        skillPatterns: [
            { name: '合规审查流程', summary: '基于制衡原理设计多层审批节点和权力分立，降低单一决策风险', category: '合规管理', tags: ['合规', '审查', '审批'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
            { name: '权限分离设计', summary: '将高风险操作拆分为提议-审核-执行三道防线，防止权力集中', category: '治理设计', tags: ['权限', '安全', '治理'], isMarketplaceSkill: true, prerequisites: [], failureModes: [], sourceTier: 'inferred', dependsOn: [], conflictsWith: [], triggers: [] },
        ],
    },
    {
...
/** 获取框架库的默认实例 */
function getFrameworkLibrary() {
    return exports.SEED_FRAMEWORKS;
}
/** 按框架ID获取其类别（用于角色-类别权重匹配） */
function getCategoryForFrameworkId(id) {
    return exports.SEED_FRAMEWORKS.find(f => f.id === id)?.category;
}
/**
 * 将一条约束匹配到框架库。
 * 匹配逻辑：约束文本中的关键词命中框架的 constraintPatterns。
 * 只匹配 constraintPatterns 非空的框架。
 */
function matchFrameworksByConstraint(constraint, candidates) {
    const constraintText = constraint.toLowerCase();
    const frameworks = candidates ?? exports.SEED_FRAMEWORKS;
    const results = [];
