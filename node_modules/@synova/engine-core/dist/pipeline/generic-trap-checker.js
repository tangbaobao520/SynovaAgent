...
exports.checkGenericTrap = checkGenericTrap;
 ...
 */
function isGenericRole(roleId, roleName) {
    ...
    if (industryMarkers.some(m => name.includes(m)))
        return false;
    // 2. 纯英文缩写且≤5字符 → 通用（pm/dev/qa/cto/cfo/...）
    if (/^[a-z]{2,5}$/.test(roleId))
        return true;
    // 3. 通用后缀 + 无行业限定词 → 通用
    ...
    if (hasGenericSuffix) {
        ...
        if (cleanedName.length <= 4)
            return true;
        // 清除后名字中无行业关键词 → 通用
        if (!industryMarkers.some(m => cleanedName.includes(m)))
            return true;
    }
    return false;
}
 ...
 */
function getSpecialNeedKeywords(constraints) {
 ...
 */
function checkGenericTrap(roleIds, roleNames, constraints) {
 ...
 * 构建重推 prompt
 */
function buildRetryPrompt(specialConstraints, constraints) {
