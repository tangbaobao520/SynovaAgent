...
exports.assessSpecificity = assessSpecificity;
exports.prioritizeConstraints = prioritizeConstraints;
exports.runEngineDerivation = runEngineDerivation;
exports.buildDesignRationale = buildDesignRationale;
exports.runPhaseA = runPhaseA;
const types_1 = require("../types");
const llm_client_1 = require("../llm-client");
const generic_trap_checker_1 = require("./generic-trap-checker");
const ammo_injector_1 = require("./phase-b/ammo-injector");
...
function assessSpecificity(taskDef) {
    let score = 0;
    const signals = [];
    const allText = [
        taskDef.job || '',
 ...
 * 完全在引擎侧完成，不调用 LLM。
 */
function runEngineDerivation(taskDef) {
    ...
    const allMatches = [];
    const unmatchedConstraints = [];
    for (const c of constraints) {
        const matches = (0, framework_matcher_1.matchFrameworksByConstraint)(c);
        if (matches.length === 0) {
    ...
    // Step 2+3: 约束→框架→角色
    const derivedRoleSources = (0, framework_matcher_1.deriveRolesFromConstraints)(constraints);
    // Step 4: 角色汇总去重
    const clusteredRoles = (0, framework_matcher_1.clusterAndDedupRoles)(derivedRoleSources);
    // Step 4.5: 冲突检测与拆分
    const resolvedRoles = (0, framework_matcher_1.detectAndResolveConflicts)(clusteredRoles);
    const conflictNotes = [];
...
// ================================================================
function buildDetailFillPrompt(locale, clusteredRoles, taskJob, isColdStart) {
...
// ================================================================
function buildFullDerivationPrompt(locale, isColdStart) {
    const coldStartSection = isColdStart ? COLD_START_EXAMPLES : '';
...
function buildUserPrompt(taskDef) {
...
const log = (0, logger_1.createLogger)('engine-server/pipeline/phase-a-derive-roles');
// ================================================================
// JSON 解析降级 — LLM 两次都失败时用规则生成最低可用输出
// ================================================================
function buildFallbackRoleOutput(mode, clusteredRoles, taskDef) {
    const roles = (clusteredRoles && clusteredRoles.length > 0
        ? clusteredRoles
        : [{ roleType: '通用执行', confidence: 0.5 }, { roleType: '质量把关', confidence: 0.5 }]).map((cr, i) => ({
        id: `role-fallback-${i}`,
        name: cr.roleType,
 ...
 */
async function executePhaseA(taskDef, locale, abortSignal, isColdStart, mode, clusteredRoles, retryPrompt, diagnosisContext) {
    let systemPrompt;
    let userMsg;
    // 弹药注入（L0 confirm 后，Phase A 前 — ARCH-00 §二 Phase A）
    const ammoText = (0, ammo_injector_1.injectAmmo)(taskDef);
    ...
    try {
        ...
        log.warn(`[phase-a] JSON 解析失败，重试: ${parseErr.message}`);
        try {
            ...
            log.warn(`[phase-a] 重试仍失败，降级到规则推导: ${retryErr.message}`);
    ...
    const designRationale = parsed.designRationale || {
        ...
        reason: parsed.roles?.map((r) => `角色「${r.name}」: ${r.responsibilities?.join('、') || '通用'}`).join('；') || '推理过程由 LLM 内部完成',
    ...
    const incubationFrame = {
        ...
        phaseLabel: types_1.PHASE_LABELS.L1_derive_roles,
        progress: 20,
    ...
    const phaseA = { teamStructure, incubationFrame, llmRaw: result.content };
    return { phaseA, parsed };
}
// ================================================================
// Step 6: 组装 DesignRationaleEntry[]（推理过程外化）
// ================================================================
function buildDesignRationale(engineResult, teamStructure, taskDef) {
    const entries = [];
 ...
 *   - 退化检测失败 → 自动重推（最多 2 次）
 */
async function runPhaseA(taskDef, locale, abortSignal, notes, diagnosisContext) {
    ...
    const mode = useEngineDerivation ? 'detail_fill' : 'full_derivation';
    if (useEngineDerivation) {
        ...
        if (notes)
            notes.push(`引擎推导产出 ${engineResult.resolvedRoles.length} 个角色（<2），降级 LLM 全量推导`);
    }
    ...
    if (resolvedForLLM.length > maxRoles) {
        resolvedForLLM = [...resolvedForLLM]
            ...
            .slice(0, maxRoles);
        if (notes)
            notes.push(`⚠️ 引擎推导 ${engineResult.resolvedRoles.length} 个角色，超过上限${maxRoles}（用户指定${userSpecifiedSize || '未指定'}人），截取置信度最高的 ${maxRoles} 个`);
    }
    const clusteredForLLM = resolvedForLLM.map(cr => ({
        roleType: cr.roleType,
        confidence: cr.confidence,
    }));
    const { phaseA, parsed } = await executePhaseA(taskDef, locale, abortSignal, isColdStart, mode, useEngineDerivation ? clusteredForLLM : undefined, undefined, diagnosisContext);
    // ── 退化检测 ──
    const roleIds = phaseA.teamStructure.roles.map(r => r.id);
    const roleNames = phaseA.teamStructure.roles.map(r => r.name);
    const trapCheck = (0, generic_trap_checker_1.checkGenericTrap)(roleIds, roleNames, taskDef.constraints);
    let finalPhaseA = phaseA;
    if (!trapCheck.passes) {
        log.info(`[phase-a] 退化检测触发: ${trapCheck.reason}`);
        if (notes)
            notes.push(`退化检测触发: ${trapCheck.reason}`);
        for (let retry = 0; retry < 2; retry++) {
            const retryPrompt = (0, generic_trap_checker_1.buildRetryPrompt)(trapCheck.specialConstraints, taskDef.constraints);
            const retryResult = await executePhaseA(taskDef, locale, abortSignal, isColdStart, mode, useEngineDerivation ? clusteredForLLM : undefined, retryPrompt, diagnosisContext);
            ...
            const reCheck = (0, generic_trap_checker_1.checkGenericTrap)(newRoleIds, newRoleNames, taskDef.constraints);
            if (reCheck.passes) {
                if (notes)
                    notes.push(`退化检测重推成功（第 ${retry + 1} 次）`);
                break;
            }
            if (retry === 1) {
                if (notes)
                    notes.push(`退化检测重推 2 次后仍未通过，保留最后一次输出`);
            }
        }
    }
    // ── Step 6: DesignRationale 组装 ──
    const designRationale = buildDesignRationale(useEngineDerivation ? engineResult : null, finalPhaseA.teamStructure, taskDef);
    // 将推理链注入 Phase A 结果
