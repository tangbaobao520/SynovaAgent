...
exports.formatConflictsForLog = formatConflictsForLog;
// ─── 矛盾关键词对 ──────────────────────────────────────────
/**
 * 矛盾关键词对定义
 *
 ...
 * 每组 [保守端, 激进端] 对应一对矛盾方向。
 */
const CONTRADICTORY_KEYWORD_PAIRS = [
    // 安全 vs 冒险
 ...
 */
function extractContradictionVector(text, pair) {
 ...
 */
function detectConflicts(frameworks) {
    const conflicts = [];
    const rejectedIds = new Set(); // 存储被拒绝的 MatchedFramework 的唯一标识
    ...
    const frameworkMap = new Map(); // framework.id → MatchedFramework[]
        ...
        for (let j = i + 1; j < uniqueIds.length; j++) {
            ...
            for (const pair of CONTRADICTORY_KEYWORD_PAIRS) {
                ...
                if (vecA.conservative > 0 && vecB.aggressive > 0) {
                    if (processedPairs.has(`${fwA.id}::${fwB.id}`))
                        continue;
                    ...
                    if (scoreA <= scoreB) {
                        for (const mf of frameworkMap.get(fwA.id) || [])
                            rejectedIds.add(mfKey(mf));
                    }
                    else {
                        for (const mf of frameworkMap.get(fwB.id) || [])
                            rejectedIds.add(mfKey(mf));
                    }
                }
    ...
    const processedOverlapPairs = new Set();
    for (let i = 0; i < uniqueIds.length; i++) {
        for (let j = i + 1; j < uniqueIds.length; j++) {
            const fwA = uniqueFrameworks[i];
            const fwB = uniqueFrameworks[j];
            // 跳过已在矛盾检测中处理的配对
            if (processedPairs.has(`${fwA.id}::${fwB.id}`) || processedPairs.has(`${fwB.id}::${fwA.id}`))
                continue;
            // 跳过已因第一遍被拒绝的框架
            const mfListA = frameworkMap.get(fwA.id) || [];
            const mfListB = frameworkMap.get(fwB.id) || [];
            if (mfListA.every(m => rejectedIds.has(mfKey(m))))
                continue;
            if (mfListB.every(m => rejectedIds.has(mfKey(m))))
                continue;
            // 规则：appliedDecisionTypes 重叠 ≥ 2，且 category 相同
            const overlapCount = intersectionSize(fwA.applicableDecisionTypes, fwB.applicableDecisionTypes);
            if (overlapCount >= 2 && fwA.category === fwB.category) {
                if (processedOverlapPairs.has(`${fwA.id}::${fwB.id}`))
                    continue;
                processedOverlapPairs.add(`${fwA.id}::${fwB.id}`);
                const overlapTypes = fwA.applicableDecisionTypes.filter(t => fwB.applicableDecisionTypes.some(bt => bt.toLowerCase() === t.toLowerCase()));
                conflicts.push({
                    frameworkA: fwA.id,
                    frameworkB: fwB.id,
                ...
                if (scoreA <= scoreB) {
                    for (const mf of mfListA)
                        rejectedIds.add(mfKey(mf));
                }
                else {
                    for (const mf of mfListB)
                        rejectedIds.add(mfKey(mf));
                }
            }
        }
    ...
    for (const fw of uniqueFrameworks) {
        ...
        if (mfList.every(m => rejectedIds.has(mfKey(m))))
            continue;
        // 检测：学术类别框架被用到了纯执行决策场景
        if (ACADEMIC_CATEGORIES.has(fw.category)) {
            const operationalOverlap = fw.applicableDecisionTypes.filter(t => OPERATIONAL_KEYWORDS.some(kw => t.toLowerCase().includes(kw.toLowerCase())));
        ...
        if (NON_ACADEMIC_CATEGORIES.has(fw.category)) {
            const strategicOverlap = fw.applicableDecisionTypes.filter(t => ['战略', '架构', '组织设计', '文化建设', '品牌', '市场教育'].some(kw => t.toLowerCase().includes(kw.toLowerCase())));
...
/** 旧 API 兼容: 格式化为日志文本 */
function formatConflictsForLog(result) {
