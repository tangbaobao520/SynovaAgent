...
const decision_extractor_1 = require("./decision-extractor");
const framework_matcher_1 = require("./framework-matcher");
const anti_pattern_deriver_1 = require("./anti-pattern-deriver");
const genome_assembler_1 = require("./genome-assembler");
const logger_1 = require("../../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/pipeline/phase-b/mvp-runner');
/**
 * 供应链对接员测试数据集
 * 来自 RS-23 供应链对接员群像蒸馏推导 + MBA智库/行业JD
 */
const SUPPLY_CHAIN_PROFILE = {
    roleName: '跨境电商供应链对接员',
...
function main() {
    ...
    // S1: 从约束推导决策类型
    log.info('--- S1: 决策提取 ---');
    const baseDecisions = (0, decision_extractor_1.extractDecisionsFromConstraints)(SUPPLY_CHAIN_PROFILE);
    log.info(`基础决策类型: ${baseDecisions.length}个`);
    baseDecisions.forEach(d => log.info(`  - ${d.name} (${d.severity})`));
    const enhancedDecisions = (0, decision_extractor_1.enhanceDecisionsFromSources)(baseDecisions, SUPPLY_CHAIN_PROFILE);
    log.info(`\n信源增强后决策类型: ${enhancedDecisions.length}个`);
    enhancedDecisions.forEach(d => log.info(`  - ${d.name} | 频率:${d.frequency} | 严重度:${d.severity}`));
    // S3: 框架匹配
    log.info('\n--- S3: 框架匹配 ---');
    const matchResult = (0, framework_matcher_1.matchFrameworks)(enhancedDecisions);
    ...
    const topFrameworks = (0, framework_matcher_1.selectTopFrameworks)(matchResult, 5);
    log.info('\nTop 5 心智模型:');
    ...
    // S5: 反模式推导
    log.info('\n--- S5: 反模式 ---');
    const antiPatterns = (0, anti_pattern_deriver_1.deriveAntiPatterns)(enhancedDecisions, matchResult.matchedFrameworks);
    ...
    // S6: 组装 PersonaGenome
    log.info('\n--- S6: 组装 ---');
    const genome = (0, genome_assembler_1.assembleGenome)(SUPPLY_CHAIN_PROFILE.roleName, enhancedDecisions, matchResult.matchedFrameworks, antiPatterns);
    log.info(`\n=== 生成结果 ===`);
...
main();
//# sourceMappingURL=mvp-runner.js.map