...
exports.checkQuality = checkQuality;
// ─── 常量 ──────────────────────────────────────────────────
...
function checkMentalModelCount(genome) {
...
/** 检查 2: 反模式数量在 [2, 7] 范围内 */
function checkAntiPatternCount(genome) {
...
function checkHonestyBoundaries(genome) {
...
function checkExDNADistinction(genome) {
...
function checkCategoryDiversity(genome) {
...
/** 检查 6: 反模式与心智模型的关联完整性 */
function checkAntiPatternLinkage(genome) {
    let linkedCount = 0;
...
function checkConfidenceNumeric(genome) {
...
/** 检查 8: 心智模型名称无重复 */
function checkNoDuplicateModels(genome) {
 ...
 */
function runQualityCheck(genome) {
    const items = ALL_CHECKS.map(check => check(genome));
    ...
    for (const item of [...criticalFailed, ...moderateFailed]) {
        switch (item.id) {
            case 'QC-01':
                suggestions.push(genome.mentalModels.length < MIN_MENTAL_MODELS
                ...
                break;
            case 'QC-02':
                suggestions.push(genome.antiPatterns.length < MIN_ANTI_PATTERNS
...
const GOV_LAYER_MIN_MODELS = {
    L3_governance: 3,
    L2_execution: 2,
    L1_understanding: 1,
};
/** 治理层 → 是否要求决策场景（解释框架级深度） */
const GOV_LAYER_REQUIRES_SCENARIOS = {
    L3_governance: true,
    L2_execution: false,
    L1_understanding: false,
};
/** 旧 API: 批量质量检查 */
function checkQuality(genomes, governanceLayers) {
    const issues = [];
    for (const g of genomes) {
        ...
        if (GOV_LAYER_REQUIRES_SCENARIOS[govLayer] && g.mentalModels && g.mentalModels.length > 0) {
            const hasScenarios = g.mentalModels.some(mm => mm.decisionScenarios && mm.decisionScenarios.length > 0);
            if (!hasScenarios) {
                issues.push({
                    roleId: g.roleId,
                    dimension: 'cognitive_depth',
