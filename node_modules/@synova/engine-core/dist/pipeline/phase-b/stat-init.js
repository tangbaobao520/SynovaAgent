...
exports.runStatInit = runStatInit;
exports.shouldUseStatInit = shouldUseStatInit;
/**
 * 角色类型→通用框架映射表
 *
 ...
 * 根据角色名称匹配角色类型
 */
function matchRoleType(roleName) {
 ...
 * 根据任务描述匹配任务域
 */
function matchTaskDomain(taskJob) {
 ...
 */
function deriveDefaultOcean(roleName) {
    const lower = roleName.toLowerCase();
    // 管理/领导类：高尽责性、中外向性
 ...
 */
function generateStatInitGenome(roleName, roleId, frames) {
 ...
 */
function runStatInit(teamStructure, taskJob) {
    const genomes = [];
 ...
 */
function shouldUseStatInit(teamStructure, forceDegrade) {
    if (forceDegrade)
        return true;
