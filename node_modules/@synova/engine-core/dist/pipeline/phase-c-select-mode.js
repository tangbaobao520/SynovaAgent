...
exports.extractBudgetFromConstraints = extractBudgetFromConstraints;
exports.hasSensitiveDomain = hasSensitiveDomain;
exports.scoreProtocolModes = scoreProtocolModes;
exports.selectProtocol = selectProtocol;
exports.runPhaseC = runPhaseC;
exports.safeDivisionOfLabor = safeDivisionOfLabor;
exports.safeConflictResolution = safeConflictResolution;
exports.deriveDefaultApprovals = deriveDefaultApprovals;
exports.enforceGapConsistency = enforceGapConsistency;
const types_1 = require("../types");
const llm_client_1 = require("../llm-client");
const llm_json_repair_1 = require("./llm-json-repair");
const cross_persona_checker_1 = require("./cross-persona-checker");
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/pipeline/phase-c-select-mode');
// ================================================================
...
function extractSignals(taskDef, phaseA, phaseB) {
    ...
    for (const r of roles) {
        if (r.governanceLayer in layerCounts)
            layerCounts[r.governanceLayer]++;
    }
...
function extractTeamSizeFromConstraints(constraints) {
...
function extractBudgetFromConstraints(constraints) {
 ...
 */
function applyTaskConstraintWeightings(taskDef, signals, scored) {
    ...
    if (teamSize <= 3) {
        // 小团队减重企业级模式的权重
        for (const mode of ['loose_federation', 'cross_check_balance', 'tencent_internal_race', 'democratic_council']) {
    ...
    if (monthlyBudget !== null && monthlyBudget < 5000) {
        for (const mode of ['iron_captain', 'bytedance_flat', 'haidilao_frontline_auth']) {
            applyPctAdjustment(scored, mode, 0.2, `低预算($${monthlyBudget}/月)倾向轻量协作`);
        }
    }
    // ── 3. 阶段加权 ──
    if (taskDef.stage === 'from_scratch') {
        for (const mode of ['loose_federation', 'cross_check_balance', 'tencent_internal_race']) {
            applyPctAdjustment(scored, mode, -0.15, '从零开始阶段避免大型组织模式');
        }
    }
    else if (taskDef.stage === 'optimization') {
        for (const mode of ['bytedance_flat', 'haidilao_frontline_auth', 'iron_captain']) {
            applyPctAdjustment(scored, mode, 0.1, '优化阶段倾向增量式协作模式');
        }
    }
    // ── 4. 领域敏感性加权 ──
    if (hasSensitiveDomain(taskText)) {
        for (const mode of ['cross_check_balance', 'democratic_council']) {
            applyPctAdjustment(scored, mode, 0.3, '安全/合规/医疗/金融领域增强审批模式');
        }
    }
}
function scoreProtocolModes(s) {
    const scores = new Map();
    for (const m of ALL_MODES)
        scores.set(m, { score: 0, reasons: [] });
    function add(m, pts, reason) {
    ...
    if (s.totalRoles <= 5)
        add('iron_captain', 3, `团队≤5人(${s.totalRoles})`);
    if (s.totalRoles <= 3)
        add('iron_captain', 2, `微型团队(${s.totalRoles}人)`);
    if (s.l3Count === 1 && s.totalRoles >= 3)
        add('iron_captain', 2, '单一治理层角色');
    if (s.hasSpeedPriority)
        add('iron_captain', 2, '速度效率优先');
    if (s.avgNeuroticism > 0.45)
        add('iron_captain', 1, `高神经质(${s.avgNeuroticism.toFixed(2)})→集中决策降低焦虑`);
    // ── democratic_council: 民主议会 — 中型团队 + 多元意见 + 质量优先 ──
    if (s.totalRoles >= 4)
        add('democratic_council', 3, `团队≥4人(${s.totalRoles})`);
    if (s.totalRoles >= 6)
        add('democratic_council', 2, `较大团队(${s.totalRoles}人)`);
    if (s.hasQualityPriority)
        add('democratic_council', 2, '质量稳定优先');
    if (s.avgOpenness > 0.6)
        add('democratic_council', 1, `高开放性(${s.avgOpenness.toFixed(2)})→需要多元意见`);
    if (s.hasCrossCultural)
        add('democratic_council', 1, '跨文化场景→需要多方视角');
    // ── loose_federation: 松散联邦 — 大团队 + 高自律 + 低同步需求 ──
    if (s.totalRoles >= 6)
        add('loose_federation', 3, `大团队(${s.totalRoles}人)`);
    if (s.totalRoles >= 8)
        add('loose_federation', 2, `超大团队(${s.totalRoles}人)`);
    if (s.avgConscientiousness > 0.7)
        add('loose_federation', 2, `高尽责性(${s.avgConscientiousness.toFixed(2)})→成员自律`);
    if (s.avgOpenness > 0.65)
        add('loose_federation', 1, '高开放性→适应自主');
    // ── cross_check_balance: 交叉校验 — 合规/安全/审计场景 ──
    if (s.hasCompliance)
        add('cross_check_balance', 4, '合规/安全/审计需求');
    if (s.l3Count >= 2)
        add('cross_check_balance', 1, '多治理层角色→需要互审');
    if (s.avgConscientiousness > 0.7)
        add('cross_check_balance', 1, `高尽责性(${s.avgConscientiousness.toFixed(2)})`);
    if (s.avgNeuroticism > 0.5)
        add('cross_check_balance', 1, `高神经质(${s.avgNeuroticism.toFixed(2)})→需要校验缓解焦虑`);
    // ── bytedance_flat: 字节扁平 — 信息密集 + 产品型 + 快速迭代 ──
    if (s.hasInnovationPriority && s.hasSpeedPriority)
        add('bytedance_flat', 4, '创新+速度双驱动');
    if (s.hasInnovationPriority)
        add('bytedance_flat', 2, '创新探索优先');
    if (s.hasSpeedPriority)
        add('bytedance_flat', 2, '快速迭代');
    if (s.avgOpenness > 0.7)
        add('bytedance_flat', 2, `高开放性(${s.avgOpenness.toFixed(2)})→信息自由流动`);
    if (s.l2Count >= 3)
        add('bytedance_flat', 1, `执行层≥3(${s.l2Count})`);
    // ── haier_ren_dan_he_yi: 海尔人单合一 — 面向终端客户 + 多BU ──
    if (s.hasExternalDependency)
        add('haier_ren_dan_he_yi', 3, '外部依赖→需要面向客户');
    if (s.hasCrossCultural)
        add('haier_ren_dan_he_yi', 2, '跨文化/多市场');
    if (s.avgExtraversion > 0.6)
        add('haier_ren_dan_he_yi', 1, `高外倾性(${s.avgExtraversion.toFixed(2)})`);
    // ── haidilao_frontline_auth: 海底捞一线授权 — 服务质量 + 一线决策 ──
    if (s.hasQualityPriority)
        add('haidilao_frontline_auth', 3, '服务质量关键');
    if (s.l2Count >= 3)
        add('haidilao_frontline_auth', 2, `执行层≥3(${s.l2Count})→需要一线授权`);
    if (s.avgExtraversion > 0.55)
        add('haidilao_frontline_auth', 1, `高外倾性(${s.avgExtraversion.toFixed(2)})`);
    // ── mckinsey_partnership: 麦肯锡合伙人 — 专家驱动 + 项目制 + 高附加值 ──
    if (s.totalRoles <= 5 && s.l3Count === 1)
        add('mckinsey_partnership', 3, `精英小团队(${s.totalRoles}人, 1治理层)`);
    if (s.isNewVenture)
        add('mckinsey_partnership', 2, '新创→需要专家驱动');
    if (s.avgOpenness > 0.6 && s.avgConscientiousness > 0.65)
        add('mckinsey_partnership', 2, '高开+高责→专家型团队特征');
    // ── tencent_internal_race: 腾讯内部赛马 — 战略窗口 + 高不确定性 ──
    if (s.hasInnovationPriority && s.isNewVenture)
        add('tencent_internal_race', 5, '新创+创新→并行验证');
    if (s.hasInnovationPriority)
        add('tencent_internal_race', 3, '创新优先→多方案并行');
    if (s.hasSpeedPriority && s.isNewVenture)
        add('tencent_internal_race', 2, '新创+速度→赛马缩短验证周期');
    if (s.totalRoles >= 5)
        add('tencent_internal_race', 1, `团队≥5(${s.totalRoles}人)→可拆分并行`);
    return scores;
}
function selectProtocol(scored, signals, team, genomes) {
    // 跨角色兼容性扣分（从外部传入的 scored 已有约束加权，此处只加冲突扣分）
    const crossPersonaPenalties = (0, cross_persona_checker_1.getCrossPersonaPenalties)(team, genomes);
...
// ================================================================
function buildSystemPrompt(locale, selectedMode) {
...
// User Prompt
// ================================================================
function buildUserPrompt(taskDef, phaseA, phaseB, selectedMode, engineReason, conflictHints) {
...
// 主函数
// ================================================================
async function runPhaseC(taskDef, phaseA, phaseB, locale, abortSignal) {
    ...
    const crossPersonaReport = (0, cross_persona_checker_1.checkCrossPersonaCoherence)(phaseA.teamStructure, phaseB.personaGenomes, selection.mode);
    const conflictHints = (0, cross_persona_checker_1.formatConflictsForLLM)(crossPersonaReport);
    ...
    try {
        ...
        const jsonStr = (0, llm_json_repair_1.extractJSON)(llmRaw);
        parsed = JSON.parse(jsonStr);
    }
    catch (llmErr) {
        log.warn(`[phase-c] LLM 填充 6 缝隙失败，降级到引擎默认: ${llmErr.message}`);
        llmRaw = `[phase-c fallback] ${llmErr.message}`;
    ...
    const incubationFrame = {
        ...
        phaseLabel: types_1.PHASE_LABELS.L3_select_mode,
        progress: 60,
...
// ================================================================
function safeDivisionOfLabor(d) {
...
function safeInformationFlow(i) {
...
function safeConflictResolution(c) {
...
function safePowerDistribution(p) {
...
function safeIncentiveAlignment(i) {
...
function safeKnowledgeSharing(k) {
...
function safeExternalInterface(e) {
...
function safeSafetyBaseline(s, taskText) {
...
function deriveDefaultApprovals(taskText) {
    const approvals = [];
    // 数据与隐私
    if (/数据|隐私|个人信息|用户数据|数据出境|数据跨境/.test(taskText))
        approvals.push('数据访问与隐私合规审批');
    // 部署与发布
    if (/部署|上线|发布|生产环境|上线|发版/.test(taskText))
        approvals.push('生产部署与版本发布审批');
    // 合规与认证（通用）
    if (/合规|认证|法规|监管|审计|许可|牌照/.test(taskText))
        approvals.push('合规审计与监管报批');
    // 资金与支付
    if (/资金|转账|支付|付款|交易|结算|汇款/.test(taskText))
        approvals.push('资金操作与支付审批');
    // 合同与法律
    if (/合同|签署|签约|协议|盖章|法务/.test(taskText))
        approvals.push('合同签署与法律文件审批');
    // 外部通信与客户
    if (/外部|客户|用户|公开|对外|品牌|媒体/.test(taskText))
        approvals.push('对外通信与客户承诺审批');
    // 领域特定风险
    // 医疗/生物/制药
    if (/临床|试验|患者|病人|药品|药械|治疗|诊断|手术|医疗|基因|遗传|生物|病毒|疫苗|人体|血液/.test(taskText)) {
        approvals.push('临床试验与伦理审查');
        approvals.push('生物安全与遗传资源审批');
        if (/器械|设备|硬件|可穿戴/.test(taskText))
            approvals.push('医疗器械认证与备案审批');
    }
    // AI/算法
    if (/AI|人工智能|算法|模型|机器学习|深度学习|大模型|LLM|GPT|神经网络/.test(taskText)) {
        approvals.push('AI模型安全评估与算法备案');
        if (/安全|红队|对抗|攻击|越狱|注入/.test(taskText))
            approvals.push('AI安全测试与红队审计审批');
    }
    // 金融/投资
    if (/金融|投资|理财|基金|证券|期货|保险|贷款|风控|信用/.test(taskText)) {
        approvals.push('金融产品合规与风控审批');
        if (/量化|交易|策略|回测|实盘/.test(taskText))
            approvals.push('量化策略实盘审批与回撤控制');
    }
    // 教育/培训
    ...
    if (/餐饮|食品|餐厅|火锅|厨房|食材|菜品|加盟/.test(taskText)) {
        ...
        if (/加盟|特许/.test(taskText))
            approvals.push('加盟商资质与合同审批');
    }
    // 制造/工业
    if (/制造|工厂|生产|产线|设备|机器|工艺|质检/.test(taskText)) {
        approvals.push('生产安全与设备变更审批');
        if (/ERP|MES|数字化|改造/.test(taskText))
            approvals.push('系统改造与数据迁移审批');
    }
    // 供应链/物流
    ...
    if (/航天|太空|卫星|轨道|发射|空间|火箭|在轨/.test(taskText)) {
        ...
        if (/国际|条约|空间法/.test(taskText))
            approvals.push('国际空间法合规审查');
    }
    // DAO/加密/区块链
    if (/DAO|去中心化|区块链|加密|智能合约|代币|治理.*投票/.test(taskText)) {
        approvals.push('智能合约安全审计与多签审批');
        if (/USDC|USDT|稳定币|资金池/.test(taskText))
            approvals.push('资金池操作与多签审批');
    }
    // 跨境/国际
    if (/跨境|海外|出海|东南亚|东盟|欧盟|美国|日本|国际|多国|本地化/.test(taskText)) {
        approvals.push('跨境合规与本地化法规审批');
        if (/数据|隐私/.test(taskText))
            approvals.push('跨境数据传输与数据主权审批');
    }
    // 兜底
 ...
 */
function enforceGapConsistency(cm) {
