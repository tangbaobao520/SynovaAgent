...
const SEED_SKILL_MAP = new Map(skill_seeds_1.ALL_SEED_SKILLS.map(s => [s.name, s]));
/**
 * 根据技能名称和分类生成合理的默认安装指令。
 * 优先从种子数据查找，其次按名称关键词匹配，最后按分类兜底。
 */
function generateDefaultInstallCommand(name, category, tags) {
    ...
    if (nameLower.includes('notion'))
        return 'manual: Notion API integration — register integration at https://developers.notion.com';
    if (nameLower.includes('slack'))
        return 'manual: Slack webhook setup — configure incoming webhook at https://api.slack.com';
    if (nameLower.includes('git'))
        return 'manual: Git CLI — install from https://git-scm.com';
    if (nameLower.includes('docker'))
        return 'manual: Docker Engine — install from https://docs.docker.com/engine/install';
    if (nameLower.includes('aws'))
        return 'manual: AWS CLI — configure via `aws configure`';
    if (nameLower.includes('jira') || nameLower.includes('confluence'))
        return 'manual: Atlassian API token — generate from https://id.atlassian.com/manage/api-tokens';
    if (nameLower.includes('github') || nameLower.includes('gh '))
        return 'manual: GitHub CLI — install from https://cli.github.com';
    if (nameLower.includes('excel') || nameLower.includes('sheet'))
        return 'manual: Spreadsheet API integration — enable Google Sheets API or Office Scripts';
    if (nameLower.includes('sql') || nameLower.includes('database') || nameLower.includes('db '))
        return 'manual: Database connection setup — configure credentials and connection string';
    if (nameLower.includes('python') || nameLower.includes('pip'))
        return 'pip install ' + nameLower.replace(/[^a-z0-9_-]/g, '-');
    if (nameLower.includes('npm') || nameLower.includes('node') || nameLower.includes('react') || nameLower.includes('vue'))
        return 'npm install ' + nameLower.replace(/[^a-z0-9_-]/g, '-');
    // 分类兜底
    if (catLower.includes('数据') || catLower.includes('分析') || tagList.some(t => /数据|分析|data/.test(t)))
        return 'pip install data-analysis-toolkit';
    if (catLower.includes('开发') || tagList.some(t => /代码|编程|code|dev/.test(t)))
        return 'npm install dev-toolkit';
    if (catLower.includes('合规') || catLower.includes('法务') || catLower.includes('审计') || catLower.includes('法律'))
        return 'manual: compliance workflow integration — configure approval chain';
    if (catLower.includes('营销') || tagList.some(t => /营销|social|推广/.test(t)))
        return 'manual: marketing platform API setup — configure API keys';
    if (catLower.includes('翻译') || tagList.some(t => /翻译|translate/.test(t)))
        return 'pip install translation-service';
    if (catLower.includes('内容') || catLower.includes('创作'))
        return 'manual: content creation tool setup';
    if (tagList.some(t => /安全|security|隐私|privacy/.test(t)))
        return 'manual: security tool integration — review access controls';
 ...
 */
function generateDefaultVersion(skillName, fromSeed) {
 ...
 */
function generateDefaultSteps(name, description) {
...
// ================================================================
function buildConstraintInjectionSection(taskDef, phaseA, phaseB, phaseC) {
    const parts = [];
    ...
    for (const genome of phaseB.personaGenomes) {
        if (genome.mentalModels.length > 0) {
            const modelLines = genome.mentalModels.map(m => `  - ${m.name}（${m.application || '通用场景'}）`);
...
// User Prompt
// ================================================================
function buildUserPrompt(taskDef, phaseA, phaseB, phaseC, enginePromptSection) {
...
// JSON 提取 + 修复
// ================================================================
function extractAndRepairJSON(content) {
    // Step 1: 标准提取 + 修复（尾随逗号/无引号键/注释）
    try {
        return (0, llm_json_repair_1.extractJSON)(content);
    }
    catch (_e) {
        log.debug('extractJSON 失败，尝试降级路径: %s', String(_e));
    }
    // Step 2: 手动提取 + 修复
    const jsonMatch = content.match(/\{[\s\S]*\}/);
    if (jsonMatch) {
        return (0, llm_json_repair_1.repairLlmJson)(jsonMatch[0]);
    }
 ...
 */
function rescueTruncatedJSON(jsonStr) {
    // 尝试 1: 直接解析
    try {
        return JSON.parse(jsonStr);
    }
    catch (_e) { /* 预期内失败，进入下一步修复策略 */ }
    // 尝试 2: 修复 + 解析
    const repaired = (0, llm_json_repair_1.repairLlmJson)(jsonStr);
    try {
        return JSON.parse(repaired);
    }
    catch (_e) { /* 预期内失败，进入下一步修复策略 */ }
    // 尝试 3: 从尾部逐步截断，找到能产生最多内容的合法位置
    let bestRescue = undefined;
    let bestLen = 0;
    ...
    if (bestRescue)
        return bestRescue;
    // 尝试 4: 双重修复（先修复再截断补全）
    const r2 = (0, llm_json_repair_1.repairLlmJson)(jsonStr);
...
function sanitizeInstallCommand(cmd, skillName, category, tags) {
    if (!cmd || cmd.trim().length === 0)
        return generateDefaultInstallCommand(skillName, category, tags);
    const clean = cmd.trim();
    // 拒绝常见的 LLM 占位符
    const placeholders = /^(可安装|\[可安装\]|需手动安装|手动安装|待定|TBD|TODO|N\/A|无|暂无|placeholder)$/i;
    if (placeholders.test(clean))
        return generateDefaultInstallCommand(skillName, category, tags);
    // 必须以 npm/pip/apt-get/manual: 开头
    if (!/^(npm |pip |apt-get |manual:)/i.test(clean))
        return generateDefaultInstallCommand(skillName, category, tags);
    return clean;
}
// ================================================================
// 合并引擎核心与 LLM 细节
// ================================================================
function mergeToSkillCards(llmSkillSets, engineMapping, roleId) {
    ...
    for (const core of cores) {
        ...
        cards.push({
            ...
            sourceTier: (llmMatch?.sourceTier || core.sourceTier || 'inferred'),
            ...
            geneSources: (Array.isArray(llmMatch?.geneSources) ? llmMatch.geneSources : []),
    ...
    for (const s of llmSkillSets) {
        ...
        cards.push({
            ...
            sourceTier: (s.sourceTier || 'speculative'),
            ...
            geneSources: (Array.isArray(s.geneSources) ? s.geneSources : []),
    ...
    for (const card of cards) {
        try {
            ...
            const report = (0, engine_context_1.getEngineContext)().securityAudit.auditSkillContent(card.name, content);
            card.securityScore = report.score;
        }
        catch (_e) {
            log.debug('技能安全审计失败，使用默认分: %s', String(_e));
        }
    }
    return cards;
}
/** 从 SkillCard 构建审计文本（关键词扫描用） */
function buildSkillAuditText(card) {
    return [
        `# ${card.name}`,
        card.summary,
        card.description,
...
// 分块生成（角色数 > 3 时使用）
// ================================================================
async function generateSkillsChunked(taskDef, phaseA, phaseB, phaseC, locale, abortSignal, mapping) {
    const allRoles = phaseA.teamStructure.roles;
    const chunks = chunkArray(allRoles, 3);
    const totalChunks = chunks.length;
    const chunkResults = await Promise.all(chunks.map(async (chunkRoles, chunkIndex) => {
        try {
            ...
            const engineSection = (0, skill_mapper_1.buildSkillPromptSections)(chunkMapping);
...
请为以上每个角色生成技能清单。只输出 JSON。`;
            ...
            try {
                ...
                log.warn(`[phase-d] 块 ${chunkIndex + 1}/${totalChunks} JSON 解析失败（${e1.message}），尝试截断补全...`);
            ...
            log.warn(`[phase-d] 块 ${chunkIndex + 1}/${totalChunks} 致命错误: ${err.message}，降级为规则推导`);
            const fallbackSkills = chunkRoles.map(role => ({
                roleId: role.id,
                roleName: role.name,
                skills: (0, skill_mapper_1.buildSkillCardsFromCores)(mapping.get(role.id) || [], role.id),
            }));
    ...
    for (const cr of chunkResults) {
        ...
        for (const role of chunkRoles) {
            const llmSkillSet = (cr.parsed.skillSets || []).find((ss) => ss.roleId === role.id);
            const skills = mergeToSkillCards(llmSkillSet?.skills || [], mapping, role.id);
...
// 主函数
// ================================================================
async function runPhaseD(taskDef, phaseA, phaseB, phaseC, locale, abortSignal) {
    // ── Step 1: 引擎映射技能核心 ──
    const { mapping, gaps } = (0, skill_mapper_1.mapSkillsForTeam)(phaseA.teamStructure.roles, taskDef.constraints);
    // ── Step 1.5: 云端市场兜底（本地未命中 → 云端查找）──
    let cloudHits = 0;
    try {
        const enriched = await (0, skill_mapper_1.enrichCoresWithCloud)(mapping, abortSignal);
        cloudHits = [...enriched.values()]
            .flatMap(cores => cores.filter(c => c.isMarketplaceSkill && c.sourceFrameworkId === 'marketplace'))
            .length;
    }
    catch (err) {
        log.info(`[phase-d] 云端市场查找降级: ${err.message}`);
    }
    const enginePromptSection = (0, skill_mapper_1.buildSkillPromptSections)(mapping);
    if (gaps.length > 0) {
    ...
    try {
        ...
        if (useChunked) {
            ...
            try {
                ...
                log.warn(`[phase-d] JSON 解析失败（${e1.message}），尝试截断补全...`);
                parsed = rescueTruncatedJSON(result.content);
            ...
            skillSets = allRoles.map((role) => ({
                roleId: role.id,
                roleName: role.name,
                skills: mergeToSkillCards((parsed.skillSets || []).find((ss) => ss.roleId === role.id)?.skills || [], mapping, role.id),
            }));
        ...
        const incubationFrame = {
            ...
            phaseLabel: types_1.PHASE_LABELS.L4_match_skills,
            progress: 80,
        ...
        // ── 降级路径: LLM 不可用 → 纯引擎映射 ──
        log.warn(`[phase-d] LLM 不可用，降级为引擎纯规则映射: ${err.message}`);
        const skillSets = phaseA.teamStructure.roles.map((role) => ({
            roleId: role.id,
            roleName: role.name,
            skills: (0, skill_mapper_1.buildSkillCardsFromCores)(mapping.get(role.id) || [], role.id),
        }));
        ...
        const incubationFrame = {
            phaseId: 'L4_match_skills',
            phaseLabel: `${types_1.PHASE_LABELS.L4_match_skills}（降级）`,
            progress: 80,
