...
exports.runPhaseE = runPhaseE;
const types_1 = require("../types");
const llm_client_1 = require("../llm-client");
const llm_json_repair_1 = require("./llm-json-repair");
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/pipeline/phase-e-assemble-blueprint');
// ================================================================
// System Prompt
// ================================================================
...
// User Prompt
// ================================================================
function buildUserPrompt(taskDef, phaseA, phaseB, phaseC, phaseD) {
...
function buildRoleSpecificRules(cls) {
    const rules = [];
    if (cls.isPM) {
...
function buildSoulMd(role, genome, cls) {
...
function buildIdentityMd(role, cls, authorityGovernance, safetyBaseline) {
    const constraints = [];
    if (cls.isPM)
        constraints.push('- 任何产品需求在进入设计前必须通过价值三问');
    if (cls.isTech)
        constraints.push('- 安全审查必须附带覆盖清单和豁免清单');
    if (cls.isGovernance)
        constraints.push('- 窗口期判断优先于技术完美追求');
    return `# IDENTITY
...
function buildHeartbeatMd(role, cls, safetyBaseline) {
...
function buildUserMd(role, taskDef) {
    return `# 用户自定义配置
...
function generateFiveFormats(taskDef, phaseA, phaseB, phaseC, phaseD) {
    ...
    const roleTable = roles.map(r => `| ${r.name} | ${r.governanceLayer || 'L2_execution'} | ${r.responsibilities.slice(0, 2).join('；')} |`).join('\n');
...
function generateGeneYaml(roles, phaseB) {
    const lines = ['genes:'];
    for (const role of roles) {
...
function buildSkillMarkdown(skill, role) {
    const lines = [
        `# ${skill.name}`,
        '',
        `> ${skill.summary}`,
...
// 主函数
// ================================================================
async function runPhaseE(taskDef, phaseA, phaseB, phaseC, phaseD, locale, abortSignal) {
    let parsed;
    let llmRaw;
    try {
        ...
        log.warn(`[phase-e] LLM 蓝图组装失败，降级到规则组装: ${llmErr.message}`);
        llmRaw = `[phase-e fallback] ${llmErr.message}`;
        parsed = { riskCoverage: [], designRationale: [] };
    }
    // 构建风险覆盖表
    const riskCoverage = (parsed.riskCoverage || []).map((rc) => ({
    ...
    // 构建设计依据
    const designRationale = (parsed.designRationale || []).map((dr) => ({
    ...
    const incubationFrame = {
        ...
        phaseLabel: types_1.PHASE_LABELS.L5_assemble_blueprint,
        progress: 95,
