...
exports.buildOrgSteward = buildOrgSteward;
/**
...
const log = (0, logger_1.createLogger)('phase-steward');
const AGENTS_ROOT = path_1.default.join(os_1.default.homedir(), '.openclaw', 'agents');
const GATEWAY_CONFIG = path_1.default.join(os_1.default.homedir(), '.openclaw', 'openclaw.json');
/**
 * 孵化 Org Steward Agent
 */
function buildOrgSteward(orgId, options = {}) {
    const db = (0, engine_context_1.getEngineContext)().database.getDb();
    const agentId = `org-steward-${orgId}`;
    const workspacePath = path_1.default.join(AGENTS_ROOT, agentId);
    // 1. 收集全景数据
  ...
  `).all();
    ...
    const projectLines = projects.map((p) => {
        const pTeams = projectTeamMap.get(p.id) || [];
        const teamSummary = pTeams.map(t => `  - **${t.name}**: ${t.agentCount} 个 Agent`).join('\n');
        return `### ${p.name}（${p.teamCount || 0} 个团队）\n${teamSummary || '  - 暂无团队'}`;
    }).join('\n\n');
...
`;
    ...
    for (const file of files) {
        fs_1.default.writeFileSync(path_1.default.join(workspacePath, file.name), file.content, 'utf-8');
    }
    // 10. 注册到 Gateway (openclaw.json)
    let registeredToGateway = false;
    try {
        if (fs_1.default.existsSync(GATEWAY_CONFIG)) {
            const raw = fs_1.default.readFileSync(GATEWAY_CONFIG, 'utf-8');
            if (raw.trim()) {
                ...
                if (!exists) {
                    agentList.push({
                        ...
                        agentDir: path_1.default.join(workspacePath, 'agent'),
                    ...
                    fs_1.default.writeFileSync(GATEWAY_CONFIG, JSON.stringify(config, null, 2), 'utf-8');
        ...
        log.warn('Gateway 配置更新失败: %s', e.message);
    }
    ...
    try {
      ...
      VALUES (?, ?, ?, 'online', ?, ?, ?, ?, ?, ?)
    `).run(agentId, `${orgName} 总管家`, agentCard.description, path_1.default.join(workspacePath, 'SOUL.md'), path_1.default.join(workspacePath, 'agent'), workspacePath, JSON.stringify(agentCard.skills.map((s) => s.name)), nowISO, nowISO);
        registeredToDb = true;
    }
    catch (e) {
        log.warn('discovered_agents 注册失败: %s', e.message);
    }
    ...
    `).run(sessionId, agentId, `${orgName} 总管家`, nowIso, nowIso);
        ...
        log.warn('默认 session 创建失败: %s', e.message);
    }
    log.info('Steward 已孵化: %s (项目:%d, 团队:%d, Agent:%d)', agentId, projects.length, allTeams.length, allAgents.length);
    return {
        agentId,
        workspacePath,
        orgId,
