...
exports.scoreRoleCredibility = scoreRoleCredibility;
exports.validateBlueprint = validateBlueprint;
exports.validateRedlinesAdapted = validateRedlinesAdapted;
const confidence_gate_1 = require("./phase-b/confidence-gate");
// ================================================================
...
// Gate #1: 合法角色数
// ================================================================
function validateRoleCount(teamStructure) {
    const errors = [];
...
// Gate #2: 权重校验
// ================================================================
function validateWeightRange(personaGenomes) {
    const errors = [];
    const warnings = [];
    ...
    if (warnings.length > 0)
        detailParts.push(`(警告) ${warnings.join('；')}`);
    if (detailParts.length === 0)
        detailParts.push('OCEAN 权重校验通过');
    return {
        gate: 'weight_range',
        passed,
...
// Gate #3: 协议合法性
// ================================================================
function validateProtocolValidity(teamStructure, collaborationMode) {
    const errors = [];
    const warnings = [];
    ...
    if (!KNOWN_COOPERATION_MODES.includes(collaborationMode.mode)) {
    ...
    if (Array.isArray(teamStructure.roles)) {
        ...
        // governanceLayer 值合法
        for (const role of teamStructure.roles) {
            if (!KNOWN_GOVERNANCE_LAYERS.includes(role.governanceLayer)) {
    ...
    if (errors.length > 0)
        detailParts.push(...errors);
    if (warnings.length > 0)
        detailParts.push(`(建议) ${warnings.join('；')}`);
    if (detailParts.length === 0)
        detailParts.push('协议合法性校验通过');
    return {
        gate: 'protocol_validity',
        passed,
        detail: detailParts.join('；'),
...
// Gate #4: 场景一致性
// ================================================================
function validateSceneConsistency(teamStructure, personaGenomes) {
    const errors = [];
    if (!Array.isArray(teamStructure.roles)) {
        return {
...
// Gate #5: 信源校验（STUB）
// ================================================================
function validateSourceVerification(personaGenomes) {
    if (!Array.isArray(personaGenomes) || personaGenomes.length === 0) {
        return {
    ...
    let totalModels = 0;
    let sourcedModels = 0;
    const unsourcedRoles = [];
    const weakSourceRoles = [];
    for (const pg of personaGenomes) {
        if (!Array.isArray(pg.mentalModels) || pg.mentalModels.length === 0)
            continue;
        let roleUnsourced = 0;
        let roleTotal = 0;
    ...
    const detailParts = [`信源覆盖率: ${sourcedModels}/${totalModels} (${Math.round(ratio * 100)}%)`];
...
// ================================================================
function validateConfidenceIndependence(personaGenomes) {
    if (!Array.isArray(personaGenomes) || personaGenomes.length === 0) {
        return {
    ...
    const result = (0, confidence_gate_1.evaluateConfidence)(personaGenomes);
    const issues = [];
...
// ================================================================
function validateGenomeQuality(personaGenomes) {
    const issues = [];
    if (!Array.isArray(personaGenomes) || personaGenomes.length === 0) {
        return {
            gate: 'genome_quality',
 ...
 */
function scoreRoleCredibility(personaGenomes) {
    return personaGenomes.map(pg => {
        ...
        if (Array.isArray(pg.mentalModels) && pg.mentalModels.length >= 2) {
            ...
            for (const mm of pg.mentalModels) {
                if (mm.application && mm.application.length > 6)
                    appliedCount++;
                if (mm.decisionScenarios && mm.decisionScenarios.length > 0)
                    appliedCount++;
            }
...
// 自动修正逻辑（轻微违规）
// ================================================================
function autoCorrect(teamStructure, gateResults) {
    const log = [];
    let corrected = false;
 ...
 * @returns 校验结果
 */
function validateBlueprint(teamStructure, personaGenomes, collaborationMode, options) {
 ...
 * 供外部审核/报告使用
 */
function validateRedlinesAdapted(personaGenomes) {
    const results = [];
    ...
    // 红线4: 诚实边界法则
    const allHaveBoundaries = personaGenomes.every(pg => Array.isArray(pg.honestBoundaries) && pg.honestBoundaries.length > 0);
    results.push({
        ruleId: 4,
        name: '诚实边界法则',
