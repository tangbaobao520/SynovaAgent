...
exports.generateSkillFromScratch = generateSkillFromScratch;
exports.generateSkillsBatch = generateSkillsBatch;
const llm_client_1 = require("../llm-client");
const llm_json_repair_1 = require("./llm-json-repair");
const engine_context_1 = require("../engine-context");
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/pipeline/skill-generator');
// ================================================================
// System Prompt
// ================================================================
function buildGenSystemPrompt(locale) {
...
function buildGenUserPrompt(req) {
    const parts = [`请为以下需求从零设计一个新技能：`];
...
function validateGeneratedSkill(skill) {
    ...
    if (!skill.description || skill.description.trim().length < 10)
        return 'description 过短';
    if (!Array.isArray(skill.steps) || skill.steps.length < 2)
        return 'steps 不足（需 >= 2）';
    if (!Array.isArray(skill.scenarios) || skill.scenarios.length === 0)
        return 'scenarios 为空';
    if (!skill.category || skill.category.trim().length === 0)
        return 'category 为空';
    return null;
}
function sanitizeInstallCommand(cmd) {
    if (!cmd || cmd.trim().length === 0)
        return 'manual: 根据技能文档手动配置';
 ...
 */
async function generateSkillFromScratch(req, abortSignal) {
    ...
    try {
        ...
        const auditResult = (0, engine_context_1.getEngineContext)().securityAudit.auditSkillContent(skill.name, auditText);
        securityScore = auditResult?.score ?? null;
        skill.securityScore = securityScore;
    }
    catch (_e) {
        log.debug('技能安全审计跳过: %s', String(_e));
    }
 ...
 */
async function generateSkillsBatch(requests, abortSignal) {
    ...
    for (const req of requests) {
        try {
            ...
            log.warn(`[skill-generator] 生成 "${req.name}" 失败: ${err.message}`);
            // 继续生成其他技能，一个失败不影响其余
        }
    }
    return results;
