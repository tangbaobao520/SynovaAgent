...
exports.toSynovaYml = toSynovaYml;
exports.toSynovaYmlString = toSynovaYmlString;
exports.fromSynovaYml = fromSynovaYml;
const pipeline_config_1 = require("../pipeline-config");
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/pipeline/synova-yml-serializer');
// ================================================================
// 约束类型推断
// ================================================================
const CONSTRAINT_TYPE_KEYWORDS = [
...
function inferConstraintType(detail) {
...
// ================================================================
function toSynovaYml(taskDef, phaseA, phaseB, phaseC, phaseD, metadata) {
    ...
    const roles = phaseA.teamStructure.roles.map((role) => {
    ...
    return {
        synova: {
            version: '1.0',
            engine: pipeline_config_1.ENGINE_VERSION,
        ...
        team: {
            protocol: {
                ...
                decisionRules: phaseC.collaborationMode.decisionRules,
                safetyBaselines: phaseC.collaborationMode.safetyBaselines,
            },
            roles,
        },
    };
}
function personToSynovaGenome(g) {
    ...
    return {
        cognitiveProfile: {
            ...
            biasVulnerabilities: g.biasVulnerabilities,
        },
...
function skillsToSynovaSkills(cards) {
    return cards.map((s) => ({
        name: s.name,
        category: s.category,
...
function toSynovaYmlString(yml) {
    const lines = [];
    // ── synova 段 ──
    ...
    if (yml.synova.metadata.description)
        lines.push(`    description: "${yml.synova.metadata.description}"`);
    if (yml.synova.metadata.author)
        lines.push(`    author: "${yml.synova.metadata.author}"`);
    if (yml.synova.metadata.tags && yml.synova.metadata.tags.length > 0) {
        lines.push('    tags:');
        for (const t of yml.synova.metadata.tags)
            lines.push(`      - ${t}`);
    }
    if (yml.synova.metadata.category)
        lines.push(`    category: "${yml.synova.metadata.category}"`);
    lines.push('');
    // ── task 段 ──
    lines.push('task:');
    ...
    if (yml.team.protocol.decisionRules && yml.team.protocol.decisionRules.length > 0) {
        ...
        for (const dr of yml.team.protocol.decisionRules) {
            ...
            if (dr.scopeType)
                lines.push(`        scopeType: "${dr.scopeType}"`);
    ...
    if (yml.team.protocol.safetyBaselines && yml.team.protocol.safetyBaselines.length > 0) {
        ...
        for (const sb of yml.team.protocol.safetyBaselines)
            lines.push(`      - "${sb}"`);
    }
    lines.push('');
    // ── team.roles 段 ──
    ...
    for (const role of yml.team.roles) {
        ...
        if (role.genome.cognitiveProfile.secondaryMode)
            lines.push(`          secondaryMode: "${role.genome.cognitiveProfile.secondaryMode}"`);
        ...
        if (role.genome.cognitiveProfile.biasVulnerabilities?.length) {
            ...
            for (const b of role.genome.cognitiveProfile.biasVulnerabilities)
                lines.push(`            - "${b}"`);
        }
        lines.push('');
        ...
        if (role.genome.mentalModels.length > 0) {
            for (const mm of role.genome.mentalModels) {
                ...
                if (mm.limitation)
                    lines.push(`            limitation: "${mm.limitation}"`);
            }
        }
        else {
            lines.push('          []');
        ...
        if (role.genome.antiPatterns.length > 0) {
            for (const ap of role.genome.antiPatterns)
                lines.push(`          - "${ap}"`);
        }
        else {
            lines.push('          []');
        ...
        if (role.skills.length > 0) {
            for (const skill of role.skills) {
                ...
                if (skill.applicableScenarios?.length) {
                    ...
                    for (const s of skill.applicableScenarios)
                        lines.push(`            - "${s}"`);
                }
                lines.push('          steps:');
                ...
                if (skill.approvalRequired?.length) {
                    ...
                    for (const ar of skill.approvalRequired)
                        lines.push(`            - "${ar}"`);
                }
            }
        }
...
// ================================================================
function fromSynovaYml(ymlString) {
    try {
        ...
        if (!synovaMatch || !taskMatch || !teamMatch)
            return null;
        // 基础结构重建
...
//# sourceMappingURL=synova-yml-serializer.js.map