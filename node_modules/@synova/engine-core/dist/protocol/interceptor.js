...
exports.ProtocolInterceptor = void 0;
const rule_engine_1 = require("./rule-engine");
const cache_1 = require("./cache");
const circuit_breaker_1 = require("./circuit-breaker");
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/protocol/interceptor');
/** S1 导入（B-07） */
let checkForbiddenCombinations = null;
...
class ProtocolInterceptor {
    ...
    /** S1 节流（B-07） */
    lastS1FullCheckTime = 0;
    /** S1 未就绪警告只打印一次 */
    _s1WarningLogged = false;
    /** 内部统计 */
    statsData = {
        totalIntercepts: 0,
        ruleMatches: 0,
    ...
    constructor(config) {
        ...
        this.ruleEngine = config?.ruleEngine || new rule_engine_1.RuleEngine();
        this.llmJudge = config?.llmJudge || null;
        this.cache = config?.cache || new cache_1.RulingCache();
        this.circuitBreaker = config?.circuitBreaker || new circuit_breaker_1.CircuitBreaker();
        ...
        this.circuitBreaker.onChange((from, to) => {
            if (to === 'OPEN')
                this.statsData.circuitBreakerTrips++;
        });
        ...
        this.circuitBreaker.onTrip(() => {
            try {
                ...
                console.warn(`[interceptor] S3熔断信号记录失败: ${err.message}`);
            }
        });
    }
    /**
     ...
     */
    async intercept(message, protocol, context) {
        ...
        if (ironResult.matched) {
            ...
            const decision = await this.processViolations(ironResult.violations, message, protocol, context, cacheKey);
            if (decision)
                return decision;
        }
        // ============================================================
        ...
        if (authViolations.length > 0) {
            const decision = await this.processViolations(authViolations, message, protocol, context, cacheKey);
            if (decision)
                return decision;
        }
        // ============================================================
        ...
        if (ruleResult.matched) {
            this.statsData.ruleMatches++;
            const decision = await this.processViolations(ruleResult.violations, message, protocol, context, cacheKey);
            if (decision)
                return decision;
        }
        ...
        if (this.ws) {
            ...
            if (wsViolations.length > 0) {
                const decision = await this.processViolations(wsViolations, message, protocol, context, cacheKey);
                if (decision)
                    return decision;
            }
        }
     ...
     */
    async processViolations(violations, message, protocol, context, cacheKey) {
        if (violations.length === 0)
            return null;
        const severityPriority = { LOCK: 3, BLOCK: 2, WARN: 1 };
        const maxSev = violations.reduce((max, v) => severityPriority[v.severity] > severityPriority[max.severity] ? v : max, violations[0]);
        // LOCK: 直接拦截，不可 override
     ...
     *   未知操作（不在任何列表）→ WARN
     */
    checkAuthority(message, protocol, _context) {
        const authority = protocol.gaps.authorityGovernance;
        if (!authority)
            return [];
        const violations = [];
        const senderRole = message.from;
        // —— 结构化角色规则检查 (GAP-3) ——
        const roleRules = authority.roleRules || [];
    ...
    /** 检查消息中是否包含审批凭证 */
    hasApprovalInMessage(message) {
        const approvalMarkers = ['[APPROVED]', '[AUTHORIZED]', 'approval_id:', 'approved_by:'];
        return approvalMarkers.some(m => message.content.includes(m) || message.metadata?.approvalId?.length > 0);
    }
    /** 操作名称模糊匹配（支持通配符和子串匹配） */
    operationMatches(actual, pattern) {
     ...
     * 追加 S1 完整推演（节流执行）
     */
    async inspect(response, protocol, sessionLog) {
        const violations = [];
        const warnings = [];
     ...
     */
    buildInspectIncentiveHints(violations, warnings, fragmentCount, totalMessages) {
        const hints = [];
    ...
    loadS1() {
        try {
            ...
            if (!this._s1WarningLogged) {
                ...
                console.info(`[interceptor] S1 演绎引擎未就绪 (${err.message})，L3 协议检查将跳过 S1 演绎推理`);
            }
        }
    }
    /**
     ...
     */
    checkWorkspaceOperation(message, protocol, _context) {
        const infoFlow = protocol.gaps['information_flow'];
        const routingMap = infoFlow['routingMap'];
        const topology = infoFlow['topology'] || 'full_mesh';
        const visibilityMatrix = infoFlow['visibilityMatrix'] || {};
        ...
        const severity = this.deriveWorkspaceSeverity(topology, message.from, wsOp.targetRole, visibilityMatrix, allowedReaders);
     ...
     */
    deriveWorkspaceSeverity(topology, fromRole, targetRole, visibilityMatrix, _allowedReaders) {
        switch (topology) {
            case 'star':
                return 'LOCK';
            case 'chain': {
     ...
     * 估算链式拓扑中两角色间的跳数（BFS）
     */
    estimateHops(from, to, visibilityMatrix) {
        const visited = new Set();
        const queue = [{ role: from, hops: 0 }];
    ...
    inferLayer(roleId) {
        ...
        if (L1.includes(roleId))
            return 1;
        if (L2.includes(roleId))
            return 2;
        if (L3.includes(roleId))
            return 3;
        return null;
    }
    /** LLM 裁决路径 */
    async llmPath(message, protocol, context, violations, cacheKey) {
    ...
    /** 降级路径 */
    fallbackPath(violations, _cacheKey, _protocolVersion, reason) {
    ...
    /** 构造 InterceptResult */
    buildResult(passed, violations, source) {
        return {
            passed,
            violations,
     ...
     */
    buildIncentiveHints(allowed, violations) {
        const hints = [];
        if (allowed) {
            if (violations.length === 0) {
                hints.push({
    ...
    /** 获取会话的 override 已用次数 */
    getOverrideCount(sessionId) {
        return this.overrideCounts.get(sessionId) || 0;
    }
}
exports.ProtocolInterceptor = ProtocolInterceptor;
