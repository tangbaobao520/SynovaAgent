...
exports.NO_REGRET_ACTIONS = void 0;
exports.validateNoRegretActions = validateNoRegretActions;
// =================================================================
// 无悔动作注册表
// =================================================================
/**
 ...
 */
exports.NO_REGRET_ACTIONS = [
    {
        id: 'NRA-01',
        name: '安全基线约束注入',
        description: '无论场景如何推算，安全约束（拒答清单、数据隔离、合规红线）必须在所有输出前预注入。' +
            '即使场景推算完全错误，安全约束的存在也不应有负面影响。',
        priority: 'P0',
        credibleHarm: 'none',
        trigger: 'always',
        executeSignature: 'injectSafetyConstraints(taskDef: TaskDefinition): TaskDefinitionWithSafety',
        implementationNote: '在 Phase A Step 1 约束分解后、Step 2 框架匹配前执行。' +
    ...
    },
    {
        id: 'NRA-02',
        name: '6 缝隙探针全覆盖保证',
        description: '任何探针流程必须确保 6 个缝隙维度全部覆盖，不允许在采集 5/8 或 6/8 时过早收敛。' +
            '即使某些缝隙在特定场景下看起来"不相关"，也必须显式探针并标记为 not_found。',
        priority: 'P0',
        credibleHarm: 'low',
        trigger: 'always',
        executeSignature: 'ensureAllGapsCovered(probeResults: ProbeResult[]): void',
        implementationNote: '在诸葛探针执行后、L1 蒸馏前执行。扫描 GapDimension[] 检查是否 8/8 覆盖。' +
    ...
    },
    {
        id: 'NRA-03',
        name: '诚实标记默认开启',
        description: '所有输出的 claim 必须默认开启诚实标记（claim_type + confidence + source_reference）。' +
            '即使后续发现信源不可靠，诚实标记的存在使得追溯成为可能。' +
            '关闭诚实标记需要显式审批（魏征或沈括），不是默认状态。',
        priority: 'P0',
        credibleHarm: 'none',
        trigger: 'always',
        executeSignature: 'attachHonestyBadge(output: EngineOutput): EngineOutputWithBadge',
        implementationNote: '在质量门禁（quality-gate.ts）输出前执行包装。' +
 ...
 */
function validateNoRegretActions() {
    const issues = [];
    if (exports.NO_REGRET_ACTIONS.length > 5) {
        issues.push(`无悔动作超过上限（${exports.NO_REGRET_ACTIONS.length}/5）。需要沈括裁定。`);
    }
    for (const action of exports.NO_REGRET_ACTIONS) {
 ...
 */
//# sourceMappingURL=no-regret-actions.js.map