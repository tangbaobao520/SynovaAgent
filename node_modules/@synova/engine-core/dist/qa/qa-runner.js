...
exports.callJudge = callJudge;
exports.computeDimensionScore = computeDimensionScore;
exports.summarizeBlueprintForJudge = summarizeBlueprintForJudge;
exports.runQASuite = runQASuite;
exports.evaluateBlueprint = evaluateBlueprint;
...
const scoring_rubrics_1 = require("./scoring-rubrics");
const llm_client_1 = require("../llm-client");
const llm_json_repair_1 = require("../pipeline/llm-json-repair");
const engine_context_1 = require("../engine-context");
const logger_1 = require("../infra/logger");
const log = (0, logger_1.createLogger)('engine-server/qa/qa-runner');
// ================================================================
// 默认配置
// ================================================================
exports.DEFAULT_CONFIG = {
    ...
    baselinePath: path_1.default.join(__dirname, 'baseline.json'),
    reportPath: path_1.default.join(__dirname, 'report.json'),
};
// ================================================================
// Judge prompt 构建
// ================================================================
function buildJudgePrompt(dimension, blueprintSummary) {
    const subRubricText = dimension.subRubrics.map((s, i) => `${i + 1}. **${s.label}** (满分${s.maxScore}分): ${s.description}\n   ${s.scoringGuide}`).join('\n\n');
...
// ================================================================
async function callJudge(systemPrompt, _config) {
    try {
        ...
        const parsed = (0, llm_json_repair_1.tryParseRepaired)(jsonStr);
        if (!parsed.success) {
        ...
        log.error(`[QA Judge] 调用失败: ${err.message}`);
        return null;
    }
}
// ================================================================
// 评分计算
// ================================================================
function computeDimensionScore(dimension, judgeResult, config) {
...
// ================================================================
function summarizeBlueprintForJudge(blueprint) {
    const bp = blueprint;
    const parts = [];
    // 任务定义
    if (bp.taskDef) {
        const td = bp.taskDef;
        parts.push(`## 任务定义\n- 工作: ${td.job || ''}\n- 阶段: ${td.stage || ''}\n- 约束: ${Array.isArray(td.constraints) ? td.constraints.join('; ') : ''}`);
    }
    // 关键基础设施（提前，避免被截断）
    // 可部署模板
    if (bp.deployableTemplate) {
        const dt = bp.deployableTemplate;
        const dtAgents = Array.isArray(dt.agents) ? dt.agents : [];
    ...
    // 设计依据（提前，避免被截断）
    const rationaleArr = Array.isArray(bp.designRationale) ? bp.designRationale : [];
    if (rationaleArr.length > 0) {
        parts.push(`## 设计依据（共 ${rationaleArr.length} 条）`);
        for (const dr of rationaleArr.slice(0, 15)) {
            parts.push(`- ${dr.dimension}: ${dr.choice} — ${dr.reason?.slice(0, 80) || ''}${dr.hypothesisTag ? ' [假设]' : ''}`);
        }
        if (rationaleArr.length > 15)
            parts.push(`... 另有 ${rationaleArr.length - 15} 条`);
    }
    else {
        parts.push('## 设计依据\n❌ 未提供 designRationale');
    }
    ...
    // L1: 团队结构
    if (bp.teamStructure) {
        const ts = bp.teamStructure;
        parts.push(`## 团队结构\n- 总角色数: ${ts.totalRoles}\n- 推荐规模: ${ts.recommendedTeamSize}\n- 推导方法: ${ts.derivationMethod}`);
        if (Array.isArray(ts.roles)) {
            parts.push('### 角色列表');
            for (const r of ts.roles) {
                parts.push(`- ${r.name} (${r.governanceLayer || 'L2'}): ${Array.isArray(r.responsibilities) ? r.responsibilities.slice(0, 3).join(', ') : ''}`);
            }
        }
    }
    // L2: 基因组
    if (Array.isArray(bp.personaGenomes)) {
        parts.push('## 角色基因组');
        for (const pg of bp.personaGenomes) {
            const ocean = pg.oceanScores;
            const hb = Array.isArray(pg.honestBoundaries) ? pg.honestBoundaries.slice(0, 2).join('; ') : '';
            const ap = Array.isArray(pg.antiPatterns) ? pg.antiPatterns.slice(0, 2).join('; ') : '';
            parts.push(`- ${pg.roleName}: O=(${ocean?.openness ?? '?'},${ocean?.conscientiousness ?? '?'},${ocean?.extraversion ?? '?'},${ocean?.agreeableness ?? '?'},${ocean?.neuroticism ?? '?'}) 心智模型=${Array.isArray(pg.mentalModels) ? pg.mentalModels.map(m => m.name).join('|') : ''} honestBoundaries=${hb ? `[${hb}]` : '缺失'} antiPatterns=${ap ? `[${ap}]` : '缺失'}`);
        }
    }
    // L3: 协作模式
    if (bp.collaborationMode) {
        const cm = bp.collaborationMode;
        ...
        for (const gap of ['divisionOfLabor', 'informationFlow', 'conflictResolution', 'powerDistribution', 'incentiveAlignment', 'trustModel', 'knowledgeSharing', 'externalInterface']) {
            ...
            if (g) {
                ...
                if (key)
                    parts.push(`  - ${gap}: ${key}=${g[key]}`);
            }
        }
        // 安全基线
        const sb = cm['safetyBaseline'];
        if (sb) {
            parts.push(`  - 安全基线: maxAutonomy=${sb.maxAutonomyLevel}, auditLog=${sb.auditLogEnabled}, approvals=${Array.isArray(sb.requireHumanApproval) ? sb.requireHumanApproval.join('/') : ''}`);
        }
    }
    // L4: 技能（含可行性关键字段，供 Judge 评分）
    if (Array.isArray(bp.skillSets)) {
        const ssArr = bp.skillSets;
        const totalSkills = ssArr.reduce((sum, ss) => sum + (Array.isArray(ss.skills) ? ss.skills.length : 0), 0);
        const placeholderCount = ssArr.reduce((sum, ss) => {
            if (!Array.isArray(ss.skills))
                return sum;
            return sum + ss.skills.filter(s => {
                const ic = typeof s.installCommand === 'string' ? s.installCommand : '';
        ...
        const noVersionCount = ssArr.reduce((sum, ss) => {
            if (!Array.isArray(ss.skills))
                return sum;
            return sum + ss.skills.filter(s => {
                const v = typeof s.version === 'string' ? s.version : '';
        ...
        for (const ss of ssArr) {
            const roleName = ss.roleName || '';
            if (!Array.isArray(ss.skills))
                continue;
            const skillList = ss.skills;
            parts.push(`### ${roleName}（${skillList.length}技能）`);
            for (const sk of skillList.slice(0, 4)) {
                const name = sk.name || '?';
                const ic = typeof sk.installCommand === 'string' ? sk.installCommand.slice(0, 60) : '❌缺失';
                const steps = Array.isArray(sk.steps) ? sk.steps.length : 0;
            ...
            if (skillList.length > 4)
                parts.push(`  ... 另有 ${skillList.length - 4} 个技能`);
        }
    }
    // L5: 5格式文件（SOUL/IDENTITY/TOOLS/HEARTBEAT/USER/AGENTS.md）
    const fiveFormats = bp.fiveFormats;
    const tp = fiveFormats?.templatePreset;
    const agents = Array.isArray(tp?.agents) ? tp.agents : [];
    if (agents.length > 0) {
        parts.push(`## 5格式文件（共 ${agents.length} 个 Agent）`);
        for (const a of agents) {
            const soulLen = typeof a.soulMd === 'string' ? a.soulMd.length : 0;
            const idLen = typeof a.identityMd === 'string' ? a.identityMd.length : 0;
            const toolsLen = typeof a.toolsMd === 'string' ? a.toolsMd.length : 0;
            const hbLen = typeof a.heartbeatMd === 'string' ? a.heartbeatMd.length : 0;
            const userLen = typeof a.userMd === 'string' ? a.userMd.length : 0;
        ...
        let agentsMdLen = typeof tp?.agentsMd === 'string' ? tp.agentsMd.length : 0;
        let agentsMdFromFallback = false;
        if (agentsMdLen < 100 && bp.deployableTemplate) {
            const dt = bp.deployableTemplate;
            const dtAgentsMd = typeof dt.agentsMd === 'string' ? dt.agentsMd : '';
 ...
 */
function summarizeProtocolForJudge(blueprint) {
    const bp = blueprint;
    const cm = bp.collaborationMode;
    if (!cm)
        return '## 协议合规性\n❌ 无协作模式，无法评估协议合规性';
 ...
 */
function buildProtocolFromBlueprint(cm) {
    try {
        const dol = cm.divisionOfLabor;
        const inf = cm.informationFlow;
        const ag = cm.authorityGovernance;
        const ti = cm.trustIncentive;
        const ks = cm.knowledgeSharing;
        const ext = cm.externalInterface;
        const extIso = ext?.isolation;
        return {
            version: 1,
            mode: cm.mode || 'democratic_council',
            gaps: {
                division_of_labor: { mode: dol?.mode || 'skill_based', roles: [], taskQueue: 'fifo' },
                information_flow: { topology: inf?.topology || 'full_mesh', routingMap: inf?.routingMap || {}, visibilityMatrix: inf?.visibilityMatrix || {} },
                authority_governance: { strategy: ag?.strategy || 'majority_vote', authority: ag?.authority || 'distributed', escalationPath: [], vetoPower: [], decisionRights: {}, tieBreaker: 'senior' },
                trust_incentive: { alignment: ti?.alignment || 'shared_goal', initialTrust: ti?.initialTrust || 0.5, rewardMechanism: 'recognition', penaltyMechanism: 'warning', trustDecayRate: 0.01, trustRebuildRate: 0.05 },
                knowledge_sharing: { defaultVisibility: ks?.defaultVisibility || 'team', artifactRegistry: 'open', retentionPolicy: 'permanent' },
                external_interface: { isolation: { sandboxLevel: extIso?.sandboxLevel || 'standard' }, allowedExternalTools: [], rateLimit: 10 },
            },
        };
    }
    catch {
 ...
 * 统计命中率、准确率、各缝隙覆盖情况。
 */
function simulateInterceptionTest(protocol) {
    const ruleEngine = (0, engine_context_1.getEngineContext)().protocol.createRuleEngine();
    ...
    const testMessages = [
        ...
        // 触发 GAP-IF-01 (WARN) 星型拓扑: "直接告诉" → information_flow
        {
            ...
            shouldViolate: protocol.gaps['information_flow'].topology === 'star',
        },
...
// 执行器：运行单个测试用例
// ================================================================
async function runSingleTestCase(testCase, config) {
    const startTime = Date.now();
    // 1. 运行 Pipeline
    let blueprint;
    try {
        const { runPipeline } = await Promise.resolve().then(() => __importStar(require('../pipeline/orchestrator')));
        const { getOrCreateTask } = await Promise.resolve().then(() => __importStar(require('../task-store')));
        const requestId = `qa_${testCase.id}_${Date.now()}`;
        const { taskRequestId } = getOrCreateTask(testCase.request, requestId);
        blueprint = await runPipeline(taskRequestId, testCase.request, new AbortController().signal);
    }
    catch (err) {
        log.error(`[QA] Pipeline 失败: ${testCase.id} — ${err.message}`);
        return {
    ...
    // 2. 逐维度评分
    const dimensions = [];
    for (const rubric of scoring_rubrics_1.ALL_RUBRICS) {
    ...
    return {
        ...
        testCaseName: testCase.name,
        timestamp: new Date().toISOString(),
        engineVersion: blueprint.engineVersion || 'unknown',
        blueprintId: blueprint.blueprintId || crypto_1.default.randomUUID(),
        pipelineDurationMs,
        dimensions,
        overallScore,
...
// 回归检查
// ================================================================
function checkRegression(current, baseline, config) {
    if (!baseline)
        return undefined;
...
// 主入口：运行全部测试套件
// ================================================================
async function runQASuite(configOverrides) {
    const config = { ...exports.DEFAULT_CONFIG, ...configOverrides };
    const results = [];
    // 用例过滤
    const filter = config.testCaseFilter;
    const corpus = filter ? test_corpus_1.TEST_CORPUS.filter(tc => filter(tc.id)) : test_corpus_1.TEST_CORPUS;
    // 加载基线
    let baselineMap = new Map();
    try {
        if (fs_1.default.existsSync(config.baselinePath)) {
            const baselineData = JSON.parse(fs_1.default.readFileSync(config.baselinePath, 'utf-8'));
...
// ================================================================
async function evaluateBlueprint(blueprint, config) {
    const cfg = { ...exports.DEFAULT_CONFIG, ...config };
    const blueprintSummary = summarizeBlueprintForJudge(blueprint);
    // 并行评分：6 个维度独立调用 LLM Judge，减少串行等待时间
    const rubricResults = await Promise.allSettled(scoring_rubrics_1.ALL_RUBRICS.map(async (rubric) => {
        const prompt = buildJudgePrompt(rubric, blueprintSummary);
    ...
    const dimensions = rubricResults.map((r, i) => {
        if (r.status === 'fulfilled')
            return r.value;
        return {
            dimension: scoring_rubrics_1.ALL_RUBRICS[i].dimension,
            score: 0,
            subScores: scoring_rubrics_1.ALL_RUBRICS[i].subRubrics.map(s => ({ label: s.label, score: 0, maxScore: s.maxScore, comment: `Judge 调用异常: ${r.reason?.message || 'unknown'}` })),
            judgeComment: '评分服务异常',
            passed: false,
            degraded: true,
        };
...
// 报告输出
// ================================================================
function saveReport(suiteResult, config) {
    const dir = path_1.default.dirname(config.reportPath);
    if (!fs_1.default.existsSync(dir))
        fs_1.default.mkdirSync(dir, { recursive: true });
    fs_1.default.writeFileSync(config.reportPath, JSON.stringify(suiteResult, null, 2), 'utf-8');
    log.info(`[QA] 报告已保存: ${config.reportPath}`);
}
function saveBaseline(suiteResult, config) {
    const dir = path_1.default.dirname(config.baselinePath);
    if (!fs_1.default.existsSync(dir))
        fs_1.default.mkdirSync(dir, { recursive: true });
    fs_1.default.writeFileSync(config.baselinePath, JSON.stringify(suiteResult, null, 2), 'utf-8');
    log.info(`[QA] 基线已保存: ${config.baselinePath}`);
}
function printSummary(suiteResult) {
