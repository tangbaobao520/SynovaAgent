...
export type ErrorCode = 'INVALID_REQUEST_ID' | 'INVALID_SCHEMA_VERSION' | 'INVALID_TASKDEF' | 'INSUFFICIENT_SANITIZATION' | 'UNAUTHORIZED' | 'FORBIDDEN' | 'TASK_REQUEST_NOT_FOUND' | 'RATE_LIMIT' | 'ENGINE_ERROR' | 'ENGINE_UNAVAILABLE' | 'TIMEOUT' | 'EXPIRED';
...
export type TaskRequestStatus = TaskRequestProcessing | TaskRequestCompleted | TaskRequestFailed;
...
export type PipelinePhase = 'L1_derive_roles' | 'L2_distill_genome' | 'L3_select_mode' | 'L4_match_skills' | 'L5_assemble_blueprint';
export declare const PIPELINE_PHASES: PipelinePhase[];
export declare const PHASE_LABELS: Record<PipelinePhase, string>;
...
export type CollaborationMode = 'iron_captain' | 'democratic_council' | 'loose_federation' | 'cross_check_balance' | 'bytedance_flat' | 'haier_ren_dan_he_yi' | 'haidilao_frontline_auth' | 'mckinsey_partnership' | 'tencent_internal_race';
...
/** 5 种跨团队通信模式 */
export type CrossTeamPattern = 'async_delegation' | 'shared_channel' | 'broadcast' | 'negotiation' | 'escalation';
/** 跨团队消息 */
...
export type CrossTeamMessageType = 'task.delegate' | 'task.result' | 'channel.post' | 'channel.reply' | 'broadcast.announce' | 'negotiation.propose' | 'negotiation.counter' | 'negotiation.accept' | 'negotiation.reject' | 'escalation.request' | 'escalation.resolve';
/** 跨团队消息负载 */
export type CrossTeamPayload = DelegatePayload | BroadcastPayload | NegotiationPayload | EscalationPayload;
/** 异步委托负载 */
...
export type MemoryEntryType = 'decision' | 'lesson' | 'pattern' | 'anti_pattern' | 'context' | 'milestone' | 'skill_reuse' | 'protocol_change';
