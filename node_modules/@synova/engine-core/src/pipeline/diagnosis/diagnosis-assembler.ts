...
export function assembleFullDiagnosisV2(teamId: string): FullDiagnosisV2 {
  ...
  try {
    hacd = computeHACD(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] HACD 计算失败');
    degradedModules.push('hacd');
  }

  // CPC: Collaboration Protocol Completeness
  let cpc: FullDiagnosisV2['cpc'] = null;
  try {
    cpc = computeCPC(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] CPC 计算失败');
    degradedModules.push('cpc');
  }

  // IPU: Information Processing Overload
  let ipu: FullDiagnosisV2['ipu'] = null;
  try {
    ipu = computeIPU(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] IPU 计算失败');
    degradedModules.push('ipu');
  }

  // HONA: Heterogeneous Node Network Analysis
  let hona: FullDiagnosisV2['hona'] = null;
  try {
    hona = computeHONA(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] HONA 计算失败');
  ...
  try {
    capabilitySpectrum = computeCapabilitySpectrum(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] capabilitySpectrum 计算失败');
    degradedModules.push('capabilitySpectrum');
  }

  // Intent Alignment: Three-way alignment gap
  let intentAlignment: FullDiagnosisV2['intentAlignment'] = null;
  try {
    intentAlignment = computeIntentAlignment(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] intentAlignment 计算失败');
    degradedModules.push('intentAlignment');
  }

  // 7 Powers: Competitive moat assessment
  let sevenPowers: FullDiagnosisV2['sevenPowers'] = null;
  try {
    sevenPowers = computeSevenPowers(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] sevenPowers 计算失败');
    degradedModules.push('sevenPowers');
  }
  ...
  // HTM: Hybrid Trust Model (ARCH-06 P2)
  let htm: FullDiagnosisV2['htm'] = null;
  try {
    htm = computeHTM(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] HTM 计算失败');
    degradedModules.push('htm');
  }

  // EOB: Elastic Organizational Boundary (ARCH-06 P2)
  let eob: FullDiagnosisV2['eob'] = null;
  try {
    eob = computeEOB(teamId);
  } catch (err) {
    log.warn({ err, teamId }, '[diagnosis/assembler-v2] EOB 计算失败');
    degradedModules.push('eob');
  }
