...
import { runModules, listModules } from './module-registry';
...
> {
  ...
  async runConsultation(
    ...
    while (phase2Iterations < this.config.maxIterations) {
      ...
      try {
        hypotheses = await this.phase2Hypothesize(teamId, scope, evidence);
        break;
      } catch (err) {
  ...
  private async phase2Hypothesize(
    ...
    const executor = new ExpertSubAgentExecutor(this.llmClient, this.toolExecutor);
