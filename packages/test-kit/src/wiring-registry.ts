...
export const WIRING_REGISTRY: WiringEntry[] = [
  ...
  // ═══ M2 权限管理 (对话变更权限) ═══
  {
    moduleName: 'permissionRoutes',
    package: '@synova/synova-agent',
    sourceFile: 'src/routes/permissions.ts',
    expectedEntries: ['src/server.ts'],
    purpose: '权限管理 API — POST /api/permissions/update, POST /api/permissions/bulk, GET /api/permissions/audit',
    required: true,
    status: 'wired',
    ref: 'M2: 对话变更权限',
  },
  {
    moduleName: 'updateAccess',
    package: '@synova/synova-agent',
    sourceFile: 'src/l4/knowledge-store.ts',
    expectedEntries: ['src/routes/permissions.ts', 'src/l3/knowledge-agent.ts'],
    purpose: '单条权限修改 — 财务领域锁定 (不可降级/不可 public)',
    required: true,
    status: 'wired',
    ref: 'M2: 对话变更权限',
  },
  {
    moduleName: 'bulkUpdateAccess',
    package: '@synova/synova-agent',
    sourceFile: 'src/l4/knowledge-store.ts',
    expectedEntries: ['src/routes/permissions.ts', 'src/l3/knowledge-agent.ts'],
    purpose: '批量权限修改 — 按领域/ID 列表',
    required: true,
    status: 'wired',
    ref: 'M2: 对话变更权限',
  },
  {
    moduleName: 'auditPermissionChange',
    package: '@synova/synova-agent',
    sourceFile: 'src/l4/knowledge-store.ts',
    expectedEntries: ['src/routes/permissions.ts', 'src/l3/knowledge-agent.ts'],
    purpose: '权限变更审计日志 — append-only',
    required: true,
    status: 'wired',
    ref: 'M2: 对话变更权限',
  },
  {
    moduleName: 'getAccessStatsByDomain',
    package: '@synova/synova-agent',
    sourceFile: 'src/l4/knowledge-store.ts',
    expectedEntries: ['src/routes/permissions.ts', 'src/l3/knowledge-agent.ts'],
    purpose: '按领域权限分布统计 — admin 概览',
    required: true,
    status: 'wired',
    ref: 'M2: 对话变更权限',
  },
