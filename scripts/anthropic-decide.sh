...
if [ -n "$RAW_CATCHES" ]; then
  while IFS= read -r line; do
    ...
    if ! echo "$CONTEXT" | grep -q "log\.\|logger\.\|console\."; then
      ...
      if ! echo "$CONTEXT" | grep -q "JSON.parse\|ENOENT\|_reading\|\.destroy\|\.end\|\.detach\|setRawMode\|best-effort\|already closed\|keep original\|benign\|intentional\|fall through\|skip non-JSON\|binary file\|not JSON\|SSE.*chunk\|already exists\|ALTER TABLE"; then
