...
class GateChecker:
    ...
    def check_gate_10(self) -> GateResult:
        ...
        # C3: 升级协议——≥3 次再诊断→全量诊断
        upgrade = self.grep(r"escalate_to_full_diagnosis|reDiagnosisCount",
                           "src/growth/lightweight-diagnosis.ts")
        if upgrade:
            info["passed"] += 1
    ...
    def check_gate_12(self) -> GateResult:
        ...
        cron_exists = self.grep(r"registerBuiltinLoops|registerHeartbeatCheck|cron|schedule",
                               "src/loops/loop-scheduler.ts")
        if cron_exists:
            info["passed"] += 1
            parts.append("CronScheduler: 注册逻辑存在")
            self.ok("CronScheduler: 注册逻辑存在")
        else:
            info["partial"] += 1
