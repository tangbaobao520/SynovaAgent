#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# check-bridge-files.sh — 桥接文件欺诈检测 (铁律 46, V4.5.1 全面加固)
#
# 检测 src/ + packages/ 下所有通过 import 引用 engine-core 的文件。
# 涵盖三种绕过模式:
#   1. 包名路径引用 (packages/engine-core/) — 旧检测
#   2. 相对路径引用 (../../engine-core/) — V4.5.1 新增
#   3. 壳包: packages/*/ 下只有 index.ts 且全部是 export from engine-core — V4.5.1 新增
#
# exit 0 = 无欺诈桥接
# exit 1 = 发现欺诈桥接文件
# ═══════════════════════════════════════════════════════════════════════════════
...
if [ -n "$VIOLATIONS" ]; then
  echo -e "${RED}════════════════════════════════════════════════════════${RESET}"
  echo -e "${RED}  铁律 46 违规: 桥接文件欺诈检测 (V4.5.1)${RESET}"
  echo -e "${RED}════════════════════════════════════════════════════════${RESET}"
  echo ""
