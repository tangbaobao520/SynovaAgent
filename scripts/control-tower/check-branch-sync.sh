...
#      (先 git fetch && git merge origin/main —— 不改 hash；rebase 改 hash 会裂 bypass 对账)
#   3. 分叉(双向都有新提交): 同样阻断 (禁止在过期基线上继续提交)
#   4. 逃生舱: SYNO_SKIP_BRANCH_SYNC=1 (需创始人批准, 记 degraded 日志)
# 降级: fetch 失败(离线) → fail-open 显式提示 (不静默 — 铁律 11)
# 测试注入: SYNO_BRANCH_SYNC_ONLY=1 只跑本检查 (check-branch-sync.test.sh 单测)
#
...
check_branch_sync() {
  ...
  if [[ "$behind" -gt 0 ]]; then
    if [[ "$ahead" -gt 0 ]]; then
      ...
      echo "    git fetch $fremote && git merge $fremote/$fmain   # 推荐（不改 hash，bypass 对账不裂）"
      echo "    # 或 git rebase $fremote/${fmain}（改 hash → 需按 D451 补记 bypass.log）"
    else
      echo -e "  ${RED}❌ 分支同步: main 有 $behind 个本分支没有的 commit — 基线已过期${RESET}"
      echo "  先拉平再提交:"
      echo "    git fetch $fremote && git merge $fremote/$fmain   # 推荐（不改 hash）"
      echo "    # 或 git rebase $fremote/${fmain}（改 hash → 需按 D451 补记）"
    fi
    return 1
  fi
