...
if [ "$STAGED" = "1" ]; then
  FILES="$(git -C "$ROOT" -c core.quotepath=false diff --cached --name-only --diff-filter=ACMR | sed '/^$/d')"
else
  FILES="$(git -C "$ROOT" -c core.quotepath=false diff --name-only --diff-filter=ACMR "${BASE}...HEAD" | sed '/^$/d')"
fi
