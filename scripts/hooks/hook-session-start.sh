#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# Loop Engineering V4.4.5 — SessionStart: 流程锁前置
#
# Anthropic 原则 5: 物理强制，零 AI 自律。
# 根因: V4.4.5 的 PreToolUse hook 只拦截 Write/Edit。Bash/Git/文件检查等操作
#       完全绕过。两个 agent 在没跑 task-start 的情况下做了破坏性操作。
#
# V4.4.5 解法: 流程锁前置到 SessionStart。
...
echo "[V4.4.5] SessionStart: 流程未完成 — 已锁定。请先运行 task-start.sh。"
exit 0
