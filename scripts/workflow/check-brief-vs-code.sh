 #!/bin/bash
 # ============================================================
 # Loop Engineering V4.4.1 �?check-brief-vs-code.sh
 # Task Brief 声明 vs 实际代码变更 一致性物理验�? #
 # 设计哲学:
 #   bash 不问 Agent "你是不是认真想了"——bash 只对比例证�? #   brief �?Q2 范围声明了改哪些文件 �?bash grep 实际变更
 #   brief �?Q1 声明了决策原�?�?bash 验证每一条在 Done 标准中有对应 verify
 #   brief �?Q3 声明了验收路�?�?bash 验证每条 verify 命令在当前代码中能执�? #
 # 用法: bash scripts/workflow/check-brief-vs-code.sh [--strict]
 #       --strict: 文件范围不匹�?= 硬阻断。默认模�? 警告�? # pre-commit �?6 组调用。全�?<3s�? #
 # Anthropic 原则 7: "算两�? �?brief 说改 A/B/C，git diff 显示 A/B/C/D/E
 #     �?要么 D/E 是遗漏的声明（brief 不诚实），要么是计划外变更（失控）�? #     不管哪种情况，物理不一�?= 应该停止并修正�? # ============================================================
 ...
 if [ -z "$BRIEF" ]; then
   echo "check-brief-vs-code: �?task brief, 跳过"
   exit 0
 fi
 ...
 # ================================================================
 # 1. Q2 文件范围一致�? brief 声称改的文件 vs 实际 git diff
 # ================================================================
 echo -e "${CYAN}--- 1. Q2 文件范围 vs 实际变更 ---${RESET}"
 
 # 提取 Q2 中声明的文件路径 (支持: - `path/to/file` �?`path/to/file` 格式)
 ...
 # 如果 brief �?Q2 没有任何文件声明，跳过此检查（允许纯文�?纯配置任务）
 if [ -z "$DECLARED_FILES" ]; then
   echo -e "  ${YELLOW}Q2 未声明具体文件路�?(可能是纯文档任务) �?跳过文件范围检�?{RESET}"
 else
   # 找出 ACTUAL 中有�?DECLARED 中没有的文件
   EXTRA_FILES=""
   ...
   if [ -n "$EXTRA_FILES" ]; then
     ...
     if [ "$STRICT_MODE" = true ]; then
       echo -e "  ${RED}Q2 未声明的变更文件 (${EXTRA_COUNT} �?:${RESET}"
       echo -e "$EXTRA_FILES"
       echo "    请在 Q2 中列出所有计划修改的文件。如确为必要: 更新 brief�?
       HARD_FAIL=$((HARD_FAIL + 1))
     else
       echo -e "  ${YELLOW}Q2 未声明的变更文件 (${EXTRA_COUNT} �? 警告):${RESET}"
       echo -e "$EXTRA_FILES"
       WARN_COUNT=$((WARN_COUNT + 1))
     fi
   else
     echo -e "  ${GREEN}实际变更文件均在 Q2 中声�?{RESET}"
   fi
 
   if [ -n "$MISSING_FILES" ]; then
     MISS_COUNT=$(echo -e "$MISSING_FILES" | grep -c . 2>/dev/null || echo 0)
     echo -e "  ${YELLOW}Q2 声明但无实际变更 (${MISS_COUNT} �? 警告):${RESET}"
     echo -e "$MISSING_FILES"
     echo "    可能已删除或计划变更。如不需�? �?Q2 中移除�?
     WARN_COUNT=$((WARN_COUNT + 1))
   fi
 fi
 echo ""
 ...
 # ================================================================
 # 2. Q1 principles vs Done verify: 每条原则对应可验证命�? # ================================================================
 ...
 if [ -f "$PLAN_FILE" ]; then
   ...
   if [ "${PRIN_COUNT:-0}" -gt 0 ]; then
     if [ "${VERIFY_COUNT:-0}" -lt "${PRIN_COUNT:-1}" ]; then
       echo -e "  ${RED}plan �?${PRIN_COUNT} 条原则，�?Done 标准�?${VERIFY_COUNT} �?verify [硬阻断]${RESET}"
       echo "    每条原则必须对应至少一个可验证�?Done 标准�?
       HARD_FAIL=$((HARD_FAIL + 1))
     else
       echo -e "  ${GREEN}原则 ${PRIN_COUNT} �?-> Done verify ${VERIFY_COUNT} �?{RESET}"
     fi
   fi
 else
   echo -e "  ${GREEN}�?plan.json �?跳过原则检�?{RESET}"
 fi
 echo ""
 
 # ================================================================
 # 3. Q3 验收路径验证: verify 命令是否可解�? # ================================================================
 echo -e "${CYAN}--- 3. Q3/Done verify 命令可行�?---${RESET}"
 
 # 提取 Done 标准中所有的 verify: <命令>
 VERIFY_CMDS=$(sed -n '/^## Done/,/^## /p' "$BRIEF" 2>/dev/null | grep 'verify:' | sed 's/.*verify:\s*//' | grep -v '^$')
 
 if [ -z "$VERIFY_CMDS" ]; then
   echo -e "  ${YELLOW}�?verify 命令 �?跳过${RESET}"
 else
   VALID_COUNT=0
   INVALID_COUNT=0
   while IFS= read -r cmd; do
     [ -z "$cmd" ] && continue
     # 检查命令是否是合理�?shell 命令格式 (不以注释开�? 不是纯描�?
     if echo "$cmd" | grep -qE '^[a-zA-Z]|^\./|^npx |^npm |^node |^bash |^git |^grep |^ls |^wc |^find |^curl |^test |^\['; then
       VALID_COUNT=$((VALID_COUNT + 1))
     else
       INVALID_COUNT=$((INVALID_COUNT + 1))
       echo -e "  ${YELLOW}可能不可执行�?verify: ${cmd:0:80}${RESET}"
     fi
   done <<< "$VERIFY_CMDS"
   
   echo -e "  ${GREEN}可执�?verify: ${VALID_COUNT}${RESET}"
   if [ "$INVALID_COUNT" -gt 0 ]; then
     echo -e "  ${YELLOW}警告: ${INVALID_COUNT} �?verify 命令格式可疑${RESET}"
   fi
 fi
 echo ""
 
 # ================================================================
 # 4. (可�? 代码变更是否侵入�?Q2 "不做" 列表
 # ================================================================
 echo -e "${CYAN}--- 4. Q2 禁区检�?---${RESET}"
 
 # 提取 Q2 "不做什�? 区域的关键路�? NO_GO_ZONES=$(sed -n '/^## Q2:/,/^## Q3:/p' "$BRIEF" 2>/dev/null \
   | grep -i '不做\|不修改\|不动\|不改\|不碰\|不涉�? \
 ...
 if [ "$HARD_FAIL" -gt 0 ]; then
   ...
   echo -e "${YELLOW}brief-vs-code: ${WARN_COUNT} 项警�?(不阻�?${RESET}"
   exit 0
 else
   echo -e "${GREEN}brief-vs-code: 全部一�?{RESET}"
   exit 0
 fi
