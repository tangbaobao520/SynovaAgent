#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# Loop Engineering v4.3.0 — PreToolUse: task brief 质量检查
#
...
if [ -f "$LOCK_FILE" ]; then
  ...
  if echo "$FILE" | grep -qE 'scripts/workflow/task-start|scripts/workflow/scope-check|\.claude/task-briefs/|\.claude/settings'; then
    ...
    echo "⛔ v4.3.0 Session Lock — 请先运行 task-start 完成 Q0-Q3"
...
if [ "$FAIL" -gt 0 ]; then
  ...
  # v4.3.0: PreToolUse exit code 在 VSCode Extension 中被忽略（不阻断 Write）。
