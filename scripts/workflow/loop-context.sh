 #!/bin/bash
 # Loop Engineering V4.4.1 �?长运行熔断器 + 状态记忆检�?
 #
 # 检测需要熔断的信号:
 #   1. verify-incremental 在单个任务中打到 5 轮上�?
 #   2. STATE.md 同一免疫警告 24h 内累积超过阈�?
 #   3. pre-commit 同一组在 24h 内连续失败超�?10 �?
 #   4. --no-verify �?24h 内使用超�?2 �?
 #
 # 退�?0 = 正常, 退�?1 = 熔断触发 (需要人工介�?
 #
 ...
 echo ""
 echo -e "${CYAN}Loop Engineering V4.4.1 �?熔断器检�?{RESET}"
 echo ""
 ...
 if [ -f "$LOOP_STATE" ]; then
   ...
   ITER=${ITER//[^0-9]/}
   if [ "${ITER:-0}" -ge 5 ]; then
     echo -e "  ${RED}熔断: verify-incremental 已达 ${ITER} 轮上�?{RESET}"
     echo "    当前任务已消耗全部自动修正轮次。请人工检查并修复根因�?
     BREAKER_TRIPPED=1
   elif [ "${ITER:-0}" -ge 3 ]; then
     echo -e "  ${YELLOW}警告: verify-incremental �?${ITER} �?(上限 5)${RESET}"
   else
     echo -e "  ${GREEN}循环轮次: ${ITER}/5${RESET}"
   fi
 else
   echo -e "  ${GREEN}循环轮次: 无活跃任�?{RESET}"
 fi
 ...
 if [ -f "$ROOT/STATE.md" ]; then
   ...
   if [ "${CRITICAL_COUNT:-0}" -gt 50 ]; then
     echo -e "  ${RED}熔断: 今日免疫警告 ERROR 级别 ${CRITICAL_COUNT} �?(阈�?50)${RESET}"
     echo "    系统在重复触发同样的错误。请检查免疫警告模式并修复根因�?
     BREAKER_TRIPPED=1
   elif [ "${CRITICAL_COUNT:-0}" -gt 20 ]; then
     echo -e "  ${YELLOW}警告: 今日免疫警告 ERROR ${CRITICAL_COUNT} �?{RESET}"
   else
 ...
 if [ -f "$FAILURE_LOG" ]; then
   ...
   if [ "${FAIL_COUNT:-0}" -gt 20 ]; then
     echo -e "  ${RED}熔断: 今日 pre-commit 失败 ${FAIL_COUNT} �?(阈�?20)${RESET}"
     echo "    门禁本身可能存在误报或过于敏感。请检�?check-secrets / as any / empty catch 的误报率�?
     BREAKER_TRIPPED=1
   elif [ "${FAIL_COUNT:-0}" -gt 10 ]; then
     echo -e "  ${YELLOW}警告: 今日 pre-commit 失败 ${FAIL_COUNT} �?{RESET}"
   else
     echo -e "  ${GREEN}pre-commit 失效�? ${FAIL_COUNT} �?(正常)${RESET}"
   fi
 fi
 ...
 if [ -f "$BYPASS_LOG" ]; then
   ...
   if [ "${BYPASS_COUNT:-0}" -ge 3 ]; then
     echo -e "  ${RED}熔断: 今日 --no-verify ${BYPASS_COUNT} �?(阈�?3)${RESET}"
     echo "    �?pre-commit �?7 组检查。请修复根因而非绕过�?
     BREAKER_TRIPPED=1
   elif [ "${BYPASS_COUNT:-0}" -ge 2 ]; then
     echo -e "  ${YELLOW}警告: 今日 --no-verify ${BYPASS_COUNT} �?{RESET}"
   else
     echo -e "  ${GREEN}绕过检�? ${BYPASS_COUNT} �?(正常)${RESET}"
   fi
 fi
 ...
 " 2>/dev/null
   echo -e "${RED}熔断已写�?.claude/breaker-state.json${RESET}"
   exit 1
 else
   rm -f "$BREAKER_FILE"
   exit 0
