 #!/bin/bash
 # Loop Engineering V4.4.1 �?Post-Merge Cleanup
 #
 # �?git merge �?main 后运行。扫描并报告:
 #   1. 残留 TODO/FIXME (无对�?Issue 编号�?
 #   2. 废弃的函�?export (grep 零引�?
 #   3. 旧文件残�?(已知迁移后的旧路�?
 ...
 echo ""
 echo -e "${CYAN}Loop Engineering V4.4.1 �?Post-Merge Cleanup${RESET}"
 echo -e "${CYAN}模式: $( $REPORT_ONLY && echo 'L1 仅报�? || echo 'L2 自动清理' )${RESET}"
 echo ""
 
 # 1. 残留 TODO/FIXME (�?Issue 编号)
 ...
 if [ -n "$ORPHAN_TODOS" ]; then
   echo -e "${YELLOW}发现�?Issue 编号�?TODO/FIXME:${RESET}"
   echo "$ORPHAN_TODOS" | while read -r line; do echo "  $line"; done
   ISSUES=$((ISSUES + 1))
 else
   echo -e "  ${GREEN}无残�?TODO/FIXME${RESET}"
 fi
 echo ""
 
 # 2. 过期 task brief (超过 30 �?
 ...
 if [ -n "$MIGRATION_ISSUES" ]; then
   ...
   echo -e "  ${GREEN}已知迁移路径已清�?{RESET}"
 fi
 echo ""
 
 # 4. 大型未引用文�?(>200�?且在 server.ts/routes/ 中无引用)
 echo -e "${CYAN}--- 4. 潜在死代�?(大文件零引用) ---${RESET}"
 UNREFERENCED=""
 while IFS= read -r file; do
   ...
   if [ "${LINES:-0}" -gt 200 ]; then
     ...
     if [ -z "$REFS" ]; then
       UNREFERENCED="${UNREFERENCED}  ${file} (${LINES} �?\n"
     fi
   fi
 ...
 echo "=========================================="
 if [ "$ISSUES" -eq 0 ]; then
   echo -e "${GREEN}Post-Merge Cleanup: 未发现问�?{RESET}"
 else
   echo -e "${YELLOW}Post-Merge Cleanup: 发现 ${ISSUES} 类问�?{RESET}"
