#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
# verify-incremental.sh — PostToolUse 分层增量验证 (Loop Engineering V4.5.1)
#
...
while IFS= read -r _bfile; do
  [ -z "$_bfile" ] && continue
  [ ! -f "$_bfile" ] && continue
  _bimports=$(grep "^import\|from" "$_bfile" 2>/dev/null | grep -v "import type") || _bimports=""  # D964: 非文本/已删文件=显式空跳过
