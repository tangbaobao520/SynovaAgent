#!/bin/bash
# ══════════════════════════════════════════════════════════════════════════════�?
# verify-incremental.sh �?PostToolUse 分层增量验证 (Loop Engineering V4.4.1)
#
# L1: oxlint 语法检�?(< 1s, 改动文件)
...
if [ "$ITER" -gt "$MAX" ]; then
  echo -e "${RED}[LOOP] 已达最大循环次�?$MAX，停止自动修正。请人工介入�?{RESET}"
  rm -f "$STATE_FILE"
  exit 0
fi
...
echo -e "${CYAN}[VERIFY $ITER/$MAX] 分层增量验证开�?..${RESET}"
...
if [ "$IS_LIGHT" -eq 1 ]; then
  echo -e "${YELLOW}[LIGHT] 轻量变更通道 (仅Markdown/HTML/JSON �?�?行改�?${RESET}"
...
if [ -n "$CHANGED_SRC" ]; then
  echo -e "${CYAN}[L1] oxlint 语法检�?..${RESET}"
  ...
  if [ -n "$OXLINT_AVAILABLE" ]; then
    ...
    if npx oxlint $OXLINT_FILES --silent 2>&1; then
      ...
      echo -e "${RED}[FAIL] L1 语法检查失�?�?请修正语法错�?{RESET}"
      exit 1
    fi
  else
    echo -e "${YELLOW}  L1 语法: oxlint 未安�? 跳过 (建议: npm install -D oxlint)${RESET}"
  fi
else
  echo -e "${CYAN}[L1] �?.ts 文件改动, 跳过语法检�?{RESET}"
fi

# ══�?L2: tsc --noEmit --incremental (利用 .tsbuildinfo 缓存) ══�?
if [ -n "$CHANGED_SRC" ]; then
  echo -e "${CYAN}[L2] tsc 类型检�?(incremental)...${RESET}"
  # 使用 --incremental 利用 .tsbuildinfo 缓存, 只检查改动文�?
  if npx tsc --noEmit --incremental 2>&1 | grep -E "^src/|^tests/" | head -20; then
    ...
    if [ "${TSC_ERRORS:-0}" -gt 0 ]; then
      echo -e "${RED}[FAIL] L2 类型检�? ${TSC_ERRORS} 个错�?{RESET}"
      exit 1
    fi
  fi
  echo -e "${GREEN}  L2 类型: 通过${RESET}"
else
  echo -e "${CYAN}[L2] �?.ts 文件改动, 跳过类型检�?{RESET}"
fi
...
if [ -n "$CHANGED_SRC" ]; then
  ...
  if [ -n "$TEST_FILES" ]; then
    ...
    echo -e "${CYAN}[L3] 无对应测试文�? 跳过${RESET}"
  fi
else
  echo -e "${CYAN}[L3] �?.ts 文件改动, 跳过测试${RESET}"
fi

# ══�?L4: 综合门禁 (接线 + 架构 + 暗默失败 + 用户可见) ══�?
echo -e "${CYAN}[L4] 综合门禁...${RESET}"

# L4a. 接线审计 (新文�?export 验证)
...
if [ -n "$TS_DIFF" ]; then
  ...
  if [ -n "$NEW_CATCHES" ]; then
    ...
    if [ -n "$SILENT" ]; then
      echo -e "${RED}[FAIL] 暗默失败: 新增 catch �?log:${RESET}"
      echo -e "$SILENT"
...
# L4d. 用户可见缺口检�?(新增 export 无对�?API 变更)
...
if [ -n "$NEW_EXPORTS_ALL" ] && [ -z "$ROUTE_CHANGED" ]; then
  # �?src 文件改动但没�?route 文件改动 �?可能遗漏用户入口
  ...
  if [ -n "$NEW_FUNCS" ]; then
    ...
    if [ -n "$GAP" ]; then
      ...
      echo -e "$GAP"
      echo -e "${YELLOW}  新增了导出但未更�?API 路由。如果这是内部函数请忽略�?{RESET}"
      # 不阻断，仅警�?
    fi
  fi
fi
