...
export class ConflictArbitrator {
  ...
  private convergenceEngine: import('./convergence-engine').ConvergenceEngine | null;

  constructor(
    auditStore?: AuditStoreLike | null,
    expertScores?: Record<string, number>,
    convergenceEngine?: import('./convergence-engine').ConvergenceEngine | null,
  ) {
    this.auditStore = auditStore ?? null;
    this.expertScores = { ...DEFAULT_SCORES, ...expertScores };
    this.convergenceEngine = convergenceEngine ?? null;
  }
  ...
  private async arbitrateConflict(conflict: Conflict, tieBreaker?: TieBreakerResult): Promise<ArbitrationResult> {
    // D8f: 检查收敛规则 — 如果存在则跳过仲裁（使用注入的共享引擎）
    if (this.convergenceEngine) {
      try {
        const convergence = this.convergenceEngine.getConvergence(conflict.edgeId, conflict.experts);
