 ...
 *      伴生内部记账（stickyDegradedSessions 聚合计，探针可读）。
 *      D865 P1-2: **不再反向改写 store.lastDegraded**——伴生只观测不写业务状态
 *      （G6 能力面约束）：旧写法会让 conversations.ts 的 lastDegraded 消费方对
 *      后续健康轮次误发 STORE_DEGRADED 错误帧（降级信号失真，K3 D831 P1-2）。
 *
 * 本不变量不检查什么（总纲 §9.4 硬要求，显式清单）:
 *   1. 不检查 lastDegraded 的"本次写入"per-op 语义本身——session-store
 *      2026-08-22 设计决定（transient 恢复不误报）；D865 起聚合粘滞只在伴生内部
 *      记账（stickyDegradedSessions），不改写 store 字段（P1-2）
...
export const onDegradedSticky: InvariantCompanion = {
  ...
  install({ fail, hit, onRollback }): void {
    ...
    SessionStore.prototype.addMessage = function (
      ...
      if (!writeDegraded && stickyDegradedSessions.has(sessionId)) {
        // B. 粘滞恢复（D865 P1-2 修订）: 伴生只观测不写业务状态（G6 能力面约束）——
        // 聚合降级在伴生内部记账（stickyDegradedSessions，探针可读），不再反向改写
        // store.lastDegraded（旧写法导致路由层对后续健康轮次误发 STORE_DEGRADED 帧）
        log.debug({ sessionId }, 'INV-DEGRADED-STICKY: 聚合降级粘滞（伴生内部记账，不改写 store 字段）');
      }
    };
