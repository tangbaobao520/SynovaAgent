 ...
 *   6. 不检查生产 message 双写路径——conversation-engine.ts:785/790（小队 A 地盘）
 *      双写不带 tool_calls/tool_call_id → 本伴生生产触发面=0（接线≠触发，K3 D831
 *      P1-3）；待 D866（小队 A 在双写点携带 tool_calls/tool_call_id）后收紧
 *
 * 契约（铁律 47）:
...
export const onToolCallPairing: InvariantCompanion = {
  ...
  notChecked: [
    ...
    '生产 message 双写路径不携带 tool_call_id（D866 前不检查；当前生产触发面=0）',
  ],
