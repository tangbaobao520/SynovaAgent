 ...
 *
 * 契约（铁律 47）:
 *   @input  — ctx: InvariantContext（fail/hit/onRollback/flagAnomaly）
 *   @output — void；wrap 后的 fetch 违约时 reject InvariantError——由韧性层
 *             （retry-middleware，D865）与工具循环原样上抛，HTTP 入口在响应头
 *             未发出时映射 5xx（conversations.ts INVARIANT_VIOLATED；头已发出=流内帧+断流）
 *   @degraded — 检查层自身异常（body 解析失败等）→ log.warn 放行原请求
 *             （检查器不得成为新的故障源；InvariantError 本身照常抛出）
 *   @error  — 违约 → fail()（InvariantError，fail-closed）；回滚遇"fetch 已被他人
 *             替换" → log.error + flagAnomaly（探针可见，D865 P2-3，防静默失效假绿）
 */
...
export const onToolSchemaSent: InvariantCompanion = {
  ...
  install({ fail, hit, onRollback, flagAnomaly }): void {
    ...
    onRollback(() => {
      if (globalThis.fetch === wrappedFetch) {
        ...
        // D865 P2-3: 只 warn 不还原 → 不变量静默失效而探针仍显示 registered=3（假绿）。
        // 升级 log.error + flagAnomaly 进 snapshot（探针可见）
        const msg = '回滚时 globalThis.fetch 已被他人替换 — 不变量可能静默失效，需人工核查';
        log.error({ degraded: true }, `INV-TOOL-SCHEMA-SENT: ${msg}`);
        flagAnomaly(msg);
      }
    });
  },
