...
/** 违约错误：带稳定 code + 归属（owner + packageName）；上抛链 fail-closed
 * （D865: 韧性层不吞 → HTTP 入口响应头未发出时映射 5xx，见 conversations.ts） */
...
export interface InvariantContext {
  ...
  /**
   * 登记非违约异常（D865 P2-3）：不抛、不卸载，但进 snapshot 的 anomalies——
   * 健康探针可见（如 fetch 回滚遇"已被他人替换"→ 不变量可能静默失效，禁只在日志 warn）
   */
  flagAnomaly(message: string): void;
}
...
export interface InvariantStatus {
  ...
  /** 非违约异常登记（D865 P2-3，探针可见——防"回滚遇替换"类静默失效假绿） */
  anomalies: string[];
}
...
export interface InvariantSnapshot {
  ...
  /** 注册表级累积异常登记（D865 P2-3）——条目卸载后仍可见（探针防假绿） */
  anomalies: string[];
}
...
export class InvariantRegistry {
  ...
  /** 注册表级累积异常（D865 P2-3）——伴生卸载后仍保留，探针可见 */
  ...
  install(companion: InvariantCompanion): void {
    ...
    const entry: Entry = { def: companion, hitCount: 0, lastFailure: null, anomalies: [], rollbacks: [] };
    ...
    try {
      companion.install({
        ...
        flagAnomaly: (message: string) => {
          entry.anomalies.push(message);
          this.anomalies.push(`[${companion.code}] ${message}`);
          log.error({ code: companion.code, owner: companion.owner }, `不变量异常登记（非违约）: ${message}`);
        },
      });
    } catch (err) {
  ...
  snapshot(): InvariantSnapshot {
    ...
    return { registered: this.entries.size, invariants, anomalies: [...this.anomalies] };
  }
