...
# ── 边界: dry-run 不建壳 ──
DRY=$(bash "$GATE" "test-dry" --dry-run 2>/dev/null | head -1)
...
SANDBOX=$(mktemp -d)
trap 'rm -rf "$SANDBOX"; rmdir "$REPO/.alloc-task-id.lock" 2>/dev/null; true' EXIT
for i in $(seq 1 20); do
  SYNO_TASK_STATE_DIR="$SANDBOX" bash "$GATE" "并发测试-$i" >/dev/null 2>&1 &
done
wait
# 统计沙箱里生成的号
IDS=$(ls "$SANDBOX"/D*.json 2>/dev/null | sed 's/.*\/D\([0-9]*\)\.json/\1/' | sort -n)
