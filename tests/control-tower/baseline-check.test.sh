...
CT_DIR="$TMP_DIR/bc-ct"

echo "── 1. seed 3 条 → 5 条运行 → 存量 3 + 新增 2 ──"
rm -rf "$CT_DIR"; mkdir -p "$CT_DIR"
SYNO_CT_DIR="$CT_DIR" SYNO_TSC_OUTPUT="$TMP_DIR/bc-known.txt" \
  bash "$TOOL" --tsc --seed > /dev/null 2>&1 || true
EXIT=0
OUT=$(SYNO_CT_DIR="$CT_DIR" SYNO_TSC_OUTPUT="$TMP_DIR/bc-new.txt" \
...
EXIT=0
OUT=$(SYNO_CT_DIR="$CT_DIR" SYNO_TSC_OUTPUT="$TMP_DIR/bc-known.txt" \
  bash "$TOOL" --tsc 2>&1) || EXIT=$?
...
EXIT=0
OUT2=$(SYNO_CT_DIR="$CT_DIR" SYNO_TSC_OUTPUT="$TMP_DIR/bc-new.txt" \
  bash "$TOOL" --tsc 2>&1) || EXIT=$?
