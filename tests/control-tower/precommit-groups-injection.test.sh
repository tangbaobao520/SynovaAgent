...
REPO_RESIDUE="$(grep -rl "$MARK" "$REPO_DIR" --exclude-dir=node_modules --exclude-dir=.git 2>/dev/null | wc -l | tr -d ' ')"   # swallow-ok: 同上探测型计数（b 面期望 =base 基线 6）；非探测路径不可达时计数为 0 会由断言判红，不静默
...
echo "b) 仓库全量命中: $REPO_RESIDUE 个文件（base 实测基线 6：docs/ 证据+preset+K3 报告，逐行如下）"
...
[ "$REPO_RESIDUE" -gt 6 ] && RESIDUE_FAIL=1
[ "$CLONE_POSITIVE" -eq 0 ] && RESIDUE_FAIL=1
[ "$RESIDUE_FAIL" -eq 0 ] && echo "✅ 残留断言满足（a=0, b<=6, c>0）" || echo "❌ 残留断言不满足（a=${CODE_RESIDUE}, b=${REPO_RESIDUE}, c=${CLONE_POSITIVE}）"
