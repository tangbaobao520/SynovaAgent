...
describe('simulateInvestment', () => {
  it('返回完整模拟结果', async () => {
    ...
    const result = simulateInvestment('test-cycle', 100, '扩大产能', TEST_CYCLE, store, [TEST_CYCLE]);
  ...
  it('传导描述包含路径信息', async () => {
    const { simulateInvestment } = await import('../../src/cycles/investment-advisor');
    const store = createMockStore();
    const result = simulateInvestment('test-cycle', 200, '优化节点', TEST_CYCLE, store, [TEST_CYCLE]);
  ...
  it('承诺清单含 can_do / cannot_do 标注', async () => {
    const { simulateInvestment } = await import('../../src/cycles/investment-advisor');
    const store = createMockStore();
    const result = simulateInvestment('test-cycle', 0, '', TEST_CYCLE, store, [TEST_CYCLE]);
