...
describe('findDiagnosticPaths', () => {
  it('Given connected Person→Risk path, When findDiagnosticPaths, Then returns diagnostic paths', () => {
    const store = fakeStore(
      [{ id:'p1', type:SOGNodeType.PERSON }, { id:'r1', type:SOGNodeType.RISK }],
      [{ id:'e1', type:SOGEdgeType.AFFECTS, from:'r1', to:'p1', weight:0.9 }],
    );
    const paths = findDiagnosticPaths(store, 'g', SOGNodeType.RISK, SOGNodeType.PERSON);
  ...
  it('Given empty graph, When findDiagnosticPaths, Then returns empty', () => {
    const store = fakeStore();
    expect(findDiagnosticPaths(store, 'g', SOGNodeType.RISK, SOGNodeType.PERSON)).toHaveLength(0);
  });
  ...
  it('Given no connection between types, When findDiagnosticPaths, Then returns empty', () => {
    ...
    expect(findDiagnosticPaths(store, 'g', SOGNodeType.PERSON, SOGNodeType.RISK)).toHaveLength(0);
  });
});
...
describe('summarizeSubgraph', () => {
  it('Given a subgraph around a root node, When summarizeSubgraph, Then returns type distribution and anomaly score', () => {
    const store = fakeStore(
      [{ id:'root', type:SOGNodeType.RISK }, { id:'p1', type:SOGNodeType.PERSON }, { id:'p2', type:SOGNodeType.PERSON }],
      [{ id:'e1', type:SOGEdgeType.AFFECTS, from:'root', to:'p1', weight:0.9 },
       { id:'e2', type:SOGEdgeType.AFFECTS, from:'root', to:'p2', weight:0.7 }],
    );
  ...
  it('Given isolated node, When summarizeSubgraph, Then returns nodeCount=1, edgeCount=0', () => {
    const store = fakeStore([{ id:'lonely', type:SOGNodeType.RISK }]);
...
describe('getGraphDiff', () => {
  it('Given a graph with nodes, When getGraphDiff, Then returns diff struct', () => {
    const store = fakeStore([{ id:'n1', type:SOGNodeType.PERSON }]);
...
describe('findCrossDimensionalBrokers', () => {
  it('Given a connected graph with 5+ nodes, When findCrossDimensionalBrokers, Then returns brokers sorted by betweenness', () => {
    ...
    const edges = [{ id:'e1', type:SOGEdgeType.INTERACTS_WITH, from:'center', to:'l1', weight:0.9 },
      { id:'e2', type:SOGEdgeType.INTERACTS_WITH, from:'center', to:'l2', weight:0.9 },
      { id:'e3', type:SOGEdgeType.INTERACTS_WITH, from:'center', to:'l3', weight:0.9 },
      { id:'e4', type:SOGEdgeType.INTERACTS_WITH, from:'center', to:'l4', weight:0.9 }];
  ...
  it('Given 2-node graph, When findCrossDimensionalBrokers, Then returns empty (too small)', () => {
    const store = fakeStore([{ id:'a', type:'X' }, { id:'b', type:'Y' }],
      [{ id:'e1', type:SOGEdgeType.INTERACTS_WITH, from:'a', to:'b', weight:1 }]);
...
describe('detectAnomalousPatterns', () => {
  ...
  it('Given edges with extreme weight, When detectAnomalousPatterns, Then anomaly detected', () => {
    const edges = Array.from({ length: 10 }, (_, i) => ({
      id: `e${i}`, type: SOGEdgeType.INTERACTS_WITH, from: `n${i}`, to: `n${i+1}`, weight: 0.5,
    }));
    edges.push({ id:'extreme', type:SOGEdgeType.INTERACTS_WITH, from:'n10', to:'n11', weight: 500 });
    const store = fakeStore(
